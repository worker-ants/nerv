...
export class SpecService {
   ...
   *
   * `root`(안정 키 또는 UUID)·`depth`·`status` 로 좁힐 수 있다. **아무것도 없으면 전
   * 계층**이다 — 화면은 그것을 쓴다(screens.md REQ-WEB-044: 깊이는 성능 장치가 아니다).
   *
   * `status` 로 거른 결과에는 **조상이 함께 온다**(REQ-API-092) — 매칭만 남기면 부모를 잃은
   * 노드가 생기고, 받는 쪽은 트리를 그릴 수 없다. 조상은 `matched: false` 로 갈린다.
   *
  ...
  async tree(input: {
   ...
   LEFT JOIN spec_version sv ON sv.id = s.current_version_id
    ...
    const pruned = pruneTree(rows, { root: input.root ?? null, depth, statuses });
