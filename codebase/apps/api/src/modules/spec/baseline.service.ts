// 베이스라인 동결·조회 + as-of/baseline manifest — E09-S06
// 정본: spec-workflow.md §3.6 · data-model.md §2.2 · api.md §2.2(REQ-API-015)
//
// 베이스라인은 **"이때의 스펙 세트"를 이름으로 고정**하는 장치다. 릴리스·감사·계약의 기준선이라
// 한 번 만들어지면 영원히 같은 답을 내야 한다 — 핀된 버전이 나중에 superseded 가 되어도
// 조회 결과는 그대로다. 그래서 `spec_baseline_item` 은 spec_version_id 를 직접 핀하고,
