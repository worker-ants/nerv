  ...
  return (
    ...
    >
      ...
      <dl className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-text-faint">
        <div>
          <dt className="inline">{t('session.heartbeat')}&nbsp;</dt>
          <dd className="inline">{relativeTime(t, card.last_heartbeat_at, now)}</dd>
        </div>
        <div>
          <dt className="inline">{t('session.lease_remaining')}&nbsp;</dt>
          <dd className="inline font-mono">{leaseRemaining(t, card.lease_remaining_seconds)}</dd>
        </div>
        <div>
        ...
        {card.branch !== null && (
          <div>
            <dt className="inline">{t('session.branch')}&nbsp;</dt>
      ...
      {(card.scope_spec_ids.length > 0 || card.scope_file_globs.length > 0) && (
        <ul className="flex flex-wrap gap-1" aria-label={t('session.scope')}>
