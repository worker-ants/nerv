...
describe('E06-S02 — md 왕복 실측', () => {
  ...
  //   · 전수 레인(`NERV_ROUNDTRIP_FULL=1`) — 리포트의 비율을 여기서 잰다(2026-09-22 부터
  //     그 비율은 **판정이 아니라 기록**이다 — 아래 "비율에 상한을 두지 않는다")
  //
  ...
  it(
    ...
    () => {
      ...
      //
      // **비율에 상한을 두지 않는다**(2026-09-22 · REQ-WEB-173). 예전에는 전수 레인에서
      // `>= 0.85` 를 걸었다. 그 상한이 지키던 것은 "직렬화가 더 나빠지지 않았는가" 인데,
      // 웹이 md 를 되돌려 보내지 않게 되면서 **직렬화가 어디에도 호출되지 않는다** —
      // 지켜 줄 대상이 사라진 상한은 측정이 아니라 통행세다.
      //
      // 그리고 그 상한은 **문서를 쓰면 깨졌다.** 표본이 이 저장소의 `docs/**/*.md` 라,
      // 손실 유형(표 · 연속 인용문 · 대괄호)을 담은 문서를 한 편 더 쓰면 비율이 내려간다 —
      // 실제로 이 커밋이 그렇게 깨뜨렸고(19/23 = 82.6%), 그때 나빠진 것은 직렬화가 아니라
      // **표본**이었다. 비용이 자라는 검사에 고정 상한을 두지 않는다(AGENTS.md 규약 7).
      //
      // 남는 것은 **리포트**다. 손실 유형이 아는 둘뿐인지는 아래 한 줄이 여전히 본다 —
      // 모르는 유형이 나오면 그때가 다시 볼 때다(scope.md §2.2 점화 기록 ②).
