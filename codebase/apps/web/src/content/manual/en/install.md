...
- Scopes start as **[Recommended]** — what an agent needs for the five skills (next · spec · impl · question · review): `spec:read` · `spec:draft` · `task:claim` · `task:update` · `review:submit` · `agent-session:launch`. Whatever your role lacks is left out when it is issued (a locked box is never issued behind your back). To give something else, switch to **[Choose myself]** and tick the boxes.
