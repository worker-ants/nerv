Settings has four tabs: **Organisation & projects, Members, Tokens and Gate policy**.

## Organisation & projects

Of the two header selects, it is the **project** you create, rename and put away here (there is effectively one organisation today, so there is little to switch between).
...
- **Delete an organisation** — possible **only while it holds no projects**, because an irreversible act should have a reversible step in front of it. **Archiving does not satisfy that condition, though** — archived projects still count, and the screen offers no way to delete a project. So an organisation that ever held a project cannot be deleted from this screen today.
...
Gate policy is **per project**, not one set for the whole organisation. With no project selected in the header it cannot be saved.
...
**A new document that establishes requirements goes past a person once.** A newly created spec has no earlier revision to revert to and nothing referencing it yet, so it scores low. Left alone, a document that establishes new requirements would be approved without a person ever seeing it. So the first revision alone gets the extra step. From the second revision on, the score decides.
