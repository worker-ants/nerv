...
그래프 조작 도구는 **캔버스 왼쪽 위**에 모여 있습니다. 보기 범위(전체/이 문서 중심) · 영역으로 묶기 · [다른 배치], 그리고 **`?`** 입니다. `?` 를 누르면 **읽는 법**(색·크기·흐려짐)과 **조작**, **빠른 그리기(WebGL)** 스위치가 함께 열립니다. 노드와 간선 수는 오른쪽 아래 모서리에, 범례는 왼쪽 아래 모서리에 있습니다.

**같은 문서와 관계라면 언제 열어도 같은 배치입니다.** 새로 고쳐도, 다른 사람이 열어도 문서가 같은 자리에 있으므로 자리를 익혀 두고 찾을 수 있습니다. 창 크기가 다르면 배치도 조금 달라집니다. 에이전트가 문서 내용이나 상태만 바꿀 때는 그림이 그대로이고, 문서나 관계가 늘거나 줄 때만 다시 배치합니다.

**[다른 배치]** 를 누르면 다른 배치를 봅니다. 엉킨 곳을 풀 때 씁니다. 누를 때마다 배치 번호가 하나씩 올라가고 번호가 주소에 남으므로, 그 배치를 링크로 건넬 수 있습니다. 번호가 1이 아니면 옆에 **배치 N** 과 **[처음 배치]** 가 표시되고, [처음 배치]를 누르면 기본 배치로 돌아갑니다.

**노드는 끌어서 옮길 수 있습니다.** 겹친 노드를 직접 떼어 놓을 때 씁니다. 옮긴 위치는 이 화면에서만 유지되고 문서의 트리 위치는 바뀌지 않습니다. 노드를 옮기면 **[옮긴 자리 되돌리기]** 가 나타나고, 누르면 이 배치의 원래 자리로 돌아갑니다. 중심 모드·영역 묶기를 바꾸거나 다른 배치를 보면 옮긴 자리는 사라집니다.

그래프는 **WebGL** 로 그려 확대·이동이 부드럽습니다. 그림이 깨져 보이면 `?` 에서 **빠른 그리기(WebGL)** 를 끄세요. 화면을 다시 불러와 일반 캔버스로 그립니다. 지원하지 않는 브라우저에서는 처음부터 일반 캔버스로 그립니다.
