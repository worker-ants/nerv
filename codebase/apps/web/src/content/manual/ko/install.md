...
- 스코프는 **`spec:read` 와 `task:claim` 둘만 켜진 채로** 시작합니다(내 역할에 없는 쪽은 처음부터 꺼진 채입니다 — 잠긴 칸이 몰래 발급되지는 않습니다). 역할 프리셋 같은 것은 없습니다 — 필요한 것을 직접 켭니다. 플러그인으로 작업까지 돌리려면 `task:update` · `agent-session:launch` 를, 초안을 쓰려면 `spec:draft` 를, 리뷰를 올리려면 `review:submit` 을 더 켭니다.
- **토큰은 역할보다 넓어지지 않습니다.** 내 역할에 없는 스코프는 **흐리게 잠겨** 보입니다. 예를 들어 `review:resolve` 는 admin · planner · qa 의 것이라 `developer` 에게는 잠겨 있습니다. 목록에서 감추지 않는 이유는 사람 전용 스코프와 같습니다 — 왜 못 주는지가 화면 안에 있어야 합니다.
...
그리고 **재시작**합니다. `/plugin` 목록에 `nerv` v0.2.0이 활성으로 보이면 됩니다.
