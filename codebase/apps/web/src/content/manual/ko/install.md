...
- 발급 직후 카드가 **이 토큰이 무엇인지 적고**(프로젝트·권한·만료) **연결 3단계**를 차례로 줍니다 — ① 토큰(복사 단추) ② 플러그인 설치 두 줄(Claude Code 에서) ③ 작업 저장소에서 실행할 `nerv-init` 한 줄. ③ 은 설치 캐시에서 `nerv-init` 을 찾아 부르는 모양이라 그대로 붙여넣으면 되고, 아래 2·3단계의 파일을 한 번에 만듭니다. **이 한 줄은 토큰을 셸 기록에 남깁니다** — 남기지 않으려면 `--token` 과 그 값을 빼고 실행하세요. 그러면 토큰을 가려서 묻습니다. 카드는 그 토큰이 **처음 쓰이면 "연결됨 — 기계 이름"** 으로 바뀝니다.
