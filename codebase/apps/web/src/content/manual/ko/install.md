...
- 발급된 목록의 **첫 열이 프로젝트**입니다. 폐기·만료된 토큰은 접혀 있고 [폐기·만료된 것도 보기]로 폅니다. **조직 admin**(조직 전체 역할이 admin 인 사람)이라면 그 아래에 **조직 전체 토큰 표**가 서서 누가 어느 프로젝트에 무슨 토큰을 갖고 있는지 보입니다. **남의 토큰을 폐기하는 것도 조직 admin 만** 합니다 — 한 프로젝트의 admin 은 자기 토큰만 폐기합니다.
