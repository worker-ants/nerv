...
- 스코프는 **`spec:read` 와 `task:claim` 둘만 켜진 채로** 시작합니다(내 역할에 없는 쪽은 처음부터 꺼진 채입니다 — 잠긴 칸이 몰래 발급되지는 않습니다). 역할 프리셋 같은 것은 없습니다 — 필요한 것을 직접 켭니다.
- **`agent-session:launch` 는 선택이 아닙니다.** `nerv_bootstrap` 이 그 스코프를 요구하므로, 켜지 않으면 아래 4·5단계가 처음부터 막힙니다. 기본 둘에 이것을 더한 셋이 사실상의 최소입니다. 그 위로 작업을 돌리려면 `task:update`, 초안을 쓰려면 `spec:draft`, 리뷰를 올리려면 `review:submit` 을 켭니다.
