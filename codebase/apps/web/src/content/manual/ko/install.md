...
| 프로젝트 slug  | 지금 프로젝트(왼쪽 열)       | `{{project}}` |
| 내 역할        | 설정 → 멤버                  | `{{role}}`    |
...
- **토큰은 프로젝트 하나에 묶입니다.** 발급 폼의 첫 칸에서 **프로젝트를 고릅니다**(기본값은 지금 보고 있는 프로젝트입니다). 그 프로젝트에서만 쓰므로, 프로젝트가 여럿이면 토큰도 따로 냅니다. 고른 프로젝트가 **권한 칸의 잠금도 정합니다** — 역할은 프로젝트마다 다를 수 있습니다. 소속된 프로젝트가 없으면 발급 단추가 눌리지 않고, 화면이 그 이유를 적습니다.
- **만료를 고를 수 있습니다** — 30일 · 90일 · 1년 · 만료 없음. 기본은 만료 없음입니다. 만료된 토큰은 그 순간부터 거절되니 폐기와 같은 효과이고, 목록에서 언제까지인지 보입니다.
- 발급 직후 카드가 **이 토큰이 무엇인지 적고**(프로젝트·권한·만료) **연결 3단계**를 차례로 줍니다 — ① 토큰(복사 단추) ② 플러그인 설치 두 줄(Claude Code 에서) ③ 작업 저장소에서 실행할 `nerv-init` 한 줄. ③ 은 설치 캐시에서 `nerv-init` 을 찾아 부르는 모양이라 그대로 붙여넣으면 되고, 아래 2·3단계의 파일을 한 번에 만듭니다. **이 한 줄은 토큰을 셸 기록에 남깁니다** — 남기지 않으려면 `--token` 과 그 값을 빼고 실행하세요. 그러면 토큰을 가려서 묻습니다. 카드는 그 토큰이 **처음 쓰이면 "연결됨 · 머신 이름"** 으로 바뀝니다.
- 발급된 목록의 **첫 열이 프로젝트**입니다. 폐기·만료된 토큰은 접혀 있고 [폐기·만료된 것도 보기]로 폅니다. **조직 admin**(조직 전체 역할이 admin 인 사람)이라면 그 아래에 **조직 전체 토큰 표**가 서서 누가 어느 프로젝트에 무슨 토큰을 갖고 있는지 보입니다. 그 표의 **[폐기]로 남의 토큰을 끊는 것도 조직 admin 만** 합니다 — 한 프로젝트의 admin 은 자기 토큰만 폐기합니다. 폐기는 되돌릴 수 없어 한 번 더 묻습니다.
- 권한은 **[권장]** 으로 시작합니다 — 에이전트가 스킬 다섯(next·spec·impl·question·review)을 쓰는 데 드는 `spec:read` · `spec:draft` · `task:claim` · `task:update` · `review:submit` · `agent-session:launch` 입니다. 내 역할에 없는 것은 빠진 채 나갑니다(잠긴 칸이 몰래 발급되지는 않습니다). 다르게 주려면 **[직접 고르기]** 로 바꿔 칸을 켜고 끕니다. 권한마다 **무엇을 허락하는지 한 줄**이 이름 아래에 적혀 있습니다.
- **`agent-session:launch` 는 빼지 마세요.** `nerv_bootstrap` 이 그 권한을 요구하므로, 빠지면 아래 4·5단계가 처음부터 막힙니다 — [직접 고르기]로 권한을 줄일 때 가장 흔한 실수입니다.
...
**담지 않는 대신, 만들어 주는 명령을 담습니다.** 작업 저장소에서 한 번 돌리면 `.mcp.json` · `.claude/settings.local.json` 의 환경변수 · `.gitignore` 의 `.nerv/` 가 한꺼번에 섭니다. **주소와 프로젝트는 이 서버의 값으로 채워 뒀으니** 남은 것은 토큰뿐이고, 그것은 화면에 찍지 않고 가려서 묻습니다.
