...
- 권한은 **[권장]** 으로 시작합니다 — 에이전트가 스킬 다섯(next·spec·impl·question·review)을 쓰는 데 드는 `spec:read` · `spec:draft` · `task:claim` · `task:update` · `review:submit` · `agent-session:launch` 입니다. 내 역할에 없는 것은 빠진 채 나갑니다(잠긴 칸이 몰래 발급되지는 않습니다). 다르게 주려면 **[직접 고르기]** 로 바꿔 칸을 켜고 끕니다.
