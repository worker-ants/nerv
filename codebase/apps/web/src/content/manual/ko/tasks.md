...
작업의 기준 스펙 버전이 `superseded`가 되면 작업 카드에 **지시 재확인 필요** 배지가 붙고, 작업 상세의 근거 줄에도 **지시 재확인** 표시가 나타납니다. 선택지는 둘입니다.

- **[기준 갱신]** 을 누릅니다. 기준을 최신 승인 버전으로 바꾸고 표시를 지웁니다. 위임 명세는 그대로이므로, 바뀐 스펙에 맞게 명세도 함께 고치세요. 바로 옆의 **[고치기]** 를 누르면 명세 칸으로 이동합니다.
- 지금 기준으로 작업을 끝내고, 차이는 **새 작업**으로 만듭니다.

어느 쪽으로 할지는 사람이 정하며, 화면이 대신 고르지 않습니다. [기준 갱신]은 planner · developer · admin 만 누를 수 있습니다.
