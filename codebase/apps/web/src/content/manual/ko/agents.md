...
- **질문 답변·게이트 면제·받은 요청 읽기도 토큰으로 할 수 없습니다.** 역할이 admin 이어도 그렇습니다 — 에이전트가 자기 질문에 스스로 답하면 사람이 지키는 게이트가 없는 것과 같습니다. 답은 사람이 웹의 받은 요청에서 합니다.
...
- **A3** — **사람 승인이 필요합니다.** 검토 요청(`nerv_spec_submit_review`)과 `critical` 하향이 여기입니다. A3 도구는 어떤 스킬의 무승인 허용 목록에도 들어가지 않습니다. **결정은 요청한 세션의 다음 하트비트로 돌아갑니다** — 승인·거절·코멘트 모두, 그 세션이 이어서 무엇을 할지 알 수 있게.
