// /settings — S8 설정 셸. 항목은 범위로 묶인다: 조직(조직·프로젝트 · 멤버·역할 · 조직 전체 토큰) · 프로젝트(게이트
// 정책 · 연동 Phase 2) · 나(에이전트 토큰) — `features/settings/settings-nav.tsx` 가 그 목록의 정본이다.
// git 연동·리뷰 게이트 정책은 Phase 2 다(scope.md §4.1).
//
// **어느 항목에 있는지, 어느 조직의 설정인지 화면이 말해야 한다**(2026-09-25 — 사람 결정 D1 · SET-06 · REQ-WEB-227).
// 머리는 "설정" 한 낱말이라 어느 조직의 것인지 탭마다 제목을 읽어야 알았다.
...
function SettingsShell(): React.JSX.Element {
  ...
  return (
    <PageBody>
      ...
      {/* 사이드바가 서는 폭에서는 셸 사이드바의 [설정] 아래가 이 목록이다(REQ-WEB-225) — 여기서는 좁은 폭의 것만 */}
