// 테이블 30종 · enum 38종의 **개수와 이름**을 고정한다.
//
// 근거: docs/04-mvp/database.md §2(DDL 정본) · codebase.md §2.3(모듈별 소유 배정).
// 이 숫자는 문서 전체에서 반복 인용되는 검산값이라(30 = 6+9+4+2+2+5+2), 선언이 하나
...
/**
 * 인프라 테이블은 도메인 엔티티가 아니다 — 30종 카운트와 ERD 에 들지 않는다.
...
describe('테이블 선언 (database.md §2)', () => {
  ...
  it('30종이다 — 6(테넌시) + 9(스펙) + 4(작업) + 2(세션) + 2(승인) + 5(리뷰) + 2(이벤트)', () => {
    expect(declaredTables).toHaveLength(30);
  });
