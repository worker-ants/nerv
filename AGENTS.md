...
3. **근거 규율.** 외부 URL 은 접속 확인된 것만 싣고, clemvion 실측은 `clemvion:경로` 로 표기하며, 수치는 정본의 값을 인용하고 새 값을 만들지 않는다.
...
7. **문서 간 참조는 링크로 잇고, 역참조는 frontmatter 가 든다.** 다른 문서나 외부 페이지를 인용하면 **인라인 링크**로 잇는다(`[4.4 API](docs/04-mvp/api.md) §1.6` — "4.4 §1.6" 처럼 번호만 적지 않는다. 도구가 따라갈 수 없는 인용은 깨져도 아무도 모른다). 각 md 의 frontmatter `references` 는 **그 문서를 링크하는 문서의 목록**이고 손으로 적지 않는다 — 문서를 고쳤으면 그 목록의 문서들을 열어 인용이 아직 맞는지 본 뒤, `codebase/` 에서 `node scripts/check-doc-links.mjs --fix` 로 목록과 파생본 머리를 다시 쓴다. CI 가 죽은 링크·역참조·맨 참조를 대조한다(REQ-CB-030).
