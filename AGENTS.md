...
7. **문서 간 참조는 링크로 잇고, 역참조는 frontmatter 가 든다.** 다른 문서나 외부 페이지를 인용하면 **인라인 링크**로 잇는다 — `[4.4 API](docs/04-mvp/api.md) §1.6` 처럼 링크 글자는 문서 번호(와 제목), 절은 링크 뒤에 `§`. `4.4 §1.6`·`4.4 v0.87` 처럼 번호만 적지 않고 참조식 링크(`[글자][라벨]`)도 쓰지 않는다(도구가 따라갈 수 없는 인용은 깨져도 아무도 모른다). 각 md 의 frontmatter `referenced_by` 는 **그 문서를 링크하는 문서의 목록**이고 도구가 쓴다 — 사람은 적지 않는다. 문서의 절·제목·사실을 바꿨으면 `codebase/` 에서 `node scripts/check-doc-links.mjs --where <문서>` 로 인용한 자리를 받아 함께 고치고, `--fix` 로 목록과 파생본 머리를 다시 쓴다. CI 가 죽은 링크·역참조·파생본 머리·맨 참조·절 실재를 대조한다(REQ-CB-030).
...
7. **커밋·push 전에 `codebase/` 에서 `pnpm preflight` 를 돌리고, 나온 것은 고친다.** CI 의 `check` 잡을 **같은 열·같은 순서로** 비춘다 — lint·typecheck·format:check, 게이트 일곱(플러그인 버전 · 배포 산출물 · 백로그 현황 · md ↔ html · `.env` 전표 · 문서 간 참조 · schema drift), L1. 넷만 돌리고 초록을 본 사람은 게이트를 한 번도 돌리지 않은 채 push 한다. L2 는 `pnpm preflight --l2`(실제 Postgres · `codebase/.env`) — 동시성을 건드렸으면 건너뛰지 않는다. **그 `.env` 가 없으면 만들지 말고 사람에게 말한다**(규약 8) — 없는 채로 L2 를 돌릴 방법은 없고, 돌리지 못했으면 돌리지 못했다고 보고한다. L3 는 `preflight` 에 없고 CI 가 매 PR 에서 본다 — 표면·CLI·계약을 건드렸으면 로컬에서도 `pnpm e2e:up && pnpm test:e2e && pnpm e2e:down`. `pnpm hooks:install` 은 옵트인이고 **훅은 게이트가 아니다** — 게이트는 CI 다.
...
8. **`codebase/.env` 는 건드리지 않는다 — 만들지도, 덮지도, 지우지도 않는다.** 그 파일은 git 에 없고(`.gitignore`) **살아 있는 자격증명**을 든다. 되살릴 수 있는 사본이 어디에도 없어서, 한 번 잃으면 끝이다.
   - **2026-09-14 실측이 이 규약의 이유다.** 일회용 L2 설정을 만들려고 `cat > .env` 로 덮고(있는지 확인하지 않았다) 끝나고 지웠다. Postgres·MinIO 자격증명은 돌고 있는 컨테이너의 env 에서 살렸지만 **`NERV_AUTH_SECRET` 은 사본이 어디에도 없어 잃었고**, 새 키를 쓰는 순간 기존 로그인 세션이 전부 무효가 됐다. 편집기 백업도 Time Machine 로컬 스냅샷도 없었다.
   - **없어서 명령이 안 돌면 사람에게 말한다.** `cp .env.example .env` 와 필수 3개 채우기는 **사람이 한다**([4.2](docs/04-mvp/codebase.md) §5.1).
   - **일회용 설정은 저장소 밖에 두고 가리킨다** — `node --env-file=<임시 경로>` · `docker compose --env-file <임시 경로>`. 저장소 안에 쓰지 않는다.
   - **같은 부류가 더 있다.** git 에 없고 공유되는 로컬 상태는 전부 남의 것일 수 있다 — **개발 compose 스택**(`nerv-postgres-1` 등은 워크트리들이 함께 쓴다: 재생성하면 다른 세션의 자격증명이 덮인다. 일회용 DB 가 필요하면 별도 컨테이너를 띄운다) · `codebase/.e2e` 세션 할당 · **다른 세션의 워크트리**(`.claude/worktrees/*`) · `git stash`(세션끼리 공유된다).
   - **되돌릴 수 없는 것은 확인부터 한다.** 덮어쓰기·삭제 전에 그 자리에 무엇이 있는지 본다. `git checkout -- <경로>` 도 여기 든다 — 커밋하지 않은 남의(또는 자기) 작업을 조용히 지운다.
