...
1. **md 가 원본, html 은 파생본.** `docs/**/*.md` 를 고치면 같은 커밋에서 대응하는 `docs/html/*.html` 도 고친다(사이드바 순서·`assets/style.css` 공용). CI 가 파일 짝·버전·절 번호·고정 ID·**고정 ID 행의 본문**을 대조한다(`check-md-html.mjs` · REQ-CB-051). **ID 가 없는 표 행은 검사가 보지 못한다** — 그 자리는 사람이 지킨다.
...
**한국의 개발자와 사용자가 실제로 쓰는 말로 쓴다**(2026-09-26 사람 지시). 사람에게 하는 대답 · 화면 문구 · 도움말 · 명세 · 커밋 메시지 · PR 본문 모두 같다.

- 영어를 옮긴 듯한 표현이나 사물이 사람처럼 움직이는 비유를 쓰지 않는다. 자주 틀린 말: "표시가 선다 · 배지를 세운다"(→ 표시된다 · 표시한다), "기능을 걷는다"(→ 없앤다 · 뺀다), "응답에 싣는다"(→ 포함한다), "배선"(→ 연결 · 구현), "닿는다"(→ 이어진다 · 적용된다), "가른다"(→ 나눈다), "쥔다"(→ 맡는다), "머리"(→ 상단 · 헤더), "화면이 말한다"(→ 보여 준다).
- 쓰지 않는 말의 전체 목록과 문장 습관은 [용어 사전](docs/glossary.md) §3.4 가 정본이다. 말투(합쇼체 · 해라체)는 같은 문서 §3.1 이다.
- 쓰고 나서 소리 내어 읽었을 때 한국어 화자가 그렇게 말하지 않을 문장이면 고친다. 영어도 같다 — 한국어 비유를 직역하지 않는다.
