...
2. **정본(single source) 원칙.** 각 주제는 정본 문서 한 곳에서만 정의하고 나머지 문서는 링크로 인용한다. 상태값·결정 번호(D-01~D-14)·요구사항 번호(FR-01~17 / NFR-01~05)는 [docs/01-problem/pain-points.md](docs/01-problem/pain-points.md)가 단일 기준이고, **그 값들을 한국어로 무엇이라 부를지는 [docs/glossary.md](docs/glossary.md)가 정본**이다 — 표제어·채택어·치환 방향과 **말투 규칙**(화면은 합쇼체, 에이전트·CLI·생성 문서는 해라체)이 거기 있다. 정본이 아닌 문서에서 재정의하지 않는다. **새 용어를 들이거나 채택어를 바꾸면 사전을 같은 커밋에서 고친다** — 사전이 없던 동안 같은 것이 두 이름으로 자랐고(기준선/베이스라인 165곳) 한 이름이 세 가지를 가리켰다(스코프).
...
6. **서버가 바뀌면 플러그인도 검증한다.** 스킬(`codebase/plugin/skills/*/SKILL.md`)은 문서가 아니라 **모델이 읽는 규약**이다 — 서버가 앞서가면 그 차이는 문서의 흠으로 끝나지 않고 **그대로 에이전트 행동의 결함**이 된다. 도구·인자·응답 필드·스코프·훅 계약을 건드렸으면 같은 커밋에서 다음을 확인한다.
   ...
   서버에 기능이 있다는 것과 그 기능이 쓰인다는 것은 다른 일이다. 이 저장소는 그 차이를 스펙 생성 진입점·베이스라인·훅 응답·첨부 확정 응답에서 네 번 겪었다.
