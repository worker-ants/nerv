...
7. **구현을 끝냈으면 정적 검사를 돌리고, 나온 것은 고친다.** 커밋·push 전에 `codebase/` 에서 이것 하나를 돌린다 — **CI 의 `check` 잡을 그대로 비춘다.**
   ...
   **왜 네 명령이 아니라 이것인가.** 규약이 오래 적어 온 네 가지(`typecheck`·`lint`·`format:check`·`test`)는 CI 가 보는 것의 **일부**다 — CI 는 그 위에 **플러그인 버전 게이트 · 배포 산출물 정합 · schema drift** 셋을 더 본다. 넷만 돌리고 초록을 본 사람은 나머지 셋을 **한 번도 돌리지 않은 채** push 한다. `preflight` 는 CI 와 **같은 일곱을 같은 순서로** 돌린다(게이트가 테스트 앞이다 — 테스트가 깨져도 드리프트는 잡히게).
