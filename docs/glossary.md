...
>
> 문서 버전 v0.7 · 2026-09-25 · HTML 파생본: [glossary.html](html/glossary.html)
>
...
**검사가 센다**(2026-09-25). `packages/schema/src/i18n/i18n.spec.ts` 의 "말투" 묶음이 화면 키 전부(위 표의 에이전트·생성 문서 머리 다섯을 뺀 것)에서 해요체 문장 끝 · 해라체 문장 끝("니다" 가 아닌 "다") · 결정 번호와 "MVP" 를 찾는다. 해라체가 맞는 견본(요구사항 문형 `spec.requirements.format`)만 이름으로 비켜 둔다. **반대 방향은 아직 세지 않는다** — 에이전트·CLI 키에 합쇼체가 아홉 곳 있다(2026-09-25 실측 — 훅 메시지 `agent.*` 셋 · `cli.err.*` 다섯 · `mcp.error.tool_failed`).
