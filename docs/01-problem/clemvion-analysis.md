...
>
> 문서 버전 v0.3 · 2026-09-24 · HTML 파생본: [clemvion-analysis.html](../html/clemvion-analysis.html)
>
> v0.3 변경(2026-09-24 — 파생본 표에 행 하나가 없었다, 전수 대조): **새 사실 없음 · §5.7 표 한 행.** "문서 정리 커밋 비중"(스펙 터치 커밋 857개 중 `docs(spec)` 211개 · 4.5개월) 행이 파생본의 표에만 없었다. 바로 위 카드가 같은 수(211 / 857)를 들고 있어 **사실이 사라진 것은 아니지만**, 형제 카드 둘은 표에도 행으로 서 있고 이 행만 빠져 출처(`clemvion:spec/` git 이력)를 잃었다.
>
