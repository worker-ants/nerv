...
>
> 문서 버전 v0.4 · 2026-09-05 · HTML 파생본: [pain-points.html](../html/pain-points.html)
>
...
>
> v0.3 변경(2026-09-05 — 인용은 24곳인데 정의가 없었다, 정합성 감사 → 사람 결정): **D-09·D-10 정의문 신설.** 이 문서는 결정 번호의 단일 기준인데 **`D-09`·`D-10` 은 정의문이 어디에도 없었다** — D-09 를 13편이, D-10 을 11편이 인용하고 있었다. 번호가 뜻을 대신하는 상태에서는 새로 인용하는 사람이 문맥에서 뜻을 추측하고, 추측이 갈려도 아무도 못 본다. 문구는 **인용 전수**(D-09 33곳 · D-10 19곳)를 읽어 복원했고 사람이 확인했다. **D-09 는 네 갈래를 포괄한다** — markdown 저장 포맷 · 안정 ID 참조 · 중복 서술 금지 · CRDT 비범위. 자리는 논증된 곳이다: D-09 는 §2.7 P7(참여 표면), D-10 은 §5 Non-goals(P1~P8 어느 행에도 매핑돼 있지 않다).
>
...
> **D-09 — 스펙은 markdown 문서이고, 참조는 안정 ID 다.** 저장 포맷은 markdown 이고 메타(타입·문서 상태·소유 역할)는 DB 컬럼이다 — 블록 JSON 이 아닌 이유는 **토큰 밀도**이고, 그래서 에이전트가 본문을 그대로 읽고 사람이 읽는 git 미러도 같은 파일이 된다. 문서끼리는 **경로·헤딩 앵커가 아니라 안정 ID 로 참조한다**: clemvion 의 인덱스 부패는 `plan/in-progress/` → `complete/` 이동이 본문 링크를 끊은 것이 원인이었다. **같은 사실을 두 문서가 각자 쓰지 않는다** — 중복 서술이 drift 의 근원이므로 단일 정의 + 관계 참조가 규약이다. **실시간 공동 편집(CRDT)은 Phase 3 이고**, MVP 의 협업 수단은 에디터 + 프리뷰 + **헤딩 앵커 코멘트**다: 문단이 아니라 헤딩에 붙이는 이유는 본문이 새 버전으로 바뀌어도 앵커가 살아남기 때문이다. 스펙 편집의 병목은 동시 타이핑이 아니라 승인·조정이다.
