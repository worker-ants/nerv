...
>
> v0.4 변경(2026-09-05 — 가리키던 사전이 없었다, 용어 검토 → 사람 결정): §4.1 이 "상태 어휘는 **용어 사전**을 그대로 따른다" 고 적었는데 **그 문자열이 저장소 전체에서 이 한 줄뿐**이었다 — 정본이라 선언한 자리에 실물이 없었으니 `D-09`·`D-10` 과 같은 모양이다. [용어 사전](../glossary.md)을 신설해 참조를 잇고, **역할을 갈랐다**: 상태값의 *목록*은 이 문서가, 그 값들을 한국어로 무엇이라 부를지는 사전이 정한다.
>
> v0.3 변경(2026-09-05 — 인용은 24곳인데 정의가 없었다, 정합성 감사 → 사람 결정): **D-09·D-10 정의문 신설.** 이 문서는 결정 번호의 단일 기준인데 **`D-09`·`D-10` 은 정의문이 어디에도 없었다** — D-09 를 13편이, D-10 을 11편이 인용하고 있었다. 번호가 뜻을 대신하는 상태에서는 새로 인용하는 사람이 문맥에서 뜻을 추측하고, 추측이 갈려도 아무도 못 본다. 문구는 **인용 전수**(D-09 33곳 · D-10 19곳)를 읽어 복원했고 사람이 확인했다. **D-09 는 네 갈래를 포괄한다** — markdown 저장 포맷 · 고정 ID 참조 · 중복 서술 금지 · CRDT 비범위. 자리는 논증된 곳이다: D-09 는 §2.7 P7(참여 표면), D-10 은 §5 Non-goals(P1~P8 어느 행에도 매핑돼 있지 않다).
>
> v0.2 변경(2026-08-27 — 화면 이름 개명, 사람 결정): **승인함 → 받은 요청**(FR-11). 이 화면에 모이는 것은 승인만이 아니라 **질문·에스컬레이션**까지 다섯 유형이고, FR-11 자신이 이미 "다섯 유형의 요청이 한 수신함에"라고 적고 있었다 — 이름만 승인이고 내용은 아니었다. 요구사항 번호·판정은 그대로이고 바뀐 것은 이름뿐이다. 과거 문서의 변경 기록 줄은 그때의 기록이라 옛 이름을 그대로 둔다.

---

## 1. 문제 지형 한눈에 보기

NERV(가칭)의 출발점은 `/Volumes/project/private/clemvion`이다. 1인 개발자가 Claude Code 하네스로 SDD(스펙 주도 개발)와 TDD를 자동화한 제품 모노레포이고, 완성도 자체는 매우 높다. 역할별 스킬 5+1종, 훅 9종, 워크플로우 3종, 31개 서브에이전트가 "리뷰 없이 push 금지", "main 브랜치에서 작업 금지" 같은 규칙을 기계적으로 강제한다. 이 하네스의 철학은 한 문장으로 요약된다 — **강제 없는 규약은 반드시 깨진다**.
...
- **정하지 않는다**: 해결 방식의 설계. 데이터 모델은 [데이터 모델](../03-proposal/data-model.md), 시스템 구성은 [시스템 아키텍처](../03-proposal/architecture.md), 워크플로우는 [스펙 워크플로우와 거버넌스](../03-proposal/spec-workflow.md), 단계별 계획은 [로드맵](../03-proposal/roadmap.md)이 다룬다.
...
- 같은 문제가 생태계 최대 SDD 도구에서도 미해결이다. Spec Kit 팀 도입 디스커션은 "두 기능이 같은 파일을 건드릴 때 두 스펙 계약이 동시에 유효한지 검증할 방법이 없다"고 보고하고, OpenSpec 공식 팀 워크플로우 문서는 충돌 방지책이 **"One change, one owner"라는 사람 규약과 일반 git 병합뿐**임을 자인한다.
...
- 버전 개념이 없으니 이력이 본문 밖으로 밀려난다. `clemvion:spec/0-overview.md`의 원칙은 "본문은 latest-only 사실을 기술"이고, 계획 워크플로우도 "옛 내용을 정리해 latest만 남김(history가 아님)"이다. 이력 전담은 git이 맡는데, git은 파일 이력은 알아도 **"이 버전이 승인되었는가"**는 모른다.
...
- **검토 결과를 제3자가 볼 수 없다.** 리뷰 산출물이 gitignored였던 기간에는 PR에 리뷰 흔적이 없어 "plan 노트가 PR에서 워크플로우 이행을 확인할 유일한 흔적"이었다. 개발자가 아닌 사람이 "이 변경이 제대로 검토됐는가"를 확인할 표면이 없다는 뜻이다.
- **시장에서 요구됐고 거부됐다.** OpenSpec 이슈 #435는 "스펙 작성은 본질적으로 협업인데 현재 도구들은 single-player이거나 일반 git 충돌에 의존한다"며 멀티유저 워크스페이스(RBAC·실시간 편집), 문서식 인라인 코멘트, 멀티 에이전트 병렬 리뷰를 요청했으나 **"closed as not planned"로 종료**됐다. 승인 워크플로우를 내장한 유일한 도구는 spec-workflow-mcp(GPL-3.0)인데, 사용자 계정·권한·원격 접근이 없는 로컬 도구라 "팀 협업"이 아니라 "1인+에이전트 협업"이다.
...
> **D-09 — 스펙은 markdown 문서이고, 참조는 고정 ID 다.** 저장 포맷은 markdown 이고 메타(타입·문서 상태·소유 역할)는 DB 컬럼이다 — 블록 JSON 이 아닌 이유는 **토큰 밀도**이고, 그래서 에이전트가 본문을 그대로 읽고 사람이 읽는 git 미러도 같은 파일이 된다. 문서끼리는 **경로·헤딩 앵커가 아니라 고정 ID 로 참조한다**: clemvion 의 인덱스 부패는 `plan/in-progress/` → `complete/` 이동이 본문 링크를 끊은 것이 원인이었다. **같은 사실을 두 문서가 각자 쓰지 않는다** — 중복 서술이 drift 의 근원이므로 단일 정의 + 관계 참조가 규약이다. **실시간 공동 편집(CRDT)은 Phase 3 이고**, MVP 의 협업 수단은 에디터 + 프리뷰 + **헤딩 앵커 코멘트**다: 문단이 아니라 헤딩에 붙이는 이유는 본문이 새 버전으로 바뀌어도 앵커가 살아남기 때문이다. 스펙 편집의 병목은 동시 타이핑이 아니라 승인·조정이다.
...
- **게이트 범위가 이 머신의 checkout 목록이다.** push 가드는 `git worktree list` 결과를 권한으로 삼는다. 다른 호스트에서의 push는 애초에 이 훅 밖이고, 훅이 설치되지 않은 클론에서 push하면 규칙 전체가 무효다.
...
- **하네스 훅 코드 약 7,600줄**(훅 + 판정 모듈 + 워크플로우 합계). 최대 단일 훅은 push 게이트 **1,005줄**이고, 훅 회귀 테스트만 57개 파일 1.2MB다.
...
| OpenSpec 팀 워크플로우 문서 | 공식 문서 | 충돌 방지는 "One change, one owner" 규약과 일반 git 병합뿐임을 자인 |
| OpenSpec 이슈 #435 | 공식 결정 | 멀티유저 워크스페이스·RBAC·인라인 코멘트 요청이 "not planned"로 종료 |
| spec-workflow-mcp | 유일한 승인 워크플로우 내장 | MCP 연동 + 웹 승인 대시보드 개념 증명. 단 계정·권한·원격 없음(GPL-3.0이라 개념만 참조) |
...
- 아래 표에 등장하는 **상태 어휘의 목록은 이 문서가 정본**이다. 그 값들을 한국어로 **무엇이라 부를지**는 [용어 사전](../glossary.md)이 정하고, 어느 문서도 재정의하지 않는다.
...
| --- | --- | --- |
| **FR-01** | 스펙 단일 진실 저장소(트리·타입·검색·고정 ID) | 프로젝트의 모든 스펙을 트리로 조회할 수 있고, 타입(`vision`/`area`/`feature`/`design`/`convention`/`adr`)과 고정 ID로 단건 조회·전문 검색이 되며, 문서를 옮기거나 이름을 바꿔도 ID 참조가 깨지지 않는다 |
| **FR-02** | 스펙 버전·문서 상태 관리(불변 스냅샷, draft→in_review→approved, 버전 diff, **프로젝트 기준선**) | `approved` 된 SpecVersion 본문은 이후 편집으로 변하지 않고, 임의의 두 버전 간 diff와 상태 전이 이력(행위자·시각·사유)을 UI에서 확인할 수 있다. 프로젝트의 approved 버전 **집합**을 이름 붙은 기준선으로 동결하고, 임의 시점에 그 세트 그대로 재현·조회할 수 있다(범위 확장 2026-08-21 — 요구공학의 baseline은 문서 1건이 아니라 "합의·검토·승인된 요구사항 집합의 시점 스냅샷"이다. 스펙이 구현보다 앞서갈 때 구현의 기준 세트를 고정하기 위해 필요) |
| **FR-03** | 요구사항(Requirement) 단위 추적(고정 ID, EARS 템플릿, 구현 상태 2축) | 한 SpecVersion에서 추출된 각 Requirement가 고정 ID와 수용 기준을 갖고, 문서 축과 독립적으로 `unimplemented→in_progress→implemented→verified` 값을 가진다(같은 문서 안의 두 요구사항이 서로 다른 상태일 수 있다) |
...
| **FR-11** | 받은 요청(Inbox — 2026-08-27 개명, 옛 이름 "승인함"): 스펙 승인/CR/플랜 승인/질문/에스컬레이션 — 승인·거절·코멘트 | 다섯 유형의 요청이 한 수신함에 모이고, 카드에서 승인/거절/코멘트를 원클릭으로 처리하면 요청한 세션이 `awaiting_input`에서 즉시 해제된다 |
...
| **NFR-02** | 실시간성(세션 보드 갱신 ≤5s) | 세션 상태가 바뀐 시점부터 보드에 반영되기까지 5초 이내이며, 클라이언트 폴링 없이 서버 푸시(WebSocket·SSE)로 전달된다 |
| **NFR-03** | 보안(토큰 권한, 권한 비확대, 프롬프트 인젝션 완화) | 에이전트 토큰은 프로젝트 소속이고 위임한 사용자의 권한을 넘는 요청은 거부되며, 스펙·리뷰 본문은 비신뢰 데이터로 취급되어 도구 실행 지시로 해석되지 않는다 |
...
| **완전한 이벤트 소싱** | 과설계이며 운영 복잡도만 늘린다 | append-only Event 테이블 + WebSocket·SSE 푸시로 활동 피드·알림·감사 로그를 한 원천에서(D-10) |

> **D-10 — 이벤트는 append-only, 전면 이벤트 소싱은 하지 않는다.** `event` 테이블 하나가 활동 피드·알림·감사 로그의 **단일 원천**이고, 도메인 상태 전이는 **같은 트랜잭션에서** 그 행을 적재한다(전이와 기록의 원자성) — 알림에 보인 것은 반드시 감사에도 남는다. **상태를 이벤트 재생으로 복원하지는 않는다**: 도메인 테이블이 현재를 들고 있고 Event 는 그 옆에 쌓인다. 실시간 전달은 두 채널이다 — 화면은 WebSocket, 브라우저 밖 소비자(CLI·외부 도구)는 SSE. 근거는 Microsoft 의 이벤트 소싱 패턴 문서가 **"프로토타입·MVP 에는 부적합하며 이득이 큰 부분에만 선택 적용하라"** 고 명시한 것과, 전면 소싱이 **개인정보 삭제권과 충돌**한다는 같은 문서의 경고다. D-10 이 정확히 그 선택 적용이다.
...
- [OpenSpec 팀 워크플로우 문서 (team-workflow.md) — Fission-AI/OpenSpec](https://github.com/Fission-AI/OpenSpec/blob/main/docs/team-workflow.md) — (2026-08-13 확인) 충돌 방지책이 "One change, one owner" 규약과 일반 git 병합뿐임을 공식 문서가 자인(P1).
...
- [Our plan for running 100 Parallel Coding Agents — Superset](https://superset.sh/blog/roadmap-to-100-agents) — (2026) 자동 품질 게이트·구조화 디스패치·완료 리뷰 워크플로우가 대규모 병렬의 전제(§3.5).
