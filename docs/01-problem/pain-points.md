...
| **NFR-02** | 실시간성(SSE, 세션 보드 갱신 ≤5s) | 세션 상태가 바뀐 시점부터 보드에 반영되기까지 5초 이내이며, 클라이언트 폴링 없이 서버 푸시(SSE)로 전달된다 |
...
| **완전한 이벤트 소싱** | 과설계이며 운영 복잡도만 늘린다 | append-only Event 테이블 + SSE로 활동 피드·알림·감사 로그를 한 원천에서(D-10) |
