...
- **비범위**: 스펙 편집기, 승인함, 리뷰 수집, 알림, CR, 커버리지, 멀티 조직, OAuth, Codex 지원.
...
| 승인함 + 인앱 알림 | FR-11 ◐ / FR-12 ◐ | 스펙 승인·플랜 승인·질문 3유형(CR·에스컬레이션은 Phase 2), 원클릭 승인/거절/코멘트 |
...
화면은 S1 홈 · S2 프로젝트 개요 · S3 스펙 상세 · S4 작업 보드 · S5 세션 모니터 · S7 승인함 · S8 설정(멤버·역할, 에이전트 토큰 탭 우선)을 구축한다(S6 리뷰 센터와 S8의 연동·게이트 정책 탭은 Phase 2). 이 순서는 [3.6 화면 설계](ui-wireframes.md)의 안내와 일치한다.
...
| CR 델타 리뷰 | **FR-04 · FR-11 ●** | ADDED/MODIFIED/REMOVED 뷰, 승인 시 파생 Task 재계산(영향 분석), SPEC-DRIFT 역류 경로. 승인함에 CR·에스컬레이션 카드가 붙어 5유형이 완성된다 |
...
1. `nerv_review_submit` · `nerv_finding_resolve` 도구(카탈로그 15종 완성)와 fingerprint 알고리즘 명세 + `/nerv:review` 스킬 추가(플러그인 스킬 4종 → 5종).
