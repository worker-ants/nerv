...
>
> 문서 버전 v0.9 · 2026-09-24 · HTML 파생본: [roadmap.html](../html/roadmap.html)
>
> v0.9 변경(2026-09-24 — 발급 CLI 를 걷는다, **사람 결정**): §2.3 산출물 1 의 **PAT 발급 CLI 를 제외한다.** 발급은 사람 전용(EP-TOK-02)이고 웹 설정 화면이 유일한 경로다 — 에이전트가 스스로 토큰을 발급하지 못하게 하는 것이 의도이고, CLI 는 그 문을 하나 더 여는 것일 뿐 필요한 이유가 없다([4.8](../04-mvp/backlog.md) E03-S02).
>
> v0.8 변경(2026-09-07 — 판정할 방법이 없던 기준 하나, 개선 계획 넷째 스프린트): **새 요구사항 없음.** 성공 기준 0-8("도구만으로 완주")의 **판정 수단**이 저장소에 없었다 — L3 시나리오 A~C 는 클레임까지만 태우고 그 이후는 L2 조각들만 봤다. [4.8 백로그](../04-mvp/backlog.md) §5.6 시나리오 F 가 그 자리를 채운다. §1.4 의 소급 판정 금지는 그대로다: 지난 일을 지금 "통과" 로 적지 않고, **다음 게이트부터** 이 표를 채운다.
>
...
```text
  ...
  ^M1~^M4     clemvion 이관 마일스톤 — 기계 산출물 → spec·plan → 사람 워크플로우 → 실시간(§7.2)
...
| 요구사항 | P0 | P1 | P2 | P3+ | 비고 |
| --- | :-: | :-: | :-: | :-: | --- |
| FR-01 스펙 단일 진실 저장소 | ◐ | ● | ● | ● | P0는 읽기·트리·고정 ID만, P1 기획자 터미널 경로 포함 |
...
**5. 판정을 남긴다** (2026-09-06 신설 — 규칙 넷을 두고 **기록할 자리를 두지 않았다**). 게이트를 통과했다는 것도 사실이고, 판정 없이 다음 Phase 로 넘어간 것도 사실이다. 아래 표가 그 기록이고, 비어 있으면 **판정하지 않았다**는 뜻이다 — "통과했겠지" 와 구별된다.

| 게이트 | 판정일 | 판정 | 미달 항목과 처분 |
| --- | --- | --- | --- |
| **G0** Phase 0 종료 | — | **기록 없음** | 2026-09-06 실측: 성공 기준 0-1~0-8 의 판정 기록이 저장소 어디에도 없다. Phase 1·2 작업은 그 사이에 진행됐다. **소급 판정은 하지 않는다**(아래) — 다만 0-8 은 2026-09-07 부터 판정 수단이 생겼다(L3 시나리오 F 가 매 PR 에서 돈다) |
| **G1** Phase 1 종료 | — | **기록 없음** | 같음 |
| **G2** Phase 2 종료 | — | — | 진행 전 |

> **소급 판정은 하지 않는다.** 지난 일을 지금 "통과" 로 적으면 규칙 1(수치가 종료 조건)이 무의미해진다. 대신 **다음 게이트부터 이 표를 채운다** — 채우는 시점은 그 Phase 의 마지막 커밋과 같은 날이다. G0·G1 을 소급해 판정할지는 사람 결정으로 남긴다.
...
| 스펙 읽기 | FR-01 ◐ | 트리·타입·고정 ID·검색(P0는 렉시컬 — FTS+pg_trgm). 편집 없음(임포트로만 적재) |
...
1. `nerv-mcp` 서버(도구 8종) + ~~PAT 발급 CLI~~(2026-09-24 제외 — 사람 결정: 발급은 사람 전용(EP-TOK-02 `HUMAN_ONLY`)이고 웹 설정 화면이 유일한 경로다. 에이전트가 스스로 토큰을 발급하지 못하게 하는 것이 의도이고, CLI 는 그 문을 하나 더 여는 것일 뿐 필요한 이유가 없다) — 도구 스펙은 [3.4 에이전트 연동 설계](agent-integration.md) §2의 카탈로그를 따른다.
...
| 0-8 | tools-only 완주 | Claude Code 1세션·Codex 1세션 각각 | `bootstrap→next→claim→heartbeat→질문·답·리뷰→done→session-end` **완주**(resources·prompts·elicitation 미사용) — **판정 수단은 L3 시나리오 F 다**([4.8 백로그](../04-mvp/backlog.md) §5.6 · 2026-09-07 신설. 그전에는 클레임까지만 태우고 있어 이 기준을 판정할 방법이 없었다) | FR-15 · D-05 |
...
| Requirement 추출 | FR-03 ◐ | EARS 템플릿, 고정 ID 발급, 구현 축 초기값 설정(자동 계산은 Phase 2) |
...
| 1-11 | 기획자 웹·터미널 왕복 | 기획자가 웹과 터미널(Claude Code/Codex)을 오가며 같은 초안을 완성 — 리스 인계·`base_hash` 충돌을 Event 로그로 질의 | 같은 초안 완성, 리스 인계·`base_hash` 충돌 **0** 실증 | FR-01 · FR-06 · FR-11 |
...
| R9 | **권한·인젝션 사고** | 권한 상승 시도 거부 로그, 스펙 본문 유래 도구 호출 시도 | 토큰 프로젝트 소속·권한 비확대, 스펙·리뷰 본문을 비신뢰 데이터로 취급, 위험 도구는 사람 승인 강제 | NFR-03 |
...
| **M3 — 사람 워크플로우** | 코멘트·승인·감사 이벤트 | Phase 2 말~Phase 3 초 | 스펙이 플랫폼에 있어야 코멘트 앵커와 승인 레코드가 의미를 갖는다. 이 시점에 비개발 직군이 정식 온보딩된다 |
| **M4 — 실시간** | 실시간 공동 편집 · 양방향 git sync | Phase 3+(착수 조건 §5.1) | 수요가 계측된 뒤에만 |

이 순서는 스펙 저장 아키텍처 리서치의 결론(기계 산출물 → 스펙 → 사람 워크플로우 → 실시간)과 같다. **M1을 M2보다 먼저 두는 실질적 이유**는 리스크 비대칭이다 — M1이 실패해도 잃는 것은 리뷰 조회 편의(원본은 git 이력에 남아 있다)지만, M2가 먼저 실패하면 제품의 단일 진실이 접근 불가가 된다. NFR-05(로컬 폴백)가 실사용으로 검증되기 전에는 M2를 실행하지 않는다.

```mermaid
flowchart LR
  G["현행<br/>모든 대상의 SoT = git"] --> A["M1 기계 산출물<br/>review 신규분 → DB"]
  A --> B["M2 spec 384 md · plan 450 md<br/>→ DB + git read-only 미러"]
  B --> C["M3 사람 워크플로우<br/>코멘트 · 승인 · 감사 이벤트"]
...
| frontmatter `id`(kebab-case) | 고정 ID 매핑 테이블 | 서버 발급 해시 ID를 정본으로 하고, 옛 `id`는 별칭으로 보존해 기존 인용을 살린다 |
...
| 상대경로 + heading 앵커 상호참조 | 고정 ID 참조 | 변환 실패 링크는 리포트로 |
