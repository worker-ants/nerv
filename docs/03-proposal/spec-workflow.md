...
>
> 문서 버전 v0.15 · 2026-09-26 · HTML 파생본: [spec-workflow.html](../html/spec-workflow.html)
>
...
**지금 세는 신호는 하나다**(2026-09-26 확인 — 결정 철회 검토). 판정(`gate-tier.ts`)은 신호 셋(재시도 임계 · 롤백 이력 · 첫 승인 버전)을 받지만, 서버가 계산해 넘기는 것은 **첫 승인 버전**(①) 하나다. 재시도 임계와 롤백 이력은 데이터에서 무엇을 셀지 — 어느 Task 의 무슨 실패를 몇 번 · "승인 후 롤백" 이 어떤 전이인가 — 가 정해지지 않아 계산되지 않는다. 그래서 설정 화면의 안내와 매뉴얼은 첫 승인 버전만 말한다([4.5](../04-mvp/screens.md) §2.8). 둘을 정의해 켤지, 신호에서 걷을지는 사람 결정 대기다.
