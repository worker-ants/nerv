...
**남은 두 가지는 사람의 결정이 필요하다.** ① 위 표의 T2 예시가 "신규 feature 스펙"인데, 같은 절의 축 표로 계산하면 참조도 파생 Task 도 없는 새 문서는 **3점(T1)** 이다 — 예시와 축이 어긋난다. 새 문서 자체에 가산점을 줄지, 예시를 T1 로 옮길지는 결정 사항이라 구현에서 임의로 정하지 않았다. ② **T1 의 24시간 이의제기 창은 구현이 없다** — `appealWindowHours` 를 산출만 하고 아무도 쓰지 않는다. 그래서 지금 T1 은 사실상 T0 와 같이 동작한다(통지도, 되돌리기도 없다).
