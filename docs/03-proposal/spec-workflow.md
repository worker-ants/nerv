...
>
> 문서 버전 v0.19 · 2026-09-26 · HTML 파생본: [spec-workflow.html](../html/spec-workflow.html)
>
> v0.19 변경(2026-09-26 — 낡은 증적을 세우는 쪽이 생겼다): **§1.3 한 구절.** 임포터가 `code:` glob 의 경로 실존 검사로 `evidence.stale` 을 세운다([4.7](../04-mvp/importer.md) REQ-IMP-032) — 그 증적은 스펙 버전에 붙어 파생에 들지 않는다.
>
> v0.18 변경(2026-09-26 — 검증 서명은 그 문장에 대한 것이다, **사람 결정** · 구현 축 상태도 검토): **§1.3 그림 한 간선 · 규칙표 한 칸 · 두 단락.** 결정 4·5 — 문장이 바뀌면 검증이 풀리고 "다시 검증 필요" 가 선다 · 사람이 해제한다([4.4](../04-mvp/api.md) REQ-API-190).
>
...
>
> v0.15 변경(2026-09-26 — 걷은 되돌리기가 그림에 남아 있었다 · 동적 강화가 세는 신호, 결정 철회 검토): **새 결정 없음 · §6.4 그림 한 카드 · §2.4 한 단락.** ① §6.4 그림의 통지 카드가 아직 "[23시간 남음] … ( 되돌리기 )" 를 그렸다 — 2026-09-02 에 걷은 T1 이의제기 창이다(v0.13 은 표만 고쳤다). 파생본의 [되돌리기] 단추도 함께 걷는다. ② §2.4 동적 강화의 신호 셋 중 서버가 세는 것은 **첫 승인 버전** 하나다 — 재시도 임계와 롤백 이력은 데이터에서 무엇을 셀지 정해지지 않아 계산되지 않는다. 사실을 적고, 정의해 켤지 걷을지는 사람 결정으로 남긴다.
>
...
>
> v0.9 변경(2026-09-06 — 보존 상수가 정본의 두 배였다, 정합성 대조 → 사람 지시): §6.5 보존 표의 "세션 Activity 원문 TTL **180일**" 을 **90일**로 고친다 — 정책 기본값 `activity_days` 는 90 이고([4.4 API](../04-mvp/api.md) §1.6a · `zod/policy.ts`), 4부가 "다른 값을 쓰면 결함" 이라 못 박은 상수다. 요약이 영구로 남는 자리(`activity_summary`)도 함께 적는다.
>
...
>
> v0.7 변경(2026-09-05 — 걷어낸 인자를 현재처럼 적고 있었다, 정합성 감사): §2 최후 방어선의 `base_version` 409 를 **`base_hash` 409** 로 고친다([4.4](../04-mvp/api.md) §1.4g).
> v0.6 변경(2026-09-04 — §3.6 의 소비 규칙이 실물이 된다): 기준선의 "소비" 행은 2026-08-21 부터 `nerv_spec_get(baseline=…)`·`?baseline=` 로 그 세트를 읽는다고 적고 있었는데 **그 인자가 없었다** — 그래서 실사용 기준선이 0개였다(실측 2026-09-04). 읽기·Task 파생·웹 진입점을 이어 그 문장을 참으로 만들었다. 세트에 없는 문서의 처리(최신으로 떨어지되 `baseline_pinned:false`)를 함께 명시했다([4.4](../04-mvp/api.md) v0.66 REQ-API-087).
> v0.5 변경(2026-09-02 — 정본 정합): 리스 인계 표기를 정본에 맞춘다(2026-09-02 · 3.5 §1.2 · [4.4](../04-mvp/api.md) §1.4h): 2026-08-30 에 보유자를 `(user, session)` 으로 좁히고 인계를 `takeover` 로 명시화했는데, 그 개정이 이 문서까지 오지 않아 여전히 "같은 사용자면 자동 인계" 라고 적고 있었다. **L3 시나리오 D 가 그 문장대로 쓰여 있었고 그래서 실패했다** — 에이전트 규약(3.4)은 아예 "이 에러는 오지 않는다" 고 적어, 그 말을 믿은 에이전트는 웹이 열어 둔 초안 앞에서 멈춘다.
...
| 인계 | **명시적** — `takeover: true` 를 실은 저장이 리스를 넘겨받고, 그 사실이 `spec.draft_updated` 페이로드에 남는다. 이전 표면에는 알림이 간다 (2026-08-30 개정 · 사람 결정 — [4.4 API 명세](../04-mvp/api.md) §1.4h) |
| 보유자 단위 | **`(user, session)`** 이다(2026-08-30 개정). 예전에는 사용자 단위라 "같은 사용자면 자동 인계" 였는데, 실제 배치는 PAT 하나 = 사용자 하나 = 세션 여럿이라 병렬 에이전트들이 서로를 전혀 막지 못했다 — **리스는 있는데 아무것도 잠그지 않았다.** 세션이 없는 웹 탭끼리는 같은 사람이면 한 자리로 본다 |
| 타인 차단 | 리스를 쥔 `(user, session)` 이 아닌 upsert 는 `NERV_DRAFT_LEASED`(하드 차단) — 응답이 보유자와 함께 `takeover` 경로를 알린다. 웹은 읽기 전용 + [이어받기] 버튼이 그 인자를 싣는다 |
| 만료 | TTL **30분** — Task 클레임 리스·stale 임계와 같은 상수를 쓴다(§4.5, 새 상수를 만들지 않는다) |
| 해제 | `nerv_spec_submit_review` 성공 · 웹 에디터 닫기 · `SessionEnd` 훅 · TTL 만료 |
| 최후 방어선 | **`base_hash` 409는 그대로 유지한다.** 리스는 1차 사전 조정(작업 낭비 방지)이고, 409는 데이터 유실 방지다 — 역할이 다르므로 둘 다 필요하다 |
...
| `verified` | `implemented` + 검증자가 서명한 Evidence(`kind=test` · `verified_by`) 1건 이상 — **그 서명이 지금 문장에 대한 것**(문장이 바뀐 뒤의 서명) — 그 요구사항에 open `critical` finding 0. 한번 서면 발견이 열려도 내리지 않는다. **문장이 바뀔 때만 풀린다** |
...
**검증 서명은 그 문장에 대한 것이다**(2026-09-26 — 사람 결정 · 구현 축 상태도 검토 결정 4·5). 새 버전 승인이 요구사항 문장을 바꿔도 `verified` 가 그대로 남아, 검증 배지가 아무도 확인하지 않은 문장을 보증했다. 이제 승인이 문장을 바꾸면 그 시각을 남기고(`requirement.statement_changed_at` — 공백·서식만 바뀐 것은 치지 않는다), 그보다 앞선 서명은 지금 문장을 보증하지 않는 것으로 본다 — 검증이 풀려 `implemented` 로 돌아가고 **다시 검증 필요** 가 선다. 이것은 상태가 아니라 **표시**다: 요구사항 관리 도구들이 위쪽이 바뀌면 아래쪽 링크에 의심 표시를 세우고 사람이 해제하게 하는 것과 같은 모양이다. **해제는 사람만 한다** — qa·admin 이 새 test 증적에 서명하거나, 앞선 테스트가 바뀐 문장에도 맞으면 같은 테스트에 다시 서명한다("영향 없음 확인" — 증적과 이벤트로 남는다). 에이전트가 붙인 test 증적은 서명이 아니다(REQ-API-143). 재작업이 필요하면 그 요구사항에 작업을 붙인다: 그 착수가 위 그림의 `implemented → in_progress` 다(done 은 되살리지 않는다 · [4.4](../04-mvp/api.md) REQ-API-190). clemvion이 `CCH-SE-02`를 놓친 사각 — "문서는 implemented인데 그 안의 한 요구사항은 미구현" — 은 상태의 단위를 요구사항으로 내리는 것만으로 사라진다.
...
> **해소 조건은 받지 않는다 — 파생한다**(2026-09-06 결정 · REQ-API-118). 이 문장은 오래 "사유 코드와 **해소 조건**을 필수로 받는다" 였는데, 해소 조건을 담을 열이 만들어진 적이 없다. 만들지 않기로 했다: **사유마다 해소 원천이 이미 저장에 있기 때문**이다 — `awaiting_answer` 는 `question.task_id`, `dependency_broken` 은 `task_dependency`, `spec_conflict` 는 `rebrief_required_at`. 열을 하나 더 두면 같은 사실에 포인터가 둘이 되고 둘은 언젠가 갈라진다(기준선/베이스라인 165곳이 그 자국이다). 게다가 그 열은 넷 중 하나에만 맞고 나머지 셋에는 NULL 이 들어간다 — **넷 중 셋이 비는 열은 계약이 아니라 흔적**이다. 서버는 `EP-TASK-04` 응답에 `blocked_resolution` 을 파생해 실어 준다(`satisfied` · `pending[]`). `external` 은 저장소 밖이라 파생할 원천이 없고, 그때 `satisfied` 는 **`false` 가 아니라 `null`** 이다 — "아니다" 와 "서버가 판정할 수 없다" 는 다른 말이다.
>
> 파생이라 얻는 것이 하나 더 있다. `blocked_reason` 은 **아무도 자동으로 지우지 않아서**, 선행 의존이 `done` 이 되어도 사람이 손으로 전이를 다시 눌러야 풀린다. 파생값은 그 순간 `satisfied: true` 가 되므로 **화면이 "이제 풀 수 있습니다" 를 말할 수 있다** — 자동 해소로 가는 길이 여기서 열린다.
...
다섯 번째 규칙은 **승인의 유통기한**이다. 승인은 특정 content hash에 대한 결정이므로, 대상 본문이 바뀌면 무효화되고 재요청된다. 승인 감사 레코드에는 승인 ID·시각·요청자·승인자·대상 리소스·판정 근거(`gate_tier`·`gate_score`·`auto_passed` — **정책 버전은 미도입**, 2026-09-07 실측)·결정·실행 결과를 남긴다 — 사후에 "그때 왜 자동 승인됐나"를 재구성하려면 정책 버전이 반드시 필요하다.
...
| 2~3 | **T1 소프트 게이트** | 즉시 approved. 등급은 감사 기록에 남고 되돌리기 창은 두지 않는다(2026-09-02 결정) | 기존 문서의 문구 명확화, 예시 추가 |
...
**게이트가 본문을 읽는다**(2026-09-02 — 정합 점검). 구현은 요구사항 **추가·삭제를 상수 0** 으로 넘기고 '수정' 만 `requirement` 행의 존재로 근사하고 있었다. 그런데 그 표를 채우는 것은 임포터뿐이라, 에이전트가 쓴 스펙에는 행이 하나도 없다 — 신규 feature 스펙이 **부작용 0 + 민감도 1 = 1점 → T0** 이 되어 사람이 한 번도 보지 않고 approved 로 갔다. 저장 응답이 이미 같은 규칙으로 본문을 읽고 있었고(`specDelta`) 게이트만 그것을 쓰지 않았다. 이제 현재 approved 본문과 견줘 추가·수정·삭제를 세고, 승인된 요구사항이 사라지면 가역성 2점을 준다.
...
② **T1 의 이의제기 창은 걷는다.** 구현이 없었고(`appealWindowHours` 를 산출만 하고 아무도 읽지 않았다), ①로 위험한 문서가 T2 로 올라간 뒤로 T1 에 남는 것은 기존 문서의 문구 수정 정도다. 거기에 되돌리기 창을 만드는 값이 낮다고 판단했다. `gate_policy.t1_objection_hours` 키는 제거했고 저장된 값은 마이그레이션 `0016` 이 지운다. **T0 과 T1 의 동작은 같다** — 남는 차이는 감사 기록의 등급뿐이다.

**곁가지로 정책이 장식이 아니게 됐다.** `tier_boundaries` 와 `dynamic_escalation` 은 화면에서 고칠 수 있는데 판정이 상수를 쓰고 있었다(2026-09-02 정정). 고칠 수 있고 효과가 없는 값이 화면에 있으면, 그것을 고친 사람은 게이트를 조인 줄 안다.

**동적 강화.** 액션 위험도만이 아니라 세션의 신뢰도도 티어를 올린다. 신호는 둘이다 — **첫 승인 버전**(위 ①)과 **같은 실패 3회 신고**: 에이전트가 `e2e-fail-3x`(clemvion 의 `ESCALATE=e2e-fail-3x` 어휘를 그대로 계승 — 같은 실패 3회 반복)로 올린 에스컬레이션이 이번 제출에 닿으면 선다. **신호가 여럿이어도 티어는 한 단계만 오른다.** 실패가 반복되는 세션에서 같은 액션의 게이트를 강화하는 것은 에이전트 운영의 표준 권고다.

**재시도 신호를 정의하고 롤백 신호를 걷는다**(2026-09-26 — 사람 결정 · 동적 강화 두 신호 검토). 이 절은 오래 "같은 Task 에서 재시도가 임계를 넘거나, 최근 30일 내 해당 영역의 승인 후 롤백 이력이 있으면 +1" 이라 적었는데, 무엇을 셀지 정해진 적이 없어 서버는 둘 다 세지 않았다. ① **같은 실패 3회 신고** — 서버는 테스트 결과를 받지 않으므로(증적에 통과·실패 열이 없고 웹훅은 PR·커밋의 존재만 적는다) 실패를 직접 세지 않고, 에이전트가 `e2e-fail-3x` 로 올린 에스컬레이션(질문의 `escalate` · 발견 처분의 `escalate_reason`)을 센다. 한 건이면 선다 — 임계 3 은 어휘 안에 있다. 신고가 제출에 닿는 길은 셋이다: 그 스펙을 가리키거나 · 그 스펙에서 파생된 작업의 것이거나 · 제출한 세션의 것. **사람이 그 스펙을 마지막으로 승인한 뒤**의 신고만 센다 — 사람이 승인했으면 본 것이고, 자동 통과는 아무도 보지 않았으므로 창을 닫지 않는다. `infra`(환경 탓)와 `sensitive-fix`(민감도)는 실패가 아니라 세지 않는다. 카드는 신호 곁에 그 신고로 가는 링크를 단다([4.4](../04-mvp/api.md) REQ-API-189 · [4.5](../04-mvp/screens.md) REQ-WEB-240). ② **롤백 이력은 걷는다** — "승인 후 롤백" 에 해당하는 전이가 NERV 에 없다: 승인된 문서에서 뒤로 가는 전이가 없고, 과거 버전 복원은 새 버전을 만드는 일이며 그 기능도 아직 없다. **재검토 트리거**: 버전 되돌리기를 만들거나 승인 뒤의 되돌림이 실측되면, 되돌림 버전이 승인된 문서의 가장 가까운 area 조상 아래에서 다음 제출에 30일 동안 서는 신호로 켠다. ③ **여럿이어도 한 단계다** — 신호가 요구하는 것은 사람이 한 번 보는 것이지 두 사람이 아니다. 쌓으면 첫 버전인 T1 문서에 신호 하나가 겹쳐 T3(직군 교차 2인 + 파생 작업의 플랜 승인)까지 갔다. ④ 판정은 **제출 때 한 번**이고, 승인 때의 기록은 그 판정을 옮겨 적는다 — 다시 계산하면 그 사이의 변화가 기록의 티어만 바꿔 슬롯과 어긋난다.
...
승인 대기는 사람의 기억이 아니라 상태 머신으로 관리한다. 받은 요청 항목은 `대기 → 결정 → 만료` 상태를 가진다.

| 시점 | 동작 | 채널 |
| --- | --- | --- |
| T+0 | 받은 요청 카드 생성 + 요청 세션에 "대기 중" 표시 | 인앱(즉시) + critical 티어 채널 |
...
Finding(tag=spec_drift, severity=warning)
   └─▶ nerv_spec_draft_upsert  →  CR draft 자동 생성 (본문·근거·해당 커밋 첨부)
        └─▶ planner 받은 요청 카드 "구현이 스펙을 개선했습니다 — 스펙 갱신 승인?"
...
| 세션 Activity 원문 | TTL **90일**(정책 기본값 `activity_days` — [4.4 API](../04-mvp/api.md) §1.6a), 요약은 영구(`activity_summary`) | 혼합 |
...
구독 규칙 매칭
   ...
   ▼
중요도 티어 산출 → 채널 라우팅 → 배칭 → 발송/받은 요청 승격
```
...
에이전트 세션의 진행 이벤트(도구 호출·파일 수정)는 **알림을 만들지 않는다.** 세션 모니터(S5)와 활동 피드에서 WebSocket으로 흐를 뿐이다. 이걸 알림으로 만들면 세션 수십 개(NFR-04) 환경에서 받은 요청이 즉시 파괴된다.

**구현 범위**(2026-09-02 실측 기준). MVP 는 위 표의 수신자 어휘 중 둘만 산출한다.

- `approval.requested` 는 **지정을 본다** — `assignee_user_id` 가 있으면 그 사람에게만, `assignee_role` 이 있으면 그 역할의 멤버에게, 둘 다 없으면 역할 큐(admin·planner)로 간다. 같은 날 사람 결정으로 붙였다: 결재 경로(EP-APR)는 처음부터 지정을 보는데 알림만 보지 않아, 지정 승인자가 designer·qa 면 **자기 앞으로 온 카드의 알림을 못 받았고** admin·planner 전원은 결정할 수 없는 카드의 알림을 받았다. 결정할 수 없는 카드의 알림은 §6.6 원칙 3 이 막으려는 바로 그 소음이다.
- 나머지 이벤트는 역할 큐(admin·planner)다. 표의 "작성자 · 워처"·"담당자"·"스레드 참여자"는 **워치 테이블이 없어** Phase 2 다([4.5 화면 명세](../04-mvp/screens.md) §2.4 말미).
- 어느 경우든 **행위자 자신은 제외한다** — 자기가 한 일의 알림은 소음이다.

수신자에게 실제로 닿는 경로는 `notification.created` 방송이다([4.4 API 명세](../04-mvp/api.md) §3.3 — 개인 룸).

### 6.4 받은 요청 승격 — 알림을 줄이는 궁극의 방법

**승인·질문·리뷰 요청은 알림이 아니라 작업 항목이다.** 흩어진 핑 대신 미결 액션을 한 곳에서 추적하는 Agent Inbox 패턴이며, NERV의 받은 요청(S7, FR-11)이 그 화면이다. 받은 요청 항목은 자체 상태 머신 `대기 → 결정 → 만료`(§2.6)를 갖고, Slack·메일은 **받은 요청으로 가는 딥링크만** 나른다. 같은 항목에 대해 채널을 넘나드는 중복 발송은 크로스 채널 dedup으로 제거한다 — 받은 요청에서 이미 처리된 항목은 대기 중이던 메일 다이제스트에서 빠진다.
...
├──────────────────────────────────────────────────────────────────────────┤
│ 🔔 통지 · T1 소프트 게이트로 자동 승인됨          [감사 기록됨]           │
│    5-system / 12-webhook · 설명 문구 명확화 (1·0·1·0 = 2) ( 문서 보기 )   │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| 통지 | Notify | 무엇이 왜 바뀌었는지 + 문서 링크(T1 의 이의제기 창은 §2.4 ②로 걷혔다) |
...
3. **결정이 필요한 것만 받은 요청으로.** 나머지는 피드다. 받은 요청 배지 숫자는 "내 결정을 기다리는 것"만 센다.
4. **중복은 채널을 넘어 제거한다.** 받은 요청에서 처리된 항목은 예약된 다이제스트·Slack 발송에서 취소한다.
5. **에이전트 진행 상황은 알림이 아니라 화면이다.** 실시간이 필요한 것은 WebSocket으로 밀고(NFR-02), 사람의 개입이 필요할 때만 받은 요청 항목이 생긴다.
