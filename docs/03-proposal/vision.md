...
>
> 문서 버전 v0.5 · 2026-09-26 · HTML 파생본: [vision.html](../html/vision.html)
>
> v0.5 변경(2026-09-26 — 검증은 전이가 아니라 서명이다, 구현 축 상태도 검토): 14:00 한 줄. `implemented → verified` 를 qa 가 올리는 전이로 적고 있었는데, 그 축은 서버가 파생하고 qa 가 하는 일은 테스트 증적에 서명하는 것이다([3.5](spec-workflow.md) §1.3).
>
> v0.4 변경(2026-09-07 — 하루가 지금 그대로 되지는 않는다, 개선 계획 아홉째 스프린트): **새 요구사항 없음 — 현황 주석 둘.** §2.3 의 하루는 **가야 할 곳**이고 그것을 고치지 않는다. 다만 두 자리가 지금 실물과 달라 주석으로 표시한다: ① 요구사항은 우측 패널의 EARS 템플릿이 아니라 **본문 줄에서 추출된다**(다음 ID 발급 경로는 서버에 있고 웹이 아직 부르지 않는다). ② **게이트 정책 편집은 admin 전용**이라 QA 가 직접 조정할 수 없다. 비전이 현황과 다른 것은 결함이 아니지만, **다르다는 사실을 적어 두지 않는 것**은 결함이다 — 읽는 사람은 그것을 오늘의 기능으로 읽는다.
>
...
**09:25 · [S3 스펙 상세]** 새 기능 "웹챗 위젯 임베드 v2"를 쓴다. 좌측 트리에서 `7-channel-web-chat` 영역을 고르고 `feature` 타입으로 새 스펙을 만든다. 본문은 markdown 에디터(D-09)이고, 우측 패널에서 요구사항을 EARS 템플릿으로 3개 등록한다 — `WHEN 방문자가 위젯을 처음 열면 THE SYSTEM SHALL 이전 대화를 복원한다` 식이다. 각 요구사항은 서버가 발급한 고정 ID(`REQ-CWC-031`)를 받는다(FR-03). *(현황 2026-09-07 — 요구사항은 **본문 줄에서 추출된다**: 우측 패널은 그것을 보여 주고 구현 상태를 잇는다. 다음 ID 를 서버가 발급하는 길은 열렸지만([4.4 API](../04-mvp/api.md) EP-REQ-04) 웹이 아직 그것을 부르지 않는다 — 등록 폼은 이월이다.)*
...
**14:00 · [S4 작업 보드]** 검증을 마친 요구사항을 `implemented → verified`로 올린다. 올리는 것은 사람이 아니라 서버다 — qa(및 admin)가 테스트 증적에 서명하면 파생된다(FR-14 · 2026-09-26 정정). 커버리지 숫자가 즉시 바뀐다.
...
**16:20 · [S8 설정·멤버 → 게이트 정책]** 이번 분기 정책을 조정한다. 저위험 변경(오탈자·주석)은 자동 통과, 인증·결제 경로에 닿는 변경은 사람 승인 필수. *(현황 2026-09-07 — **게이트 정책 편집은 admin 전용**이다: 이 화면은 admin 이 아닌 역할에게 비활성 + 사유로 뜨고 서버도 403 을 낸다([3.5 스펙 워크플로우](spec-workflow.md) §2.4 · S8). 하나(QA)가 직접 조정하려면 admin 을 겸하거나 admin 에게 요청한다.)* 위험 분류를 게이트 티어에 직결하는 이 설계는 필드 리포트의 P0~P3 위험 분류 권고를 정책 UI로 옮긴 것이다.
