...
>
> v1.41 변경(2026-09-14 — 전표가 적은 소비자가 하나 적었고 구분자가 하나뿐이었다, 사람 지시): **새 요구사항 없음 — §5.2 두 행 · 신뢰 오리진 파서 · `.env.example` 에 `NERV_S3_REGION`.** 포트 손잡이 작업(v1.40 이 지도를 깐 그 일)의 **1단계**이고, 판정이 바뀌는 것은 신뢰 오리진 하나뿐이다. ① **`NERV_S3_REGION` 이 `.env.example` 에만 없었다** — 코드가 읽고(`storage.service.ts` 생성자) 전표에도 있는데 실물 전표에서 빠져 있어, MinIO 아닌 S3 를 쓰는 운영자는 그 손잡이의 존재를 알 길이 없었다. 곁들여 그 행의 **소비자가 `api` 만 적혀 있던 것을 `api · worker` 로 정정**했다: `StorageService` 는 `WorkerModule` → `SpecModule` 로 워커의 모듈 그래프에도 올라 생성자에서 같은 값을 읽는다. **게이트 ④ 는 이것을 잡지 못한다** — `api` 만 적혀 있어도 읽는 코드가 있으므로 통과한다(소비자가 *덜* 적힌 어긋남은 지금 아무도 세지 않는다). ② **`NERV_TRUSTED_ORIGINS` 의 구분자가 쉼표뿐이었다.** 다중 등록 자체는 처음부터 동작했지만, 오리진을 한 줄에 하나씩 적는 것이 k8s ConfigMap 의 여러 줄 문자열에서는 자연스러운 모양이고 그렇게 적은 배치에서는 **목록 전체가 한 덩어리의 못 쓰는 값**이 됐다. 쉼표·공백·줄바꿈을 다 받고, 값을 경로 없는 오리진으로 정규화하며, **스킴을 빠뜨린 값은 버린다** — `localhost:5173` 은 URL 로 파싱되지만 그 오리진은 불투명 오리진, 즉 문자열 `"null"` 이라 그대로 두면 `Origin: null` 을 싣는 요청이 통과한다. 스킴을 빠뜨린 오타 하나가 CSRF 방어선에 구멍을 내는 자리였다. 정규화도 버림도 **조용히 하지 않는다**: 사람은 자기가 적은 값이 목록에 없다는 사실을 알아야 한다. ③ `originOf` 를 가드 파일에서 `common/origins.ts` 로 옮겼다 — 두 곳이 **같은 함수로** 오리진을 읽어야 판정이 갈리지 않고, 반대 방향(파서가 가드를 import)은 고리가 된다. **가드의 판정은 한 건도 바뀌지 않았다**(불투명 오리진 거부는 새 파서에만 있다). ④ **목록의 구성은 그대로다** — `NERV_WEB_URL` 을 자동으로 더하지 않으며, L1 이 그 사실을 태운다([4.1](scope.md) §2.3 2단계의 몫이고, 앞당기면 1단계가 선언한 "동작은 한 줄도 바뀌지 않는다" 를 깬다). L1 스위트 여덟을 붙였다.
>
> v1.40 변경(2026-09-14 — 한 숫자를 세 뜻으로 읽고 있었다, 사람 지시): **새 요구사항 없음 — §5.2 전표의 주소·포트 넷을 층으로 가른다 · `.env.example` 을 구역 여덟으로 재편.** `NERV_WEB_URL`·`NERV_API_URL`·`NERV_API_PORT`·`NERV_HTTP_PORT` 가 **다 `8080`** 이었고, 전표는 그 넷이 왜 같은지도 무엇이 다른지도 적지 않았다 — `NERV_API_PORT`·`NERV_HTTP_PORT` 두 행의 **설명 칸이 비어 있었다.** 층은 셋이다: 앞문의 호스트 공개 포트 · API 프로세스의 리슨 포트 · 앞문을 가리키는 공개 오리진 둘. 적어 두자 실물 셋이 드러났다. ① **`NERV_API_PORT` 는 compose 에서 `.env` 를 읽지 않는다** — `api` 서비스에 `"8080"` 으로 박혀 있고 앞문·헬스체크도 그 포트를 본다. 즉 개발 루프 전용 손잡이인데 **거기서도 바꾸면 조용히 깨진다**(`vite.config.ts` 의 프록시 대상과 `origins.ts` 의 `DEFAULT_ORIGIN` 이 `localhost:8080` 으로 박혀 있어 API 만 옮겨 간다). ② 공개 주소 둘이 따라가는 것은 **`NERV_HTTP_PORT`** 이고 `NERV_API_PORT` 가 아니다 — 전표에 없던 결합이라 앞문 포트를 옮긴 운영자는 두 주소를 함께 옮길 이유를 알 수 없었다. ③ **개발 루프에서 두 주소는 이미 갈려 있다** — 화면이 Vite(`:5173`)·API 가 `:8080` 이므로 `NERV_WEB_URL` 은 `:5173` 이어야 하고, `:8080` 으로 둔 배치에서는 스펙 딥링크(E10-S04)가 **화면을 서빙하지 않는 API** 를 가리켜 404 다. `.env.example` 과 `origins.ts` 는 "개발 루프도 앞문 하나가 둘을 함께 서빙한다" 고 적고 있었는데, 같은 파일의 `NERV_TRUSTED_ORIGINS` 주석이 그 반대를 말하고 있었다(이름을 가른 REQ-CB-036 의 근거가 이미 실물로 있던 셈이다). 곁들여 `.env.example` 을 **구역 여덟**으로 재편했다 — 주소·포트 넷을 한 구역에 모으고, 소속이 어긋나 있던 셋을 옮겼다(`NERV_WORKER_TICK_MS` 가 임베딩 구역에 · `NERV_GITHUB_WEBHOOK_SECRET`·`NERV_EXPORT_DIR` 이 MinIO 구역에 있었다). **값은 한 줄도 바꾸지 않았다** — 바꾼 것은 구역과 설명이다.
>
...
>
> v1.34 변경(2026-09-10 — 같은 스택에 두 번 돌리면 빨갛고, 빨강은 매번 다른 자리에 앉았다, 실측): **새 요구사항 없음 — §4.3 분리의 실물 넷 → 다섯 · `pnpm test:e2e` 행 개정.** compose 파일이 약속한 "매 실행이 같은 한 벌에서 출발한다"를 tmpfs 는 **매 기동**까지만 지켰다 — 컨테이너가 살아 있는 동안 넘어가는 것이 셋이다: ① **Valkey 의 요청 쿼터**(`nerv:rl:*` · [4.4 API](api.md) §1.8 웹 세션 600/분 — 웹 L3 한 번이 시드 사용자 **한 명**으로 435~465 요청을 쓴다) ② **api 프로세스 메모리의 인증 쿼터**(better-auth · IP당 `/sign-in/email` 10/분 — 한 번에 로그인 5회라 **세 번째 연속 실행은 `global-setup` 에서 죽는다**) ③ **가입 계정**(`user` 6 → 7 → 8). 쿼터가 바닥나면 그 창이 끝날 때까지 **모든** 요청이 429 라 빨강은 그때 마침 돌던 테스트에 앉는다 — 실측한 세 번의 자리가 다 달랐다(`global-setup` 로그인 · `review-scroll` 발견 카드 · `shell` 로그아웃). **CI 는 늘 새 스택에서 한 번만 돌아 이것을 보지 못한다** — 그래서 로컬에서 되풀이하는 사람만 자기 변경과 아무 상관 없는 빨강을 본다. `e2e-stack.mjs run` 이 **실행 전에 스택을 기동 직후 상태로 되돌린다**(쿼터 키 삭제 · api 재기동 · `migrate`·`seed` 재적재 — 실측 +6초, 연속 4회 전부 36/36 초록). 판정은 하나도 바꾸지 않았다. 대신 **이 스택의 DB 는 매 실행 초기화된다** — 손으로 넣어 둔 행은 다음 `pnpm test:e2e` 가 가져간다(원래 매 실행 폐기 대상인 스택이다).
>
...
> v1.10 변경(2026-09-02 — 정합 점검): §2.2 잡 목록에 **`partition.job.ts`** 를 더한다 — [4.3](database.md) §2.14 가 워커 잡으로 약속한 월 파티션 선생성이고, 없는 동안 서버는 마이그레이션 두 달 뒤에 멈추는 상태였다(REQ-DB-021).
...
5. **실행 전에 기동 직후로 되돌린다**(2026-09-10 추가) — 위 넷이 지키는 것은 매 **기동**까지였다. 컨테이너가 사는 동안 넘어가는 셋(Valkey 요청 쿼터 · api 메모리의 인증 쿼터 · 가입 계정)이 다음 실행의 전제를 바꾼다. `pnpm test:e2e` 는 러너를 부르기 전에 그 셋을 되돌린다(아래 실측).

#### 재실행이 빨갛던 이유 (2026-09-10 실측)

같은 스택에 스위트를 두 번 돌리면 빨갛고, **빨강이 매번 다른 테스트에 앉았다.** 새 스택에서는 재현되지 않는다 — 그래서 CI는 늘 초록이었고, 로컬에서 되풀이하며 고치던 사람만 자기 변경과 아무 상관 없는 빨강을 봤다. 원인은 셋이고 모두 "컨테이너가 사는 동안 남는 것"이다.

| 남는 것 | 어디에 | 한 번의 소비 | 넘으면 |
| --- | --- | --- | --- |
| 요청 쿼터 | Valkey `nerv:rl:*` | 웹 L3 한 번이 시드 사용자 **한 명**으로 435~465 요청(한도 600/분 · §1.8) | 그 고정 창이 끝날 때까지 **모든** 요청이 429 |
| 인증 쿼터 | api 프로세스 메모리(better-auth) | 로그인 5회 + 가입 1회(한도 IP당 10/분) | **세 번째 연속 실행**이 `global-setup` 로그인에서 죽는다 |
| 가입 계정 | DB `user`·`auth_account` | 실행마다 1건(6 → 7 → 8) | 판정은 안 바뀐다 — 다만 "같은 한 벌"이 아니다 |

쿼터는 행과 다르다. **행은 더해져도 시드의 사실이 그대로지만, 예산은 앞 실행이 쓰면 다음 실행이 못 쓴다.** 그리고 바닥나는 시점은 창 경계가 어디 떨어지느냐로 정해지므로, 빨강은 그때 마침 돌던 테스트에 앉는다 — 실측한 세 번이 각각 `global-setup` 로그인 · `review-scroll` 발견 카드 · `shell` 로그아웃이었다. 원인을 가리키지 않는 빨강이 가장 나쁜 신호다.

`e2e-stack.mjs run` 이 러너를 부르기 전에 되돌린다: **쿼터 키 삭제**(`nerv:rl:*` 만 — `FLUSHALL` 은 나중에 누가 Valkey에 둔 다른 것까지 조용히 가져간다) · **api 재기동**(인증 쿼터가 그 프로세스 메모리에 있어 다른 방법이 없다) · 그 김에 **`migrate`·`seed` 재적재**(한 번짜리 의존이라 `up --wait` 가 다시 돌린다 — 가입 계정이 여기서 지워진다). 비용은 실측 **+6초**이고, 연속 4회 실행이 전부 36/36 초록이다. **판정은 하나도 바꾸지 않았다** — 지우는 것은 잔량뿐이고, 되돌린 상태가 곧 CI가 늘 보는 그 상태다.

> 대신 **이 스택의 DB는 매 실행 초기화된다.** 손으로 넣어 둔 행은 다음 `pnpm test:e2e` 가 가져간다 — 원래 매 실행 폐기 대상인 스택이고, 그 대가로 "매 실행이 같은 한 벌에서 출발한다"가 기동뿐 아니라 실행 단위로도 참이 된다.

#### 관측 스펙은 자기 신원으로 돈다 (2026-09-10 · 실측)

쿼터의 주체는 **사용자**다(§1.8 · 웹 600/분). 그래서 스위트 전체가 한 사람으로 돌면 그 한도가 곧 **스위트가 자랄 수 있는 상한**이 된다 — 검사를 더할수록 남의 검사가 깨지는 구조이고, 규약 7 이 말하는 "비용이 자라는 검사에 고정 상한" 과 같은 모양이다.

관측 스펙 둘(`audit`·`shots`)을 조직 admin(`admin@example.com`)으로 옮겼다. 그 둘을 고른 이유는 **판정이 하나도 없기** 때문이다(`audit` 은 구조 덤프, `shots` 는 스크린샷) — 신원이 바뀌어도 깨질 판정이 없고, 바뀌는 것은 무엇이 관측되느냐뿐이다. 그리고 그 둘이 로드의 절반 가까이를 쓴다.

| | 한 실행의 요청(한도 600/분) |
| --- | --- |
| 축 통일 전(2026-09-10 이전) | `jimin` 혼자 **435~465** |
| 축 통일 뒤([4.5](screens.md) §1.4) | `jimin` 혼자 **375~382** |
| 신원을 나눈 뒤 | `jimin` **209** · `admin` **183** |

**곁들여 얻은 것이 더 크다.** 시드의 `jimin` 은 프로젝트 planner + developer 이고 조직 admin 이 아니라, `/settings/members`·`/settings/gates` 는 **잠긴 컨트롤만** 관측돼 왔다 — 디자인 확인의 대상인 실제 폼(역할 토글·초대·tier 입력)을 아무도 본 적이 없었고, 스크린샷 목록에는 그 두 화면이 아예 없었다. 둘을 목록에 넣었다(스위트 36 → 38). 조직 admin 은 조직 스코프 멤버십(`project_id` NULL)이라 `rolesInProject` 가 그것을 포함해 **프로젝트 화면은 그대로 다 보인다**. 잠긴 쪽의 *판정*은 L1 이 태우므로(`routes/settings/workspace.spec.tsx`) 잃은 것은 그림뿐이고, 그 그림은 원래 확인에 쓰이지 않던 것이다.
...
| `pnpm test:e2e` | **스택을 기동 직후 상태로 되돌린 뒤**(쿼터 키·api·시드 — 아래 실측) 할당된 환경(`NERV_E2E_BASE_URL`·`DATABASE_URL`)을 실어 러너를 돌린다 |
...
런처가 하는 일 다섯: ① `.env` 가 없으면 **먼저 멈추고 안내한다**(없는 것과 안 읽은 것은 다른 문제인데 에러 메시지가 같아진다 — 파일을 대신 만들지는 않는다: [AGENTS.md](../../AGENTS.md) 구현 작업 규약 8) ② 저장소 루트에서 `tsc -b --watch` 를 돌려 참조 전체(schema → api → cli)를 빌드한다 ③ `dist/main.js` 가 **생긴 뒤에** API 를 띄운다(빈 체크아웃의 첫 실행이 실패하지 않게) ④ 하나가 죽으면 전부 내린다 — 반쯤 살아 있는 루프가 가장 헷갈린다 ⑤ Vite 는 **`apps/web` 의 shim** 으로 띄우고 없으면 먼저 멈춘다(2026-09-14 신설).

**⑤ 가 왜 있나**(실측). `vite` 는 `apps/web` 의 의존이고 이 저장소에는 호이스팅 설정이 없으므로 pnpm 은 **루트 `node_modules/.bin` 에 `vite` 를 만들지 않는다.** 런처는 오래 그 루트 경로를 불렀고, 옛 레이아웃이 남긴 고아 shim 이 그 자리에 있어서 돌았을 뿐이다 — `pnpm install` 은 그 파일을 관리하지도 지우지도 않는다. 그래서 두 가지가 조용히 성립했다: **새로 클론한 장비에는 처음부터 없고**, 있던 장비에서도 store 의 peer 해시가 바뀌는 순간 죽는다. 루트 devDependency 를 하나 더한 날(`yaml`) 그 해시가 `vite@8.2.2_…_tsx@4.23.12` → `…_yaml@2.9.1` 로 바뀌어 낡은 shim 이 없는 경로를 열었고, 사람이 받은 것은 `Cannot find module …/vite/bin/vite.js` 였다.
...
**두 리슨 포트가 자기 층의 모든 자리를 정한다**(2026-09-14 · REQ-CB-038). 그 전에는 `NERV_API_PORT` 가 전표에 있고 코드가 읽는데도 **아무 배치에서도 듣지 않았다** — compose 가 `"8080"` 을 박아 두었고 업스트림·헬스체크도 그 숫자를 따로 적었으며, 개발 루프에서는 `vite.config.ts` 의 프록시 대상이 박혀 있어 포트를 옮기면 API 만 옮겨 갔다. 이제 compose·이미지·개발 루프가 한 변수에서 조립하고, 리터럴이 되돌아오면 게이트가 잡는다(⑦). **k8s 는 예외다** — `containerPort` 는 정적 필드라 ConfigMap 값을 참조할 수 없어 두 곳을 함께 고쳐야 하고, 어긋남은 게이트가 센다(⑨).

**개발 루프에서는 그 둘이 실제로 갈린다.** 화면은 Vite(`:5173`)에 뜨고 API 는 `:8080` 이라 `NERV_WEB_URL` 은 `http://localhost:5173` 이어야 한다 — `:8080` 으로 두면 에이전트가 붙여 주는 스펙 딥링크(E10-S04)가 **화면을 서빙하지 않는 API** 를 가리켜 404 다(SPA 는 앞문의 몫이고 api 는 정적 자산을 서빙하지 않는다). 이름을 가른 이유(REQ-CB-036)가 이미 실물로 드러나 있는 자리인데, 아래 표의 기본값 열은 compose 경로의 값이라 그 사실이 어디에도 적혀 있지 않았다. `NERV_API_URL` 은 개발 루프에서도 `:8080`(API 프로세스) 그대로이고, 오리진이 갈리는 몫은 `NERV_TRUSTED_ORIGINS` 가 받는다.

**한 `.env` 가 두 위상을 섬기는 대가가 `NERV_WEB_PORT` 에 있다.** compose 에서 맞는 값(`8080`)이 든 `.env` 로 `pnpm dev` 를 돌리면 Vite 가 api 와 **같은 포트를 잡는다** — `NERV_WEB_URL` 때는 틀린 딥링크로 끝났지만 이번엔 기동이 깨진다. 그래서 기본값을 경로별로 두고(compose `8080` · 개발 루프 `5173`), `pnpm dev` 가 두 포트가 같으면 **먼저 멈추고 이유를 말한다**. 그냥 띄우면 사람이 받는 것은 원인을 가리키지 않는 `EADDRINUSE` 이거나 반쪽만 살아 있는 루프다.
...
| `NERV_TRUSTED_ORIGINS` | | (비움) | api(better-auth) | baseURL 밖에서 화면을 띄울 때만 추가(쉼표 구분) — CSRF 방어선이라 기본은 비운다. 개발 루프는 화면이 Vite(:5173)·API 가 :8080 이라 오리진이 달라 `http://localhost:5173` 이 필요하다. compose 는 둘이 같아 불요. 구분자는 **쉼표·공백·줄바꿈** 셋 다다(2026-09-14 — 쉼표뿐이라 오리진을 한 줄에 하나씩 적은 배치에서는 목록 전체가 한 덩어리의 못 쓰는 값이 됐다. k8s ConfigMap 의 여러 줄 문자열이 그 모양이다). 값은 **경로 없는 오리진으로 정규화**되고, **스킴을 빠뜨린 값은 버려진다** — `localhost:5173` 은 URL 로 파싱되지만 그 오리진은 불투명 오리진, 즉 문자열 `"null"` 이라 그대로 목록에 넣으면 `Origin: null` 을 싣는 요청(샌드박스 iframe · 일부 리다이렉트)이 통과한다. 정규화한 사실도 버린 사실도 **기동 로그가 한 줄로 말한다** — 모르면 열어 둔 줄 아는 오리진이 실은 없는 것이 된다. 읽는 것은 `common/origins.ts` 의 `trustedOriginsFromEnv` 이고 `/mcp` 가드와 **같은 함수로** 오리진을 읽는다. 목록의 **구성은 그대로다** — `NERV_WEB_URL` 을 자동으로 더하지 않는다([4.1](scope.md) §2.3 2단계의 몫) |
...
# ${NERV_PUBLIC_ORIGIN}·${NERV_WEB_PORT} 가 치환된 뒤 로드되므로, compose 와 k8s 가
# 같은 이미지를 쓴다.
#
# **`listen` 에 변수를 쓸 수 있는 이유가 그 치환이다.** nginx 자신은 `listen` 에서 변수를
# 해석하지 못하지만, envsubst 는 nginx 가 파일을 읽기 **전에** 텍스트를 갈아 둔다. 대가는
# 변수가 비면 `listen ;` 이 되어 앞문이 기동에 실패한다는 것이고, 그 방어선은 이미지의
# `ENV NERV_WEB_PORT` 기본값이다(Dockerfile.web · 전표 §5.2).
...
FROM runtime AS api
# 문서용 선언이다 — 실제 리슨 포트는 NERV_API_PORT 가 정한다(전표 §5.2). 값이 어긋나도
# 동작은 바뀌지 않으므로 조용히 낡을 수 있는 자리다.
EXPOSE 8080
