...
> v1.75 변경(2026-09-26 — 아무도 부르지 않는 문구가 74개였다): **REQ-CB-057 신설 · §3.4 한 단락.** 카탈로그의 키마다 제품 코드가 부르는지 L1 이 확인한다. 게이트 수는 그대로다 — L1 이라 preflight·CI 가 이미 돈다. 이번에 지운 키 40개와 연결한 키 하나(`cli.report.counts`), 연결을 기다리는 키 셋은 [4.8](backlog.md) §1.4 에 있다.
...
| `PLUGIN_COVERAGE_WINDOW_DAYS` | `30` | 플러그인 활성화 현황이 보는 세션의 창(2026-09-24 · E12-S03). 호스트는 등록되지 않으므로 창이 없으면 떠난 기계가 분모를 영원히 끈다 — 30일은 파일럿 한 주기(2주 스프린트 둘)다([4.4](api.md) §2.5c) |
...
zod 스키마의 검증 메시지는 키를 담는다 — 요청 본문 검사(`parseBody`)는 어긴 규칙에 카탈로그 키가
달려 있으면 봉투의 `message` 를 그 문구로 만든다([4.4](api.md) REQ-API-191 · 상한·하한은 `{max}`·`{min}`).
키가 없는 위반은 `error.request.schema` 다 — 이 문장은 2026-09-26 까지 약속으로만 있었다.

**쓰지 않는 문구는 지운다**(2026-09-26 — REQ-CB-057). 타입은 없는 키를 부르는 것만 막고, 아무도 부르지 않는 키는 보지 않는다. 화면과 기능을 없앨 때 문구가 따라 지워지지 않아 그런 키가 74개였다 — 번역하는 사람은 그 문장을 계속 고치고, 읽는 사람은 그런 화면이 있다고 믿는다. 쓰지 않는 키가 결함을 가리킨 자리도 있었다: CLI 리포트가 카탈로그와 같은 문장을 한국어로 박아 두어 영어 리포트에 그 줄만 한국어였다. 그래서 L1(`packages/schema/src/i18n/catalog-usage.spec.ts`)이 제품 소스(테스트 제외)에서 키마다 쓰임을 찾는다. 실행 중에 키를 조립하는 자리(`` t(`status.${…}`) ``)는 접두사 목록에 등록하고, 그 이름공간이 다른 문구와 섞여 있으면 값까지 적는다. 지우면 안 되는데 아직 부르는 곳이 없는 키는 사유와 함께 따로 적는다 — 연결하면 검사가 그 목록에서 빼라고 실패한다.
...
| **REQ-CB-057** | WHEN 카탈로그에 키가 있으면, THE SYSTEM SHALL 제품 코드(테스트 제외)가 그 키를 문자열 그대로 부르거나 등록된 동적 접두사 아래에 있음을 L1 에서 확인한다 — 쓰지 않는 키는 지우고, 연결을 기다리는 키는 사유와 함께 예외 목록에 둔다. WHEN 동적 접두사의 키를 만드는 자리가 없어지면 THE SYSTEM SHALL 그 접두사를 목록에서 빼게 한다. |
...
jobs:
  check:                       # 매 PR — **열한 단계**. `pnpm preflight` 가 이것을 같은 순서로 비춘다
    ...
    steps:
      ...
      - name: 배포 산출물 정합   # 백업 스크립트 사본 diff · 오버레이 2종의 **렌더 결과**(REQ-CB-046)
        working-directory: .
        run: |
...
| `NERV_WEB_URL` | | `http://localhost:8080` | api(스펙 딥링크의 절대 URL · **CORS 허용 오리진**과 better-auth 신뢰 목록의 기준 · `/mcp` Origin 대조 대상) · web 컨테이너(nginx 의 `/mcp` Origin 1차 검증 — `NERV_PUBLIC_ORIGIN` 으로 받는다) | **사람이 브라우저로 여는 주소.** 경로 없는 오리진만. 기본값은 compose 경로의 값이다 — **개발 루프에서는 `http://localhost:5173`** 이다(화면이 Vite 에 뜬다 · 위 문단) |
...
| `NERV_TRUSTED_ORIGINS` | | (비움) | api(better-auth · CORS · `/mcp` 가드 — 셋이 한 목록을 본다) | 화면 주소 밖에서 화면을 띄울 때만 추가 — CSRF 방어선이라 기본은 비운다. 개발 루프는 화면이 Vite(:5173)·API 가 :8080 이라 오리진이 달라 `http://localhost:5173` 이 필요하다. compose 는 둘이 같아 불요. 구분자는 **쉼표·공백·줄바꿈** 셋 다다(2026-09-14 — 쉼표뿐이라 오리진을 한 줄에 하나씩 적은 배치에서는 목록 전체가 한 덩어리의 못 쓰는 값이 됐다. k8s ConfigMap 의 여러 줄 문자열이 그 모양이다). 값은 **경로 없는 오리진으로 정규화**되고, **스킴을 빠뜨린 값은 버려진다** — `localhost:5173` 은 URL 로 파싱되지만 그 오리진은 불투명 오리진, 즉 문자열 `"null"` 이라 그대로 목록에 넣으면 `Origin: null` 을 싣는 요청(샌드박스 iframe · 일부 리다이렉트)이 통과한다. 정규화한 사실도 버린 사실도 **기동 로그가 한 줄로 말한다** — 모르면 열어 둔 줄 아는 오리진이 실은 없는 것이 된다. 읽는 것은 `common/origins.ts` 의 `trustedOriginsFromEnv` 이고 `/mcp` 가드와 **같은 함수로** 오리진을 읽는다. **목록의 구성이 2026-09-20 에 바뀌었다**(REQ-CB-041): 허용 오리진은 `NERV_WEB_URL` + 이 값의 **합집합**이고 CORS 와 better-auth 가 같은 목록을 본다([4.1](scope.md) §2.3 2단계 · 아래 §5.2c). 그래서 개발 루프에서 `NERV_WEB_URL` 을 `:5173` 으로 둔 배치는 이 값을 비워도 된다 — `.env.example` 이 그 값을 남겨 두는 것은 공개 주소를 compose 값(`:8080`)으로 둔 채 개발 루프를 돌리는 사람을 위해서다 |
| `NERV_COOKIE_DOMAIN` | | (비움 — 호스트 전용) | api · worker(better-auth 쿠키 속성) | 세션 쿠키의 `Domain`(REQ-CB-042 · §5.2c). **비우는 것이 기본이고, 서브도메인으로 가른다고 필요해지는 값이 아니다** — 쿠키는 API 호스트가 내주고 브라우저는 그 호스트로 도로 싣는다. `SameSite=Lax` 가 보는 것은 오리진이 아니라 **사이트**라 두 호스트가 같은 등록 도메인 아래면 그대로 선다([4.1](scope.md) §2.3 의 확정이 그것을 요구하는 이유다). 적는 것은 쿠키를 한 호스트보다 **넓게** 둘 때이고(같은 도메인 아래의 다른 화면·프리뷰가 같은 세션을 써야 하는 배치), 그러면 그 도메인 아래 **모든 호스트**가 세션 쿠키를 받는다 — 신뢰하지 않는 호스트를 그 아래 두지 않는다는 운영 약속이 함께 간다. 두 공개 주소의 공통 상위가 아닌 값이면 **기동을 거부한다**: 틀린 `Domain` 은 브라우저가 조용히 버려 "로그인은 200 인데 세션이 없다" 로만 드러나므로, 뜨기 전에 잡는 편이 싸다. 앞의 점(`.example.com`)은 값의 일부가 아니다(RFC 6265) |
...
### 5.2c 허용 오리진과 쿠키 도메인 — 브라우저가 다른 오리진에서 부를 때 (2026-09-20 신설)

[4.1 MVP 범위와 스택 확정](scope.md) §2.3 의 **2단계**다. 1단계는 이름만 갈랐고(REQ-CB-036), 화면이 API 와 다른 오리진에 뜨는 순간 필요한 것이 둘 남아 있었다 — **브라우저가 그 호출을 허락받는가**(CORS)와 **세션 쿠키가 어디까지 실리는가**(Domain)다. 한 호스트가 둘을 함께 서빙하는 지금 배치에서는 **동작이 바뀌지 않는다**: 같은 오리진에서는 프리플라이트가 일어나지 않고, 쿠키 도메인의 기본값은 비움이다.

**허용 오리진은 한 목록이다**(REQ-CB-041). 구성은 `NERV_WEB_URL` + `NERV_TRUSTED_ORIGINS` 의 합집합이고, 읽는 것은 `common/origins.ts` 의 `allowedOriginsFromEnv` **하나**다 — CORS(`@fastify/cors`)와 better-auth 의 `trustedOrigins` 가 같은 함수를 부른다. 두 곳이 갈리면 증상이 사람을 엉뚱한 곳으로 보낸다: CORS 만 좁으면 로그인은 되는데 그 다음 요청이 전부 막히고, better-auth 만 좁으면 프리플라이트는 통과하는데 로그인만 `INVALID_ORIGIN` 이다.

| 자리 | 무엇 | 왜 |
| --- | --- | --- |
| 허용 오리진 | `NERV_WEB_URL` + `NERV_TRUSTED_ORIGINS` — **와일드카드 없음** | `credentials: true` 와 `*` 는 브라우저에서 함께 서지 않는다. 요청의 `Origin` 을 그대로 비추는 것도 하지 않는다 — 비추는 순간 허용목록이 아니라 개방이다 |
| `Origin` 없는 요청 | 그대로 통과 · CORS 헤더 없음 | 에이전트·CLI·서버 대 서버는 CORS 의 대상이 아니다. 여기서 막으면 브라우저가 아니라 CLI 가 죽는다 |
| 목록 밖 오리진 | 헤더를 **붙이지 않을 뿐** 오류를 내지 않는다 | 막는 것은 브라우저의 몫이다. 5xx 로 답하면 "서버가 고장났다" 로 읽힌다 — 고장난 것은 부르는 쪽의 오리진이다 |
| 요청 헤더 | `accept` · `accept-language` · `authorization` · `content-type` · `idempotency-key` · `x-nerv-org` | 화면이 **실제로 싣는 것**만이다(`apps/web/src/lib/api.ts`). 목록에 없는 헤더는 프리플라이트에서 거절되어 요청이 서버에 닿지 않는다. `/mcp`·`/ingest` 의 헤더는 없다 — 브라우저 클라이언트가 없는 표면이고([4.4 API](api.md) §1.1), 생기면 그때 목록이 자란다 |
| 응답 헤더 노출 | `Idempotency-Replayed` · `Retry-After` | 적지 않으면 브라우저 JS 에서 **보이지 않는다**(기본 노출은 여섯 개뿐이다). 멱등 재생 표시와 429 의 대기 시간이다 |
| 프리플라이트 캐시 | 600초 | 더 크게 적어도 브라우저가 자기 상한으로 자른다 |

**쿠키 도메인은 손잡이지 필수가 아니다**(REQ-CB-042). 세션 쿠키는 API 호스트가 내주고 브라우저는 **그 호스트로 보내는 요청에** 도로 싣는다. `SameSite=Lax` 가 보는 것은 오리진이 아니라 **사이트**이므로, 화면과 API 가 같은 등록 도메인 아래의 서브도메인 둘이면 호스트 전용 쿠키 그대로 선다 — [4.1](scope.md) §2.3 이 "같은 등록 도메인 아래여야 한다" 를 확정으로 둔 이유가 그것이다. `NERV_COOKIE_DOMAIN` 은 쿠키를 한 호스트보다 **넓게** 둘 때의 값이고, 넓히면 그 도메인 아래 모든 호스트가 세션 쿠키를 받는다(그 대가가 §2.3 이 적어 둔 운영 약속이다).

**틀린 값은 조용하다 — 그래서 기동을 거부한다.** 브라우저는 자기 호스트의 상위가 아닌 `Domain` 쿠키를 버리면서 아무 말도 하지 않는다: 로그인 응답은 200 인데 다음 요청에 세션이 없고, 사람이 보는 것은 "로그인이 안 된다" 하나다. 그래서 두 공개 주소의 공통 상위가 아니거나, IP 이거나, 라벨이 하나(공개 접미사)이면 api·worker 가 뜨지 않는다. 판정은 Nest 초기화 **전에** 한 번 더 본다 — 초기화 중에 던지면 `NestFactory` 의 `abortOnError` 기본값이 프로세스를 abort 시켜 운영자가 받는 것이 문구가 아니라 덤프가 된다.

**라이브러리의 테스트 모드를 껐다**(2026-09-20 실측 · better-auth 1.7.1). `advanced.disableOriginCheck` 를 비워 두면 better-auth 는 `isTest()` 일 때 오리진 검증을 **스스로 끈다**. 운영에서는 켜져 있으니 동작은 같지만, 그 상태로는 **L2 가 방어선을 끈 채 초록을 본다** — 위 목록을 CSRF 방어선이라 적어 두고 한 번도 태우지 않는 셈이다. `false` 를 명시해 테스트와 운영의 자세를 같게 했고, 그 덕에 아래 검증이 성립한다.
...
deploy/k8s/
  base/
    ...
    ingress.yaml                 # §6.3 전문 — **api. 호스트만**(표면 여섯) · WS 업그레이드 · 타임아웃
  overlays/
    dev/
      kustomization.yaml         # 이미지 태그 · host(dev) · configmap · replica 1 패치 · resources 에 ../../base/web
      ingress-api-host.yaml      # JSON6902 — api. 호스트 문자열만 (전략적 병합은 rules 를 덮는다 · REQ-CB-046)
      ingress-web-host.yaml      # JSON6902 — app. 호스트 문자열만
...
**앞문이 둘이다**(2026-09-20 · [4.1](scope.md) §2.3 4단계 · REQ-CB-045). `base/ingress.yaml` 은 **API 호스트만** 알고, 화면의 `/` 규칙은 `base/web/ingress.yaml` 로 갈라져 나갔다 — 그 셋(Deployment·Service·Ingress)은 base resources 에 없고 **화면을 파드로 세우는 배치만** 더한다.
...
spec:
  ...
  tls:
    - hosts: [api.nerv.example.com]
      secretName: nerv-api-tls
  rules:
    - host: api.nerv.example.com
      http:
        paths:
          ...
          # 이 **어디에도 없는 경로**가 되어 마켓플레이스 추가가 실패한다(호스트를 가르기
          # 전에는 web 으로 가 SPA 의 index.html 을 200 인 채로 받았다 — 증상만 바뀌었다)
          - { path: /plugin,    pathType: Prefix, backend: { service: { name: nerv-api, port: { name: http } } } }
```

`base/web/ingress.yaml` (**base resources 에 없다** — 자가호스팅 오버레이가 `../../base/web` 으로 더한다):
...
metadata:
  name: nerv-web
  annotations:
    nginx.ingress.kubernetes.io/proxy-read-timeout: "60"     # SPA 와 /config.json 뿐이다 — SSE·WS 는 api. 로 간다
spec:
  ingressClassName: nginx
  tls:
...
**호스트는 오버레이가 JSON6902 로 갈아 끼운다** — `host` 문자열 하나와 TLS 의 호스트 하나다. 전략적 병합 패치로 `rules` 를 덮으면 **경로가 통째로 사라지는데**, 2026-09-20 까지 두 오버레이가 정확히 그 모양이었다: base 의 7개가 렌더 결과에서 **0개**였고, `kubectl kustomize` 도 `apply` 도 롤아웃도 전부 성공한다. 죽는 것은 트래픽뿐이고 그것은 배포가 끝난 뒤에 드러난다. 게이트가 이제 **렌더 결과**를 센다(REQ-CB-046 · `check-k8s-render.mjs`).
...
| **REQ-CB-013** | WHEN `POST /mcp` 요청의 `Origin` 헤더가 존재하고 **`NERV_API_URL` 과 허용 오리진 목록(§5.2c) 어느 것과도 다를 때**, THE SYSTEM SHALL 앱 가드에서 403을 반환한다 — 전단(nginx·Ingress)의 차단 여부와 무관하게. (2026-09-13 개정 — 대조 대상이 **둘**이 됐다: 전에는 이름 하나가 화면과 API 의 주소를 겸해 하나로 보였다 · REQ-CB-036. 2026-09-20 개정 — 거기에 `NERV_TRUSTED_ORIGINS` 가 **함께 든다**: 허용 오리진을 CORS·better-auth·이 가드가 한 출처에서 읽지 않으면, 운영자가 화면을 띄우라고 선언한 오리진이 표면마다 다르게 취급된다 · REQ-CB-041. 값이 같은 배치에서는 판정이 한 건도 달라지지 않는다. **앞문은 여전히 오리진 하나만 안다** — nginx `map` 은 envsubst 치환이 한 줄이고 같은 값 두 줄이면 기동을 거부하므로, 1차 차단은 화면 주소 하나를 보고 최종 강제는 이 가드다) |
...
### 6.3a 호스트를 가르는 절차 — 되돌리는 길을 먼저 깐다 (2026-09-20 신설)

한 호스트로 돌고 있는 배치를 `app.` / `api.` 둘로 옮기는 순서다([4.1](scope.md) §2.3 4단계). **옛 호스트는 지우지 않는다** — 되돌리는 길이 그것이다.

| # | 무엇 | 왜 이 순서인가 |
| --- | --- | --- |
| 1 | DNS 에 `app.`·`api.` 를 **옛 호스트와 같은 LB** 로 올린다 | 이름만 늘린다. 트래픽은 아직 옛 이름으로 온다 |
| 2 | 인증서를 두 호스트로 받는다(SAN 둘 또는 와일드카드) | **인증서가 먼저다** — 없는 호스트로 규칙을 열면 브라우저가 TLS 에서 막고, 그때의 오류는 원인을 가리키지 않는다 |
| 3 | 오버레이의 호스트 패치와 ConfigMap 의 주소 둘을 새 이름으로 바꾸고 적용한다 | 이 둘은 **함께** 간다 — 게이트가 그 일치를 센다(REQ-CB-046). 적용 직후 새 호스트로 로그인까지 태워 본다(쿠키·CORS·`/config.json` 이 한 번에 드러난다) |
| 4 | 옛 호스트 규칙을 **남긴 채** 사람들을 새 주소로 옮긴다 | 옛 이름이 살아 있는 동안은 한 줄을 지우는 것으로 되돌아간다 |
| 5 | 플러그인의 하드코딩된 주소를 `api.` 로 바꾸고 **`plugin.json` 버전과 함께** 배달한다([4.6](plugin.md) · REQ-PLG-017) | **마지막이다.** 먼저 바꾸면 배달만 되고 서버는 옛 주소다. 설치한 쪽은 버전이 바뀔 때만 새 사본을 받는다 |
| 6 | 한 릴리스를 지켜본 뒤 옛 호스트 규칙을 걷는다 | 걷는 순간 되돌리는 길이 없어진다 — 그래서 마지막이고, 급할 이유가 없다 |

**되돌리는 길은 3번 하나를 되돌리는 것**이다: 오버레이의 호스트 패치와 ConfigMap 의 주소 둘을 옛 이름으로 되돌리고 적용한다. 5번까지 갔다면 플러그인도 함께 되돌려야 하는데, 그것은 **버전을 한 번 더 올리는 일**이라 시간이 든다 — 그래서 5번을 4번 뒤에 두었다.

**CDN 배치는 3번이 다르다.** 웹 파드 대신 CDN 에 화면을 올리고 `/config.json` 을 그 자리에 놓는다(§5.2d) — 오버레이는 `../../base/web` 을 더하지 않고, `app.` Ingress 도 서지 않는다. 그 배치에서 이 문서가 세는 것은 API 호스트뿐이다.
...
images:
  ...
  - name: nerv-web
    ...
    newTag: "<git-sha>"
patches:
  # 호스트는 **JSON6902 로 문자열만** 바꾼다 — 전략적 병합은 rules 를 통째로 덮어
  # 경로를 0개로 만든다(REQ-CB-046 이 그 렌더 결과를 센다)
