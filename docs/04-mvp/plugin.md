...
>
> v0.51 변경(2026-09-05 — 이름만 있던 인자에 뜻을 준다, 정합성 감사 → 사람 결정): **패키지 0.2.8 → 0.2.10.** `skills/spec` 이 `requirement_id` 를 **쓰라고** 말하게 한다(규약 6) — 어느 요구사항 주변을 뒤질지 알면 그것으로 좁히고, 없는 요구사항은 거절이라 오타를 바로 안다.
...
> v0.49 변경(2026-09-05 — 파생본이 원본과 다른 말을 하고 있었다, 정합성 감사): html 파생본이 **훅 두 변형을 뒤바꿔** 싣고 있었다 — §3.1 `hooks/hooks.json` 자리에 http 변형이, `hooks.http.json` 자리에 command 변형이 있어 **html 만 읽으면 기본 훅이 http 로 읽혔다**(v0.30 의 사람 결정이 파생본에서 뒤집혀 있었다). §1.1 은 더 나빴다: 디렉터리 트리 자리에 statusline 스크립트가, `plugin.json 전문` 라벨 아래에 hooks.json 이 들어 있어 **매니페스트가 html 에 아예 없었다** — "버전이 곧 배달이다" 의 그 버전이 파생본에 없었다는 뜻이다. §1.2 MVP 포함/제외 표(11행)와 **§2.0·§2.1(`skills/next/SKILL.md` 전문)이 통째로 빠져 있어** 목차의 `#sec-2` 가 죽은 링크였다.
...
> v0.47 변경(2026-09-05 — 새로 생긴 인자를 스킬이 알아야 한다, 규약 6): **패키지 0.2.6 → 0.2.7.** `nerv_spec_search` 가 `type`·`status` 를 받게 됐으므로([4.4](api.md) v0.80 · REQ-API-099) `skills/spec` 이 그것을 **쓰라고** 말하게 한다 — 받는다고만 적고 무엇을 하라는 말이 없으면 그 기능은 없는 것과 같다. 트리와 같은 쉼표 목록 표기라는 것까지 적는다: 같은 뜻의 인자가 도구마다 다른 모양이면 에이전트는 실패로 배운다.
...
1. `nerv_spec_tree`(`project`, 필요 시 `root`·`depth`·`status`·`type`·`baseline`)로 트리
   ...
   지어내지 않는다** — 트리는 `root`(고정 ID 또는 UUID — 그 문서와 그 아래만)·`depth`(뿌리에서
...
#### 플러그인이 켜졌는가는 플러그인만 말한다 — `X-NERV-Plugin` (2026-09-24 신설 — E12-S03)

세션은 두 길로 들어온다 — 이 훅(플러그인)과 MCP `nerv_bootstrap`(플러그인 없이도 된다). 서버는 둘을 가를 칸이 없어서, 관리형 settings 로 배포한 뒤 **"어느 기계에서 켜졌는가"** 에 답하지 못했다(백로그 E12-S03 의 마지막 남은 줄).

포워더가 **세션을 만드는 훅에서만** 자기 버전을 싣는다. 버전은 스크립트 옆의 `plugin.json` 이 정본이다(§3.6 · REQ-PLG-017) — 그래서 버전을 올리면 헤더도 따라 오른다.

```bash
manifest="$(dirname "${BASH_SOURCE[0]}")/../.claude-plugin/plugin.json"
version="$(sed -n 's/^[[:space:]]*"version"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' "${manifest}" | head -n 1)"   # → X-NERV-Plugin
```

- **`session` 에만 붙인다.** 다른 훅은 같은 세션이라 읽는 곳이 없다.
- **Codex 에는 없다.** Codex 경로에는 `plugin.json` 이 없어 헤더가 빠진다 — 플러그인은 Claude Code 의 것이고, 없는 것을 있다고 말하지 않는다. 서버도 활성화 현황을 Claude Code 세션만으로 센다([4.4 API 명세](api.md) §2.5c).
- **http 변형에는 없다.** `headers` 는 상수와 `${VAR}` 만 받아 `plugin.json` 을 읽을 자리가 없다 — 상수로 박으면 버전을 올릴 때마다 한 자리가 더 늘어난다.
...
**설치되는 주소는 따로 있다**(2026-09-04 · 실기기 실측). 카탈로그 추가는 `http://localhost` 로도 성공하지만 **설치가 거부된다** — `Archive URLs must use https:// and must not point at a loopback, link-local, or cloud-metadata host`. 두 단계 사이에서 갈라지므로 운영에서 `NERV_API_URL` 이 `http://` 이거나 내부 주소면 "추가는 됐는데 설치가 안 된다" 가 된다. 개발 기본값은 설치 불가이고 그것이 맞다 — 개발은 위 표의 **수동** 경로를 쓴다.

**그 판정의 정본은 `@nerv/schema` 의 `checkPluginInstallUrl()` 하나다**(2026-09-21 · [4.2](codebase.md) §1.2 · REQ-CB-006). 두 표면이 그것을 본다 — 서버는 카탈로그를 만들 때 운영자 로그에 한 번 경고하고, 화면은 설치 장의 값 카드에서 **사람이 명령을 치기 전에** 말한다([4.5](screens.md) REQ-WEB-165). 조건은 둘이고 서로 다른 거부다: **https 가 아니거나**(`not_https`), 호스트가 **루프백·링크로컬**이거나(`loopback` — 클라우드 메타데이터 주소가 여기 든다). 그래서 https 의 공개 호스트, 즉 정상 운영에서는 **한 번도 걸리지 않는다.** 판정은 던지지도 문구를 만들지도 않고 이유를 코드로 돌려준다 — 로그 한 줄로 옮기는 것은 서버, 화면 문구로 옮기는 것은 웹의 일이다.
...
**설치된 플러그인은 `version` 이 오를 때만 새 사본을 받는다.** 같은 버전이면 파일을 고쳐도 이미 설치한 쪽은 옛 버전을 계속 읽는다 — 오류도 경고도 없고, **자기가 낡았다는 것을 알 방법도 없다.**
...
- 헤드리스/CI 경로(`claude -p`)는 PAT를 단일 호출 권한으로 주입하고, 플러그인·MCP 적재는 stream-json의 `system/init` 메시지로 확인한다.
...
# 턴 완료 알림 → NERV Ingest 로 전달 — **아직 켜지 않는다.**
# 수신 엔드포인트(/ingest/hooks/notify)는 서버에 없다(4.6 §5.1 Phase 2). 켜면 매 턴 404 가
# exit 0 으로 삼켜져 아무도 모르는 실패가 된다. 엔드포인트가 생기면 이 줄을 풀고
# .nerv/env 에 NERV_AGENT_TYPE=codex 를 둔다(포워더의 X-NERV-Agent 기본값이 claude-code 다).
...
- [Connect Claude Code to tools via MCP — Claude Code Docs](https://code.claude.com/docs/en/mcp) — (2026-08-13 확인) `.mcp.json` 권한·`${VAR}` 확장·비대화형 모드의 사전 로그인/PAT(§3.3·§4).
