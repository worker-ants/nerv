...
>
> 문서 버전 v0.7 · 2026-08-23 · HTML 판: [importer.html](../html/importer.html)
>
...
형식은 미결이다: 데이터 모델 §5.1 의 표시 키 정본은 `<project.key>-<타입>-<base32 6자>`(예: `CLV-T-7QF3K2`)인데 임포터는 `TSK-<hex>` 를 쓴다. 정합에는 시드·웹 라우트·문서가 함께 걸리므로 **별도 결정이 필요하다(2026-08-23 미결)**.
...
| REQ-IMP-020 | WHEN 서로 다른 원본 파일이 같은 표시 ID 를 받으면 THE SYSTEM SHALL 그 항목들을 적재에서 빼고 리포트에 `aborted` 로 남긴다 — 덮어쓰지 않는다(§2.6b) |
