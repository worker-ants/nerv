...
>
> 문서 버전 v0.27 · 2026-08-24 · HTML 판: [screens.html](../html/screens.html)
>
...
- **오른쪽 패널이 열린다**: 머리에 고른 문서의 **이름과 키**, 아래에 이웃을 **역참조 / 레퍼런스**로 갈라 건수와 함께 적는다(레일 §2.4와 같은 순서·같은 어휘 — 두 화면이 관계를 다르게 부르면 사람은 그 둘이 같은 것인지부터 의심한다). 관계 종류(`references`·`refines`…)를 줄마다 적는다.
