...
>
> 문서 버전 v0.75 · 2026-09-05 · HTML 파생본: [screens.html](../html/screens.html)
>
...
>
> v0.74 변경(2026-09-05 — 파생본이 원본과 다른 말을 하고 있었다, 정합성 감사): ① html 판의 §2.4 기준선 서술이 개정 전 판이었다("기준선 없음"도, "그 세트가 담은 문서만"(REQ-API-098)도 없었다) — 마지막 두 커밋이 html 을 버전 줄만 고쳤다. ② html 툴바 목업이 필터 셋을 **화면 머리 줄**에 그려 **같은 파일의 REQ-WEB-140 과 반대**였다. ③ §1.6 커버리지 표는 "이 표에 '없음' 이 생기면 결함이다" 라 적어 두고 `/signup`·`/invite/:token`·`/help`·`/help/:chapter` **넷이 비어 있었다**. ④ 존재한 적 없는 스코프 `policy:edit` 을 게이트 정책의 권한으로 적던 것을 실물(사람 전용 + admin 역할)로 고친다. ⑤ `/p/:proj/coverage` 는 "라우트만 예약" 이라 적혀 있었으나 라우트 파일도 없다.
...
> v0.66 변경(2026-09-04 — 기준선을 만들 문이 없었다): **REQ-WEB-135·136 신설.** §2.4 와이어프레임은 처음부터 `[베이스라인 ▾]` 과 `[현재 세트로 동결…]` 을 그리고 있었는데 화면에 없었고, 그래서 서버에 테이블·엔드포인트 넷·L2 가 다 있는 채로 **실사용 기준선이 0개**였다(실측). 스펙 생성 진입점에서 이미 같은 일을 겪었다(v0.63). 고른 기준선은 `?baseline=` 로 주소에 남아 상세까지 따라가고, 어느 세트로 읽는지를 배지가 말한다.
...
> v0.56 변경(2026-09-01 — 사람 요청): **REQ-WEB-121·122**. 버전 diff 화면을 붙인다 — 서버(EP-SPEC-06)는 2026-08-22 부터 줄 수 있었는데 부르는 쪽이 없었다. 레일 클릭은 직전과의 비교, select 둘은 임의 비교, `?v=` 는 그 판 전문. 요구사항 델타가 먼저 오고 기본은 변경분만이다.
> v0.55 변경(2026-08-31 — 사람 요청): **REQ-WEB-119·120**. 받은 요청의 카드에서 그 문서로 갈 수 있게 한다 — 키를 링크로, 본문은 카드 안에서 펼친다(검토 중인 그 판을 펼칠 때 받아 온다). 질문 카드의 발견은 `?finding=` 으로 리뷰 센터의 그 지적을 편다.
...
| `/settings/gates` | S8 게이트 정책 | **사람 전용**(`HUMAN_ONLY` — 토큰으로는 못 바꾼다) + **admin 역할**. `policy:edit` 이라는 스코프는 **존재한 적 없다**(2026-09-05 정정) | |
...
4. **기준선 선택기** — EP-SPEC-11 목록 + [기준선 생성…](planner·admin — EP-SPEC-12 다이얼로그). 기본 항목은 **`기준선 없음`** 이다. 고르면 `?baseline=` 쿼리로 목록이 **그 세트가 담은 문서만, 그 세트가 핀한 판으로** 렌더된다(spec-workflow §3.6 · REQ-API-098) — 세트 밖의 문서를 함께 보이면 보는 사람은 그 세트가 그것을 담고 있다고 읽는다. 그 동안에는 상태·종류 필터를 그리지 않는다(세트 항목은 전부 승인본이라 거를 것이 없다). 새 라우트 없음 — 뷰 상태 쿼리다.
...
| REQ-WEB-037 | WHEN 대상 문서의 **현재 판을 마지막으로 쓴 뒤** `spec.recheck_requested`가 그 문서 앞으로 발행됐으면 THE SYSTEM SHALL S3 상태 패널에 참조 갱신 배지를 표시하고 **무엇 때문인지**(신호를 만든 참조 스펙 키)를 함께 보인다. 판정은 서버가 하고 화면은 그 값만 읽는다(2026-09-03 정정) |
...
같은 진단이 한 줄 더 있었다. **제목 아래 설명은 대부분 화면이 이미 보여주는 것을 문장으로 옮겨 적은 것**이다 — 스펙 목록의 "한국어로 물어도, 안정 ID로 물어도 찾습니다"는 바로 옆 검색창의 placeholder 와 같은 말이고, 세션의 "누구의 어느 머신이 무엇을 하고 있는지"는 그 아래 표의 열 이름이다. 한 줄이 **21~22px + 여백 7px**(실측)를 먹고 열 화면에 있다.
...
- **버전 목록은 누를 수 있어야 한다**(2026-09-01 — 사람 요청 · REQ-WEB-121·122). 서버는 **처음부터** 이것을 줄 수 있었다 — EP-SPEC-06 이 요구사항 델타와 줄 diff 를 함께 돌려주는데(2026-08-22) 웹이 그 경로를 한 번도 부르지 않았고, 그래서 **테스트도 0건**이었다. 레일의 버전을 누르면 `?diff=vN-1..vN`(가장 흔한 물음이 한 번에 답해진다), 옆의 [이 판 전문]은 `?v=N`. **주소가 진실이라** 공유·북마크·뒤로가기가 그대로 산다. 임의의 두 판은 diff 머리의 select 둘로 고른다 — 모드를 새로 만들지 않고 이미 화면에 있는 `v2 → v3` 를 누를 수 있게 한 것이다. 순서는 **항상 오래된 쪽 → 새 쪽**으로 정규화한다: `added`/`removed` 는 시간 순서가 있어야 뜻이 있고, 뒤집힌 diff 는 "전부 삭제됨" 으로 읽힌다.
...
- **받은 요청에서 문서로 간다**(2026-08-31 — 사람 요청 · REQ-WEB-119·120). 결재하려면 본문을 봐야 하는데 카드에는 제목과 키뿐이었고, **키는 글자였을 뿐**이라 그 문서를 보려면 목록에서 손으로 찾아야 했다 — 다른 탭에서 열고 돌아오는 왕복이 승인 병목(P4)의 한 조각이다. 키를 링크로 만들고, 카드 안에서 본문을 펼친다. **펼칠 때 받아 온다**: 결재 목록에 본문을 싣는 것과 펼친 하나를 받는 것은 다른 비용이다. 그리고 **검토 중인 그 판**을 받는다 — 카드가 보여준 것과 승인되는 것이 같아야 한다는 규약(§2.3 지문 대조)과 같은 이유다. 질문 카드의 발견도 갈 곳이 생긴다: `?finding=<id>` 로 리뷰 센터를 열면 그 지적이 레일에 펴지고, 이미 처분돼 기본 필터에서 빠졌으면 필터를 풀어 찾아 준다(막다른 길을 만들지 않는다 — §1.5).
...
| REQ-WEB-130 | WHEN 스펙 상세가 approved 판을 보이면 THE SYSTEM SHALL **다음 판을 시작하는 진입점**을 두고, 지금 읽는 본문과 그 판의 지문으로 새 draft 를 연다. WHILE 승인본을 보고 있으면 THE SYSTEM SHALL 본문 편집을 막는다(D-02 — 가변 구간은 draft 하나뿐) |
...
- **출처는 링크다.** `context{spec_id,task_id,finding_id}`([4.4](api.md) §1.4d)가 카드에 스펙 키·Task 키로 서고 눌러서 원문으로 간다. Finding 은 안정 키가 없으므로 짧은 id(앞 8자 — 리뷰 센터와 같은 표기)를 단다.
...
**상태 토큰 8종**의 원본은 제안서 HTML 공통 팔레트(`docs/html/assets/style.css`의 CSS 변수)다. Tailwind 테마 토큰으로 1:1 이식하고, 라이트/다크는 CSS 변수 값 교체로 처리한다. **전환은 `<html data-theme>` 이 정한다**(2026-08-28 개정 — 사람 요청 §1.7a). 이전 판은 "`data-theme` 아님 — `prefers-color-scheme`"이었고 그때는 사람이 고를 수단이 없었다. 이제 `prefers-color-scheme` 는 **고른 값이 없을 때의 기본**으로 남고(그 판정은 스크립트가 해서 `data-theme` 에 적는다), 미디어 쿼리에는 스크립트가 붙기 전 한 프레임을 위한 배경색 두 줄만 둔다.
...
- [3.4 에이전트 연동 설계](../03-proposal/agent-integration.md) — `nerv_*` 도구 카탈로그(§2.3)·하트비트 역채널(§2.4)·토큰 스코프(§6.1)
