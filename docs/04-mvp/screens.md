...
>
> 문서 버전 v0.68 · 2026-09-05 · HTML 판: [screens.html](../html/screens.html)
>
...
| REQ-WEB-138 | WHEN 스펙 목록에서 상태 필터를 고르면 THE SYSTEM SHALL 그 값을 `?status=` 로 주소에 남기고(다른 뷰 상태를 지우지 않는다), 목록에 **그 상태의 문서와 그 조상만** 남긴다 — 조상은 자리를 지키러 온 것이라 아래에 걸린 것이 하나도 없는 가지는 남기지 않는다. WHILE 상태 필터가 걸린 동안 THE SYSTEM SHALL 걸린 노드를 접힌 가지에 숨기지 않고, `전체 M` 은 필터와 무관한 프로젝트의 문서 수로 유지한다 — 총계가 필터를 따라 움직이면 두 수를 함께 적는 뜻이 없어진다 |
