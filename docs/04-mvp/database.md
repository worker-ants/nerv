...
>
> v0.42 변경(2026-09-10 — 길을 내고도 그 길을 지나가는 데이터가 없었다, 사람 지시): **새 요구사항 없음 — 개발 시드에 `evidence` 7건.** 시드는 리뷰·활동·질문을 다 심으면서 **증적만 0건**이었다. 그래서 작업 상세의 증적 카드가 개발 환경에서 늘 "아직 없습니다" 였고, 그 카드에 붙은 링크([4.5](screens.md) REQ-WEB-159~162 · [4.4](api.md) REQ-API-157)는 **시드로는 한 번도 그려진 적이 없다** — L3 도 스크린샷도 그 길을 보지 못했다. 종류 여섯을 전부 쓴다: 링크가 되는 넷(`pr`·`commit`·`code_path`·`review`)과 매뉴얼로 가는 하나(`user_guide`), 그리고 **링크가 되지 않는 하나**(`test` — 저장소마다 모양이 달라 짐작하지 않는다. 평문으로 남는 자리가 하나는 있어야 그 규칙이 눈에 보인다). 하나는 `repo` 를 달아 **다른 저장소**를 가리킨다 — 세션 복원은 백엔드의 일이고, 그 경로가 화면에 실제로 그려지는 유일한 자리다. 하나는 요구사항에도 매달아 S3 요구사항 탭의 증적 수가 0 이 아니게 한다(FR-13).
>
...
**시드 계정은 확인된 상태다(2026-09-22 정정).** 가입 이메일 인증을 강제하기로 한 뒤로 `user.email_verified` 가 거짓인 계정은 **로그인할 수 없다.** 마이그레이션 0028 이 그 시점의 기존 계정을 백필하지만 시드는 TRUNCATE 후 재삽입이라 그 뒤에 다시 심긴다 — 그래서 시드가 직접 참으로 세운다. 이 한 줄이 빠지면 개발 환경 전체가 로그인 화면에서 멈춘다(실측: E2E 전역 셋업이 거기서 30초를 기다리다 죽었다). 시드는 화면을 열어 보라고 있는 것이므로, **심어 놓고 못 들어가는 계정은 심지 않은 것과 같다.**

**대기 중인 결재를 심는다(2026-09-24 정정).** `approval` 은 **한 행뿐이었고 그것도 이미 결정이 끝난 면제**였다 — 대기 중인 결재는 **한 건도 없었다.** 그래서 S7 받은 요청은 L3 에서도 디자인 확인용 스크린샷에서도 **늘 빈 상태**로만 찍혔고, 2026-09-22 에 들어온 일괄 승인·거절([4.5](screens.md) REQ-WEB-181~183 · [4.4](api.md) REQ-API-162~164)은 L1·L2 만 덮은 채 그 길을 한 번도 지나가지 못했다. 아래 Activity·증적·관계 그래프에서 세 번 겪은 것과 **같은 형태의 네 번째**다. 그래서 **검토 중인 개정판 넷**과 그 위의 **결재 슬롯 다섯**을 심는다 — 섞여 있어야 화면이 무언가를 말한다: **저위험 셋**(정족수 1 · 일괄로 지나간다)과 **T3 하나**(슬롯 2 · 일괄에서 빠지고 `0/2 승인` 배지가 선다). 저위험이 하나뿐이면 일괄이 한 건짜리 조작이 되어 아무것도 시연하지 못하고, T3 가 없으면 **빠지는 길**이 시드에 없다. 슬롯의 모양은 실물이 세우는 것 그대로다 — 슬롯 1 은 기본 큐(`assignee_role` NULL), 슬롯 2 는 문서 타입의 직군(`SPEC_APPROVER_ROLES` — `design` → `designer`). **작성자도 요청자도 지민이 아니다.** 지민은 L3·개발 환경이 들어오는 신원인데 지시자≠승인자는 세 축이라([3.5](../03-proposal/spec-workflow.md) §2.3 — 요청자·작성자·작성 세션 소유자), 셋 중 하나라도 지민이면 `can_approve` 가 거짓이 되어 **카드는 있는데 아무것도 누를 수 없는 화면**이 된다 — 그건 빈 화면보다 나쁘다. 하나는 **세 시간째** 기다린다(한 시간을 넘긴 카드의 경고색 — [4.5](screens.md) REQ-WEB-024 — 도 그려진 적이 없었다). `current_version_id` 는 승인본에 그대로 둔다: 실물도 기존 스펙에 새 초안을 만들 때 현재 버전을 옮기지 않으므로, 트리와 상세는 승인본을 계속 가리키고 검토 중인 개정판은 받은 요청에서만 보인다.

**관계 그래프가 그릴 것을 심는다(2026-09-22 정정).** 스펙이 셋뿐이고 `spec_relation` 이 **0건**이던 동안, 스펙 목록의 관계 그래프 탭은 언제나 "관계가 아직 없습니다" 였다 — 캔버스가 아예 마운트되지 않아, **밀도가 유일한 설계 문제인 그 화면이**([4.5](screens.md) §2.4a) **디자인 확인용 스크린샷에 한 번도 들어간 적이 없다.** 아래 Activity·증적에서 두 번 겪은 것과 같은 형태이고 귀결도 같다: 화면에 길을 내고도 그 길을 지나가는 데이터가 없으면 L3 도 스크린샷도 그 길을 보지 못한다. 그래서 트리를 **문서 20 · 영역 4 · 종류 6 · 관계 28**로 넓힌다 — 셋 다 그림이 말해야 하는 것이다: 영역이 하나뿐이면 상자끼리 밀어내는 정리 패스(REQ-WEB-174·175)가 할 일이 없고, 종류가 둘뿐이면 범례가 두 줄로 끝나며(REQ-WEB-176), 피참조 수가 고르면 차수 기반 크기가 아무 차이도 내지 않는다. 그래서 영역을 **건너는** 간선을 28 중 17로 두고 피참조 수를 0~6으로 갈랐다. **영역(`area`) 넷만 본문이 없다** — 임포터의 골격 배치와 같은 모양이고([4.7](importer.md)), 나머지는 승인본 하나씩을 갖는다(트리는 차는데 누르면 빈 문서인 상태를 만들지 않는다).

**증적도 심는다(2026-09-10 정정).** 이 표만 비어 있었다 — 시드가 리뷰·활동·질문을 다 심으면서 `evidence` 는 0건이라, 작업 상세의 증적 카드가 개발 환경에서 늘 "아직 없습니다" 였다. Activity 에서 겪은 것과 같은 형태이고 귀결은 하나 더 나쁘다: **화면에 길을 내고도 그 길을 지나가는 데이터가 없으면 L3 도 스크린샷도 그 길을 보지 못한다**(증적 링크 — [4.5](screens.md) REQ-WEB-159~162). 종류 **여섯을 전부** 쓰고, 그중 하나는 일부러 **링크가 되지 않는 것**(`test`)이다 — 평문으로 남는 자리가 하나는 있어야 "짐작해서 링크를 만들지 않는다" 는 규칙이 눈에 보인다. 하나는 `repo` 를 달아 다른 저장소를 가리키고(그 경로가 화면에 그려지는 유일한 자리다 · [4.4](api.md) REQ-API-157), 하나는 요구사항에도 매단다(FR-13 — 요구사항 탭의 증적 수).
...
이 한 벌로 S1 홈(질문 배지 1), S3 스펙 상세(SPC-CWC-007 v4 approved + REQ-CWC-031), S4 작업 보드(in_progress 2 · blocked 1), S5 세션 모니터(active 2 · awaiting_input 1), **S6 리뷰 센터(열린 발견 3 — severity 3종 · 브랜치 2 · 면제 1 · 라운드 체인 1→2로 dedup 표기 1건)**, **S7 받은 요청(대기 결재 — 저위험 3 · T3 1(슬롯 2) · 질문 카드 1)**이 전부 비어 있지 않게 뜬다 — 화면 개발([4.5 화면 명세](screens.md))의 로딩/빈/에러 3상태 중 "데이터 있음" 경로를 즉시 확인할 수 있다.
