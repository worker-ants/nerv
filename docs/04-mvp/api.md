...
>
> 문서 버전 v1.65 · 2026-09-25 · HTML 파생본: [api.html](../html/api.html)
>
...
| REQ-API-140 | WHEN 티어가 요구하는 승인자 수가 2 이상인 스펙이 제출되면 THE SYSTEM SHALL 그 수만큼 결재 슬롯을 만들고(둘째부터는 문서 타입의 직군 큐, 그 역할의 멤버가 없으면 기본 큐로 강등), WHEN 결정이 오면 THE SYSTEM SHALL 이번 라운드에 `approve` 를 낸 **서로 다른 사용자** 수가 필요 수에 닿을 때만 문서를 전이하고 응답에 `quorum` 을 싣는다. WHERE 승인 가능한 사람이 필요 수보다 적으면 THE SYSTEM SHALL 그 수만큼만 세우고 `quorum_relaxed` 를 남긴다 | T3 제출은 슬롯 2 · 첫 승인은 `in_review` 유지와 `satisfied:false` · 둘째 승인에 approved 와 `spec.approved` 1건 · 같은 사람의 둘째 승인은 403 `already_approved` · 동시 승인에도 전이 1회 · 거절 뒤 재제출은 0부터 |
