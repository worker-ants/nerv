...
>
> 문서 버전 v1.59 · 2026-09-24 · HTML 파생본: [api.html](../html/api.html)
>
...
| EP-SPEC-08 | `PUT /api/v1/projects/{proj}/specs/{spec}/draft` | EP-SPEC-07과 동일(`spec:draft`) | `SpecDraftUpsertInput`(body_markdown — **비신뢰 경계로 통째로 감싸여 오면 400 `wrapped_body`**(REQ-API-153): 포장은 본문이 아니다, **base_hash**, change_summary, **relations**(선언 관계 — 2026-09-05 배선 · REQ-API-043. 그전에는 REST 만 이것을 읽지 않아 관계가 성공 응답과 함께 버려졌다)) | `SpecDraftResult`(version, 델타 요약, 검증 경고, `web_url`) | 새 draft 버전 생성 시 `spec.draft_created`, **같은 draft 재저장은 `spec.draft_updated`**(2026-08-29 개정 — 예전에는 "이벤트 없음(리스 갱신만)"이었다. 에이전트가 스펙을 쓰는 방식이 대부분 이 경로라, 본문이 바뀌어도 화면이 새로고침 전에는 알 수 없었다. 저장 빈도는 자동 저장 주기(60초)라 방송이 넘치지 않는다) |
