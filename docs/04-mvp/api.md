...
> v1.73 변경(2026-09-26 — 일괄 결정 51건이 "스키마와 맞지 않습니다" 한 줄로만 돌아왔다, 사람 요청): **REQ-API-191 신설 · §1.4 한 칸 · EP-APR-06 요청 칸.** 상한은 zod 에 있었고 그 상한을 말하는 문구(`error.approval.bulk_limit`)도 카탈로그에 있었는데, 요청 본문 검사(`parseBody`)가 어떤 위반이든 같은 한 줄로 답해서 둘이 이어지지 않았다. 이제 어긴 규칙에 카탈로그 키가 달려 있으면 그 문구로 답한다 — 상태와 `details.issues` 는 그대로라 기계가 읽는 쪽은 바뀌지 않는다. 곁들여 **EP-APR-06 이 201 로 답하던 것**을 REQ-API-162 대로 200 으로 고쳤다 — Nest 는 `@HttpCode` 가 없는 POST 에 201 을 주고, 새 L2 가 그것을 찾았다.

> v1.72 변경(2026-09-26 — `code:` glob 이 아무 말 없이 사라졌다): **EP-IMP-02 한 칸.** `evidence[]` 항목이 `stale` 을 싣는다 — 원본 저장소를 보는 CLI 가 경로 실존 검사로 판정한다([4.7](importer.md) REQ-IMP-032).

> v1.71 변경(2026-09-26 — 검증 서명은 그 문장에 대한 것이다, **사람 결정** · 구현 축 상태도 검토): **REQ-API-190 신설 · REQ-API-097 한 구절 · EP-REQ-01 · 02 · EP-COV-01 칸.** 승인이 요구사항 문장을 바꾸면 그 시각을 남기고(`statement_changed_at` · `0033`), 그보다 앞선 서명은 지금 문장을 보증하지 않는다 — 검증이 풀리고 `reverify_required` 가 선다. 해제는 qa·admin 의 서명(EP-REQ-03)이다. 곁에 요구사항 상세의 증적이 목록과 같은 범위(파생 Task 의 것 포함)를 보이게 고쳤다.

> v1.70 변경(2026-09-26 — 구현 축의 그림이 두 벌이었다, **사람 결정** · 구현 축 상태도 검토): **REQ-API-097 정정.** 파생 규칙의 "그 밖에는 `unimplemented`" 가 코드와 달랐다 — done 이 있는데 아직 전부가 아니거나 증적이 없으면 `in_progress` 다. 낡은 증적(`stale`)은 세지 않는다고 적고, `verified` 칸의 옛 단서("지금 스키마가 담지 못한다" — 2026-09-07 부터 파생한다)를 걷었다([3.5](../03-proposal/spec-workflow.md) §1.3).

> v1.69 변경(2026-09-26 — 재시도 신호, **사람 결정** · 동적 강화 두 신호 검토): **REQ-API-189 신설 · EP-APR-01 한 칸 · §2.1a 주석 한 줄.** 제출 판정이 에이전트의 `e2e-fail-3x` 신고를 센다 — 그 스펙을 가리키거나 · 그 스펙의 파생 작업의 것이거나 · 제출한 세션의 것, 사람이 그 스펙을 마지막으로 승인한 뒤. 가장 최근의 신고 하나를 `gate_evidence` 로 싣고 카드가 읽는다. 롤백 이력 신호는 걷었고, 신호가 여럿이어도 한 단계이며, 승인 때의 기록은 제출 때의 판정을 옮겨 적는다([3.5](../03-proposal/spec-workflow.md) §2.4).

> v1.68 변경(2026-09-26 — 티어가 이유 없이 왔다, **사람 결정** · 동적 강화 두 신호 검토): **REQ-API-188 신설 · EP-APR-01 한 칸 · §2.1a 한 문장.** 카드가 "게이트 티어와 산출 근거" 를 보여야 하는데([3.5](../03-proposal/spec-workflow.md) §6.4) 근거는 제출 응답의 한 줄 문자열뿐이었고 저장되지 않아, 카드는 배지만 그렸다 — 첫 승인 버전 가산으로 T2 가 된 문서도 사람에게는 이유 없이 T2 였다. 제출 이벤트와 슬롯마다의 결재 요청 이벤트가 4축 합계 · 축별 점수 · 티어를 올린 신호를 싣고, 받은 요청 카드가 그것을 읽는다. 곁에 §2.1a 의 "`spec_gate.*` 3키" 를 2키로 고쳤다(이의제기 창 키는 2026-09-02 에 걷혔다).

> v1.67 변경(2026-09-26 — 보낸 결정을 바로잡는 길, **사람 결정** · 결정 철회 검토): **EP-APR-01 한 칸 · 결정과 전이 단락 뒤 한 단락 · §2.1a 주석 한 줄.** ① 결정 철회 API 는 만들지 않는다 — 결정은 거두지 않고, 바로잡는 것은 새 결정이다([4.5](screens.md) REQ-WEB-239 · 재검토 트리거도 거기). ② 질문 카드가 `session_id` 를 싣는다 — 답을 보낸 토스트가 그 세션으로 데려간다. ③ `dynamic_escalation` 주석이 서버가 세지 않는 신호 둘을 말하고 있었다 — 지금 세는 것은 첫 승인 버전 하나다([3.5](../03-proposal/spec-workflow.md) §2.4).

> v1.66 변경(2026-09-26 — T3 가 한 사람의 승인으로 확정됐다, 결정 철회 검토에서 실측): **REQ-API-133 · 134 · 140 개정.** ① T3 의 두 슬롯 중 하나만 거절되면 다른 슬롯은 결정 없이 남고, 다시 제출하면 그 슬롯을 재사용하는데 요청 시각이 옛 라운드의 것이라 정족수가 빼고 셌다 — 필요 수가 1 이 되어 **새 슬롯 하나의 승인으로 문서가 확정**됐다(L2 재현 · 일괄 승인 자격도 같은 이유로 열렸다). 재사용하는 순간 이번 제출의 요청으로 다시 찍는다. ② 서로 다른 사용자를 지정자(`assignee_user_id`)가 아니라 **결정한 사람**(`decided_by_user_id`)으로 센다 — admin 이 남에게 지정된 카드를 누르면 지정자가 승인한 것으로 세였다. ③ 그 거절 뒤 요청 세션이 결정 없이 남은 형제 슬롯에 붙잡혀 `awaiting_input` 에 머물렀다 — 검토 중이 아닌 문서의 남은 슬롯은 대기 사유가 아니다. ④ 하트비트의 `decided_by` 가 지정자의 이름이었다 — 누른 사람이다.

> v1.65 변경(2026-09-25 — 비밀번호를 잊은 사람에게 길이 없었다, **사람 결정 D10** · UI/UX 검토 SET-13): **REQ-API-187 신설 · §1.3 한 항목 · §1.8 한 줄.** 내 계정(v1.64)은 지금 비밀번호를 아는 사람의 문이었다. 인증 스택은 재설정 문을 열어 두고 있었지만 보낼 곳(`sendResetPassword`)이 없어 늘 400 이었다 — 메일이 켜진 배치에서 재설정 메일을 줄 세운다(아웃박스의 셋째 종류 `reset_password` 가 처음 쓰인다 · [4.3](database.md) §2.17). 응답은 계정이 있든 없든 같고, 링크는 API 를 거쳐 화면으로 가며(만료를 먼저 알린다), 토큰은 한 번 · 60분(`PASSWORD_RESET_TTL_MINUTES`)이고, 정하면 **모든** 세션이 끊기고 이메일이 확인된 것이 된다. 메일이 꺼진 배치는 켜지 않는다(`RESET_PASSWORD_DISABLED`). 곁들여 **가입 확인 메일이 가입한 화면의 언어로** 나간다 — 화면이 인증 요청에 언어를 싣지 않아 언제나 기본 언어였다([4.5](screens.md) REQ-WEB-231).
...
>
> v1.61 변경(2026-09-24 — 누가 지적했고 무엇을 손봐야 하는지 말하지 않았다, UI/UX 검토 P09c): **REQ-API-183 신설 · EP-SPEC-01 · EP-CMT-01 응답 칸.** 코멘트 목록은 작성자의 id 만 실어 화면은 누가 무엇을 지적했는지 말할 수 없었다 — 이름·(에이전트면) 기계·해소한 사람을 싣는다. 트리의 노드에는 명세의 행(§2.4)이 약속한 최근 갱신과 열린 코멘트 수가 없었다 — 싣는다(그래프 EP-SPEC-19 도 같은 노드다)([4.5](screens.md) REQ-WEB-216).
>
> v1.60 변경(2026-09-24 — 보는 버전과 기다리는 결재를 말하지 않았다, UI/UX 검토 P09a): **REQ-API-182 신설 · EP-SPEC-03·07·08 응답 칸.** 승인본 위에 초안을 쓴 에이전트가 건넨 `web_url` 은 버전을 싣지 않아 누른 사람은 승인본에 섰다 — 기본이 그 초안을 열지 않을 때(승인본이 있고 그것과 다를 때)만 `?v=` 를 싣는다. 문서 조회는 **그 버전을 기다리는 결재**(`pending_approval_id`)를 싣는다 — 검토 중인 문서를 연 사람을 받은 요청의 그 카드로 데려가려면 카드가 무엇인지 알아야 한다([4.5](screens.md) REQ-WEB-214).
>
> v1.59 변경(2026-09-24 — 활동과 알림이 무엇에 일어났는지 말하지 않았다): **REQ-API-181 신설 · EP-EVT-01·EP-NTF-01 응답 칸.** 이벤트 피드는 대상을 `(subject_type, subject_id)` 로만 실어 홈과 개요가 "초안 수정 · 관리자 · 16일 전" 을 여덟 줄 늘어놓았고, 알림 목록은 스펙·작업 주체만 조인해서 가장 무거운 두 알림 — 결재 요청과 에이전트 질문 — 이 키도 제목도 없이 "승인 요청" 만 반복했다. 대상이 한 단계 건너(결재 → 스펙 버전·플랜·발견 · 질문 → 작업·스펙 · 클레임·리뷰 세션 → 작업)에 있어도 따라가 스펙·작업·발견·질문·리뷰 브랜치·세션의 이름을 **두 목록이 같은 조각으로** 싣는다(`modules/event/event-subject.ts` · [4.5](screens.md) REQ-WEB-210).
>
> v1.58 변경(2026-09-24 — 화면이 서로를 글자로만 가리켰다): **REQ-API-180 신설 · EP-TASK-01·04 · EP-REV-03·07 응답 칸 · EP-REV-03 `branch`.** 작업·세션·리뷰를 서로 잇는 링크에 필요한 값이 응답에 없었다 — 보드는 누가 실행하는지 몰랐고(세션 id 뿐), 작업 상세는 담당을 id 로만 줬으며, 발견은 나온 작업도 올린 작업도 싣지 않았다(화면은 `promoted_task_key` 로 "올림" 을 그리려 했지만 값이 오지 않아 단추가 늘 켜져 있었다). 두 번째 승격은 id 만 돌려줘 어느 작업인지 말하지 못했다. 발견 목록은 작업 상세의 리뷰 줄·게이트 표가 곧장 올 수 있게 **한 브랜치로** 거른다 — 목록과 facet 이 같은 규칙이다([4.5](screens.md) REQ-WEB-209).
>
> v1.57 변경(2026-09-24 — 세션 카드가 한 일과 기다리는 것을 말하지 않았다): **REQ-API-179 신설 · EP-SES-01 응답 칸.** 세션 목록이 **활성 클레임만** 조인해, 끝난·유령 세션은 전부 "클레임한 작업 없음" 이었고 무엇이 회수됐는지([4.5](screens.md) REQ-WEB-020) 알 수 없었다. 입력을 기다리는 세션도 **무엇을** 기다리는지 싣지 않아 사람은 누가 무엇에 답해야 하는지 몰랐다 — 활성 클레임이 없으면 마지막 클레임(작업 키·제목·끝난 방식)을, 입력 대기면 열린 질문(없으면 이 세션이 올린 결정 전 결재)을 싣는다([4.5](screens.md) REQ-WEB-208).
>
...
>
> v1.54 변경(2026-09-24 — 보드가 임포트 자리표시자를 찬 것으로 셌다, UI/UX 검토 P04): **REQ-API-175 신설 · EP-TASK-01 응답 · EP-TASK-03·05 권한 칸.** 보드 목록(EP-TASK-01)의 `delegation_complete` 가 "네 칸이 NULL 이 아닌가" 만 봐서 임포트 자리표시자(`import.delegation_missing`)를 찬 것으로 셌다 — 승격·전이의 판정(`isDelegationFilled` · REQ-API-131)은 그것을 빈 것으로 본다. 그래서 되돌린 임포트 작업이 backlog 에서 [채우기] 없이 갇혔다. 목록도 같은 판정을 쓴다(자리표시자 목록은 `@nerv/schema` 한 곳 · SQL 조각 `filledSql`). 곁들여 EP-TASK-03·05 의 역할 목록을 `@nerv/schema` 의 상수(`TASK_CREATE_ROLES`·`TASK_EDIT_ROLES`)로 올렸다 — 서버 가드와 화면이 같은 값을 본다([4.5](screens.md) REQ-WEB-203). 값은 그대로다.
>
...
>
> v1.44 변경(2026-09-24 — 같은 목록을 두 표면이 각자 물었다): **REQ-API-167 신설 · EP-APR-05 전표.** EP-APR-05 가 자기 질의를 따로 들고 있어 **같은 질문에 다른 답**을 냈다 — 실측 2026-09-24(결재 3 + 질문 2 를 심고 나란히 호출): 프로젝트 표면 **3건(결재만)** · 전역 표면 **5건(결재+질문)**, 보관한 뒤로는 프로젝트 **3건** · 전역 **0건**. 전표는 처음부터 "그 프로젝트의 받은 요청(**결재 + 질문**)" 이라 적었고, 보관 필터는 2026-08-27 에 전역 쪽에서 고쳤는데 이 표면에는 닿지 않았다. 열도 달라 `waiting_seconds`(카드의 필수 표기)·`project_slug`(링크)·`spec_key`·`spec_title` 이 없었다 — **같은 컴포넌트가 그릴 수 없는 모양**이었다. 판정이 두 벌이면 언젠가 한쪽만 고쳐진다(D-05) — 셋 다 그렇게 벌어졌다. 질의를 없애고 **전역 표면을 프로젝트로 좁혀 부른다**: 이 표면이 따로 있는 이유는 주소뿐이고(프로젝트 안에서 읽는 곳), 그래서 `state`·`cursor`·`limit` 도 같게 읽는다. 서비스 질의 하나가 사라졌다.
>
> v1.43 변경(2026-09-24 — 전표가 `Page` 라 적는 동안 서비스는 맨 배열이었다, **사람 결정**): **REQ-API-166 신설 · §1.6 한 줄 · EP-APR-01 전표.** 받은 요청이 **100건에서 말없이 잘렸고, 그 상한이 오래 기다린 쪽을 잘랐다**. 질의가 `requested_at DESC LIMIT 100` 으로 가장 최근 100건을 집고 화면이 그것을 다시 오래된 순으로 세웠다 — 실측 2026-09-24(120건 · 0~119시간 전): **가장 오래 기다린 20건이 목록에서 통째로 빠졌다.** 화면이 존재하는 이유가(REQ-WEB-024 — 오래 기다린 것이 위로) 상한에 정확히 뒤집혀 있었고, 카드가 없으니 배지도 그것을 세지 않았다. 그리고 이 자리는 **전표가 이미 `Page<ApprovalCard>` 라 적고 있었다** — 2026-09-06 에 "`cursor` 는 읽지 않는다 · 100건에서 잘린다" 고 *정정해 둔* 채였다. REQ-API-120 이 이벤트 피드에서 잡은 것과 같은 형태의 드리프트다. 봉투는 `{items, next_cursor, total}` 이고, **`total` 이 하나 더 있는 이유**가 §1.6 에 들어간다: 쪽을 나누면 "목록 길이 = 전체 수" 가 깨지는데 받은 요청은 그 수를 화면 세 자리가 쓴다(헤더 배지 · 홈의 인사 · "전체 N건"). 출처를 둘로 두면 배지와 목록이 갈라지고, 그러면 지울 수 없는 숫자가 남는다([4.5](screens.md) REQ-WEB-035). 대기는 결재·질문 **두 소스를 같은 정렬 키로** 병합한다([4.5](screens.md) REQ-WEB-185).
>
> v1.42 변경(2026-09-24 — 읽는 곳 없는 값을 카드마다 계산하고 있었다): **새 요구사항 없음 — EP-APR-01 응답의 열 구성.** ① 결정에 남긴 말(`comment_md`)을 카드에 싣는다 — 거절 사유는 필수인데 목록이 그 문장을 나르지 않아 화면이 그릴 수 없었다([4.5](screens.md) REQ-WEB-133 개정). ② `state=decided` 에서는 `can_approve`·`can_approve_reason`·`can_bulk_approve`·`bulk_block_reason`·`content_hash` 를 **싣지 않는다**. 그 넷은 "내가 이것을 누를 수 있는가" 에 답하는 값이고 처리됨 카드에는 누를 것이 없는데(REQ-WEB-133), 카드마다 상관 서브쿼리를 넷 세우고 그중 하나는 멤버십을 훑는다. **빼는 것이지 `false` 로 채우는 것이 아니다** — 거짓 값은 "누를 수 없다" 라는 판정으로 읽히고 화면의 폴백이 그것을 그대로 믿는다. 정족수 둘은 남긴다: T3 를 하나 승인하고 둘째를 기다리는 중이라면 그 카드의 `1/2 승인` 은 처리됨에서도 사실이다.
>
...
>
> v1.38 변경(2026-09-22 — 가입은 이메일 확인까지다, **사람 결정**): **새 요구사항 없음 · §1.3 인증 경로 한 항목.** 가입하면 확인 메일이 줄을 서고, **확인 전 로그인은 403 `EMAIL_NOT_VERIFIED`** 다. 확인 링크에는 **주소 둘**이 들어간다 — 토큰을 확인하는 것은 API 이고 사람이 되돌아가는 곳은 화면이라, `callbackURL` 을 비우면 확인은 되는데 사람은 API 호스트의 빈 페이지에 떨어진다. 다시 받는 길(`/send-verification-email`)은 로그인과 같은 한도를 쓴다. **강제 여부는 SMTP 에서 유도한다** — 강제는 메일을 보낼 수 있을 때만 성립하기 때문이다.
>
> v1.37 변경(2026-09-22 — 초대 메일이 자동으로 나간다, **사람 결정**): **새 요구사항 없음 · §2.1b 메일 행 개정 · EP-INV-01 응답에 `queued`.** 그전에는 admin 이 링크를 **손으로 날랐다** — 토큰 원문이 생성 응답에 한 번만 나오고 화면이 [복사]를 주는 것이 전부였다. 이제 초대 행과 메일이 **한 트랜잭션**에 들어가고([4.3](database.md) §2.17) 워커가 보낸다. **토큰은 응답에 그대로 남긴다** — 메일이 꺼졌거나 막힌 배치에서도 길이 하나는 있어야 한다: 없는 기능을 만들자고 있는 기능을 걷지 않는다. 응답의 `queued` 가 그 둘을 가른다(화면이 "보냈습니다" 와 "전달하세요" 를 같은 문구로 덮으면 한쪽은 반드시 거짓이다). 목록에는 `last_sent_at` 이 선다 — **안 보낸 것과 보냈는데 안 온 것은 다른 문제**이고, 가르지 못하면 admin 이 같은 초대를 세 번 만든다.
>
> v1.36 변경(2026-09-22 — 거절만 하고 길을 주지 않았다, 사람 물음): **REQ-API-161 신설 · REQ-API-021 개정.** "플러그인으로 스펙 메타를 못 바꾸는 이유가 있느냐" 는 물음에서 나왔다. 이유는 있고 §2.2 가 적어 둔 그대로다(트리는 거버넌스 축이고 메타는 버전 이력을 만들지 않는다). **빠져 있던 것은 거절 뒤의 길이다** — `meta_change_not_allowed` 응답에 `kind`·`fields`·`endpoint` 는 있는데 **그 문서가 어디 있는지가 없어서**, 에이전트가 사람에게 청하려 해도 줄 주소가 없었다. 그 문서의 웹 주소(`web_url` — 같은 도구의 성공 응답이 이미 주는 값)를 함께 싣는다. 문구도 고쳤다: "EP-SPEC-15 입니다" 만으로는 **사람이 웹에서 하는 일**이라는 것이 읽히지 않는다. 곁들여 스킬이 그 경계를 말하게 했다([4.6](plugin.md) v0.71 — 지금까지 스킬은 이 제약을 한 줄도 적지 않았다).
>
> v1.35 변경(2026-09-21 — 발급 응답이 자기가 어느 프로젝트의 것인지 말하지 않았다, 사람 보고): **REQ-API-160 신설 · EP-TOK-02·04 응답 열.** 토큰은 처음부터 프로젝트 하나에 묶이는데([4.3](database.md) `api_token.project_id`) **발급 응답에 그 값이 없었다** — `{tokenId, token, prefix, scopes}` 뿐이다. 원문은 이 응답에서 **한 번만** 나가고 그 뒤로 어디에도 없으므로, 그 한 번에 "무엇에 쓰는 것인지" 가 함께 있지 않으면 받은 사람이 들고 나가는 것은 문자열 하나다 — 나중에 목록과 대조하려 해도 거기에도 원문이 없어 접두 8자뿐이다. 응답에 `project{slug,name}`·`name`·`expires_at` 을 더한다(권한은 이미 있었다). 곁들여 **EP-TOK-04 에 `project_name`·`created_at`** 을 더했다 — admin 의 조직 전체 표가 `project_slug` 만으로는 사람 말로 읽히지 않는다. **질의 인자는 그대로 없다**(2026-09-06 정정 그대로 · 거르기는 화면이 한다): 없는 인자를 보내면 조용히 무시되고 그때 목록은 거른 것처럼 보인다([4.5](screens.md) REQ-WEB-167·168).
>
> v1.34 변경(2026-09-20 — 쿠키로 오는 쓰기를 서버가 대조한다, 사람 확정): **새 REQ-API 없음 — §1.3b 의 한 줄이 계약이 됐다**(행동 요구는 [4.2](codebase.md) REQ-CB-043). v1.33 이 "서버가 그 표면에서도 `Origin` 을 대조할 것인가는 아직 결정하지 않았다" 고 적어 둔 자리다. 이제 `/api/auth/*` 와 `/api/v1` 이 **같은 목록으로** 판정하고, 쿠키로 인증하는 상태 변경 요청은 `Origin` 이 없어도 `403` 이다. 면제는 PAT 요청과 읽기 둘이다 — 브라우저 밖에서 쓰기를 부르는 소비자는 PAT 를 쓴다(§1.3 의 2경로 그대로다).
>
> v1.33 변경(2026-09-20 — CORS 규약을 적는다, 사람 지시): **§1.3b 신설 · 새 REQ-API 없음**(행동 요구는 [4.2](codebase.md) REQ-CB-041·042 가 갖는다). v1.32 가 "CORS 규약은 이 문서에 아직 없다 — [4.1](scope.md) §2.3 의 2단계다" 라고 적어 둔 자리를 채운다. 계약은 셋이다 — ① 허용 오리진은 `NERV_WEB_URL` + `NERV_TRUSTED_ORIGINS` 의 **한 목록**이고 CORS·better-auth·`/mcp` 가드가 같은 출처에서 읽는다 ② `Origin` 없는 요청(에이전트·CLI)은 그대로 통과하고, 목록 밖 오리진에는 **헤더를 붙이지 않을 뿐 오류를 내지 않는다** ③ 화면이 싣는 요청 헤더 여섯과 읽어야 하는 응답 헤더 둘(`Idempotency-Replayed`·`Retry-After`)이 목록에 있다 — 노출하지 않으면 브라우저 JS 에서 보이지 않는다. **엔드포인트 전표는 한 줄도 바뀌지 않는다**: 같은 오리진에서는 프리플라이트가 일어나지 않으므로 지금 배치의 응답은 그대로다.
>
...
>
> v1.30 변경(2026-09-10 — 커서가 같은 밀리초 안의 행을 잃었다, 실측): **새 요구사항 없음 — §1.6 에 정밀도 한 줄.** REQ-API-124 는 "커서에 담기는 것은 정렬 키 *전부* 와 id" 라 적었는데, **담는 값의 정밀도**는 적지 않았다. 작업 목록이 `new Date(String(updated_at)).toISOString()` 로 시각을 실어 Postgres 의 µs 를 ms 로 잘랐고, 같은 ms 안에서 µs 만 다른 행은 `<` 도 `=` 도 아니어서 쪽에서 사라졌다(실측: 다섯 건 중 **둘만** 나왔다 · 전체 L2 에서는 7회 중 1회만 드러나던 간헐 실패였다). `updated_at` 기본값이 `now()`(트랜잭션 시각)라 한 트랜잭션 안은 값이 정확히 같아 무사했고, **임포터처럼 트랜잭션을 빠르게 이어 만든 행들**만 그 창에 빠졌다 — 그래서 오래 보이지 않았다. 곁들여 같은 목록이 커서 세 자리를 **캐스팅 앞에서 거르지 않아** 낡은 커서가 22007·22P02 로 500 이 될 수 있었다(이벤트·알림·세션은 이미 걸렀다 — 이 목록만 규칙 밖이었다). `cursor-seek.spec.ts` 가 이제 작업 목록도 센다.
>
> v1.29 변경(2026-09-10 — 링크가 404 로 끝났다, 사람 결정): **REQ-API-158 신설 · EP-PRJ-03·04 개정 · 마이그레이션 0026.** 증적의 커밋·코드 경로를 링크로 만들면서([4.5](screens.md) REQ-WEB-159) 여태 **GitHub 모양 하나**만 만들었다 — 자체 호스팅 GitLab 은 경로에 `/-/` 가 끼므로 그 링크가 404 로 끝난다. 링크가 생기기 전에는 아무 일도 아니었지만, 생긴 뒤로는 **"없는 편이 나은" 종류의 오답**이다. `project.repo_host`(`github`·`gitlab`)를 두고 EP-PRJ-03 이 싣고 EP-PRJ-04 가 받는다. **값은 사람이 고른다** — 도메인으로 추정하면 자체 호스팅에서 반드시 틀린다(`git.example.com` 은 아무것도 말하지 않는다). 그리고 이 값은 **접속에 쓰이지 않는다**: 서버는 대상 저장소에 접근하지 않고([4.1](scope.md) §5) 정하는 것은 주소의 모양뿐이다. 기본은 `github` 다 — 지금 만들어져 있는 링크가 전부 그 모양이라, 다른 기본값은 아무도 고르지 않은 값 때문에 오늘 되던 링크를 깨뜨린다. 어휘 밖은 400 이다(REQ-API-112 — 조용한 변환을 하지 않는다). **검증은 여전히 호스트를 보지 않는다**(`evidence-locator.ts`) — 그쪽은 무엇을 받아들일지의 규칙이고 이쪽은 표시 규칙이다.
>
> v1.28 변경(2026-09-10 — 증적이 자기 저장소를 말하지 못했다, 사람 지시): **REQ-API-157 신설 · EP-TASK-04·EP-PRJ-04 개정.** ① `evidence.repo` 는 "멀티 저장소 대비" 로 처음부터 있는 열이고 GitHub 웹훅이 `repository.full_name` 을 채우는데, **EP-TASK-04 가 그것을 고르지 않았다** — 화면은 증적 전부를 프로젝트의 저장소 하나로 읽고, 저장소가 둘 이상인 프로젝트에서 커밋 링크가 조용히 **남의 저장소**를 가리킨다(REQ-WEB-159 가 그 링크를 만든 뒤로 실제 오답이 된 자리다). 값은 주소가 아니라 **경로**(`org/repo`)이므로 호스트는 프로젝트의 `repo_url` 에서 빌리고 경로만 갈아 끼운다 — 프로젝트 주소가 없으면 `repo` 가 있어도 갈 곳이 없다(호스트를 지어내지 않는다). ② **빈 문자열은 "안 건드림" 이 아니라 비운다**: 화면이 저장소 주소와 기준 갈래를 편집할 수 있게 된 뒤로([4.5](screens.md) REQ-WEB-160) 사람은 잘못 적은 주소를 **지울** 수도 있어야 하는데, EP-PRJ-04 의 `coalesce` 는 null 을 "안 건드림" 으로 읽어 지우는 뜻을 실을 자리가 없었다. 빈 값은 NULL 로 눕히고 앞뒤 공백은 걷는다 — `''` 와 NULL 이 섞이면 읽는 쪽이 둘 다 봐야 하고, 그 둘 중 하나를 잊는 것이 "설정했는데 안 먹는다" 의 흔한 모양이다.
>
> v1.27 변경(2026-09-10 — 키로 부르면 `done` 만 깨졌다, 사람 보고): **REQ-API-156 신설 · §1.4b 산문.** §1.4b(REQ-API-038)가 "`task_id` 는 키와 UUID 를 모두 받는다" 고 못 박았고 도구 스키마도 그렇게 광고하는데, **계약이 안 지켜지는 자리가 하나 남아 있었다** — `nerv_task_update` 의 `status: "done"` 은 키로 부르면 **항상** 실패했다(실사용 발견 2026-09-08 · sudoku). `transition()` 은 참조를 풀어 `taskId` 에 담고 두 줄 아래의 `UPDATE` 는 그것을 옳게 쓰는데, **게이트에만 해소되지 않은 `input` 이 넘어갔다**: `evidence.task_id = 'SUD-T-…'` 가 22P02 로 터졌고 `approval`·`review_session` 두 자리는 `review_coverage` 를 켠 첫 프로젝트가 밟을 잠복이었다. 나쁜 것은 **터지는 자리**다 — 게이트 자체 검사보다 먼저 터져 "무엇이 빠졌다"(`kind: "done_gate"`) 대신 캐스팅 오류가 나오고, 그것이 REQ-API-106 의 22P02 그물에 걸려 **400** 으로 나간다. 그 결정은 스스로 "이것은 그물이지 검사가 아니다" 라고 적었는데(v0.86), 지금 그 그물이 **서버 결함을 호출자의 입력 오류로 보이게** 했다 — 에이전트는 자기 인자를 고쳐 보다 포기한다. 게이트의 시그니처를 `(tx, taskId, projectId, specImpact)` 로 바꿔 **해소되지 않은 참조가 닿을 길을 타입에서 없앤다**(호출부에서 펴 넘기는 안도 있었지만, 그러면 다음에 인자를 하나 더 넘길 때 같은 실수가 다시 가능하다). 감사 한 자리도 함께 고쳤다: `claim.conflict_blocked` 의 주체가 키일 수 있었고 그 자리는 `catch` 안이라 터지면 원래 `CONFLICT_SCOPE` 를 **덮었다**. 게이트 판정 규칙·정책 기본값·오류 분류·22P02 그물은 그대로다 — 그물이 잘못한 게 아니라 걸릴 일이 없어야 했다.
>
...
>
> v1.23 변경(2026-09-07 — 계약을 태운다, 개선 계획 넷째 스프린트): **새 요구사항 없음(§3.5 keep-alive 에 손잡이 표기).** v1.7 이 "코멘트 라인 `: ping` 이 아니라 `event: ping` 메시지다" 를 세 자리에서 고쳤는데 **그 계약을 태우는 검사가 없었다** — 문서만 고친 정정은 다음에 또 갈린다. 주기를 `NERV_SSE_KEEPALIVE_MS` 로 낮춰 L2 가 실제 스트림에서 형식을 본다([4.2](codebase.md) REQ-CB-035). 그 손잡이는 검사만을 위한 것이 아니다: 앞문의 유휴 타임아웃이 25초보다 짧은 배치에서 고칠 수단이 재배포뿐이었다.
>
> v1.22 변경(2026-09-07 — 역참조만 자른 뒤에 걸렀다, 개선 계획 넷째 스프린트): **REQ-API-154 신설 · EP-SPEC-02 행.** 검색의 필터 넷 중 종류·상태·요구사항은 `limit` 으로 자르기 **전에** 걸리는데 `references` 만 **뒤에서** 걸렸다 — 그 문서를 참조하는 스펙이 상위 `limit` 밖에 있으면 있는데도 빈 결과가 된다. "이걸 고치면 무엇이 흔들리나" 에 답하려고 만든 필터가 조용히 "아무것도 안 흔들린다" 고 답하던 셈이다.
>
> v1.21 변경(2026-09-07 — 있다고 적힌 경계가 없었다, 개선 계획 넷째 스프린트): **REQ-API-153 신설 · §4 두 행 · EP-SPEC-08.** [3.4](../03-proposal/agent-integration.md) §6.3 과 배포된 스킬 다섯이 2026-08 부터 "스펙 본문은 `<nerv:spec … trust=\"untrusted\">` 경계 안에 온다" 고 적어 왔는데 **서버 어디에도 그 경계가 없었다**(유령 응답 필드). 모델은 스킬이 말한 대로 경계를 찾고 찾지 못하면 본문을 그냥 읽으므로, **없는 경계를 있다고 가르치는 것은 경계가 없는 것보다 나쁘다** — 프롬프트 인젝션을 막는다고 적어 둔 방어가 통째로 서류상이었다. 감싸는 것은 사용자가 쓴 글뿐이다(문서 본문·요구사항 문장·코멘트·검색 스니펫·질문 답변). **사람의 지시(하트비트 `instructions`)는 감싸지 않는다** — 그것은 따라야 할 것이고, 따라야 할 것을 "데이터일 뿐" 이라 표시하면 경계가 반대로 쓰인다. **REST·미러는 감싸지 않는다**: 사람과 에디터가 읽는 곳에서 태그는 표시가 아니라 본문의 일부가 된다. 포장째 저장하면 400 이다 — 조용히 벗기면 "경계 안에 무엇을 넣어도 저장된다" 가 되고, 그때부터 그 경계는 아무것도 뜻하지 않는다.
>
...
>
> v1.17 변경(2026-09-07 — 게이트가 자기 신고 한 줄로 열렸다, 개선 계획 셋째 스프린트): **REQ-API-146~148 신설 · `gate_policy.done_gate` 신설.** ① done 게이트 강화를 **정책으로 켠다**(기본 off — 서버가 일괄로 켜면 오늘 통과하던 작업이 내일 막히고 그 이유를 아무도 고르지 않았다). `evidence_source: ci_or_human` 은 에이전트가 스스로 올린 증적만으로 닫는 길을 막고, `review_coverage: true` 는 FR-10 이 오래 이월해 온 조건(리뷰 라운드 존재 · 열린 critical 0)을 실제로 본다. 면제(`gate_bypass` 결재)가 있으면 넘어간다 — 기록된 예외다. ② **증적 위치의 형식을 검사한다** — 커밋 자리에 "다 했습니다" 가 들어가면 게이트는 통과하지만 증적은 아무것도 가리키지 않는다. 판정은 `@nerv/schema` 한 벌이고 호스트는 박지 않는다(`evidence.repo` 가 있는 것은 저장소가 여럿일 수 있다는 뜻이다). ③ **리뷰가 Task 에 이어진다** — `task_id` 를 명시하지 않으면 그 세션의 활성 클레임에서 채운다. 실데이터 리뷰 1,992건 중 링크된 것이 2건이라, 커버리지를 켜도 판정할 데이터가 없었다.
>
...
>
> v1.14 변경(2026-09-07 — 구현 축이 절반만 파생됐다, 개선 계획 셋째 스프린트): **REQ-API-141 신설.** ① **`verified` 를 파생한다**([3.5](../03-proposal/spec-workflow.md) §1.3 표의 마지막 줄) — 열은 처음부터 있었고(`evidence.verified_by`·`finding.requirement_id`) 파생만 없었다. QA 페르소나의 유일한 판정에 길이 없던 자리다. ② **증적 술어가 한 벌이 된다** — 파생과 커버리지의 `evidence_missing` 이 서로 다른 식으로 세어, 정상 경로(증적은 Task 에 붙는다)로 끝낸 요구사항을 대시보드가 "증적 결손" 으로 찍는 동안 파생은 `implemented` 라고 말했다. 둘 다 서버가 한 말이라 사람은 어느 쪽도 못 믿는다. ③ **회수·전이도 재파생한다** — `done` 만 재파생하고 있어서, 회수된 Task 의 요구사항은 아무도 하지 않는 일을 계속 `in_progress` 로 보였다.
>
> v1.13 변경(2026-09-07 — T3 가 1인으로 닫히고 있었다, 개선 계획 셋째 스프린트): **REQ-API-140 신설 · EP-APR-03 개정.** `requiredApprovers` 는 2026-08-23 부터 산출됐는데(T3=2) **읽는 곳이 없었다** — 첫 approve 가 문서를 approved 로 옮겼고, 실측 T3 승인 3건은 모두 1인 승인이다. 게이트가 조용히 약해진 자리이고, 카드에 n/2 표시가 없어 승인자는 자기가 마지막 결재라고 믿었다. 이제 제출이 **필요 수만큼 슬롯**을 세우고(슬롯 1 은 기본 큐, 둘째는 문서 타입의 직군 큐 — 그 역할의 멤버가 없으면 기본 큐로 강등하고 `role_slot_fallback` 을 남긴다), 결정은 **서로 다른 사용자**의 승인 수가 필요 수에 닿을 때만 문서를 옮긴다. 라운드 경계는 `spec_version.submitted_at` 이라 스키마 변경이 없다 — 거절 뒤 다시 제출하면 옛 승인은 자연히 빠진다. 집계는 대상 행을 `FOR UPDATE` 로 잠그고 한다: 동시 승인 둘이 각자 1을 세고 **아무도 전이하지 않는** 경합이 그 자리다. 승인 가능한 사람이 필요 수보다 적으면 그만큼만 세우고 `quorum_relaxed` 를 남긴다(§2.3 소규모 완화).
>
> v1.12 변경(2026-09-07 — 결재 판정이 세 벌이었다, 개선 계획 셋째 스프린트): **REQ-API-136~139 신설 · EP-APR-01·EP-APR-03·EP-SPEC-10 개정.** ① **지시자≠승인자는 세 축이다**(§2.3). 요청자만 비교하던 동안 구멍이 있었다 — 작성자가 남에게 제출을 부탁하면 요청자는 그 남이 되고 **작성자가 자기 초안을 자기 손으로 승인**할 수 있었다. 에이전트가 쓴 초안도 같다: 그 세션의 소유자가 승인하면 자기가 시킨 것을 자기가 통과시킨 것이다(D-01). 이제 요청자·작성자·작성 세션 소유자 셋을 본다. ② **제출은 작성자 본인 또는 planner·admin 이다** — 앞문을 닫는다. ③ **매트릭스의 ○ 를 역할 슬롯으로**. `approval.assignee_role` 이 있으면 그 역할(과 admin)만 그 카드를 내린다 — designer 가 지정 없이 모든 design 문서를 결재하게 만드는 것은 ○ 를 ● 로 바꾸는 결정이라 하지 않는다. ④ **받은 요청은 내 큐만 본다**(§6.6 원칙 3). 예전에는 지정 승인자만 걸러, 결재권 없는 사람의 목록에도 카드가 있었다 — 그 사람은 그것을 자기 일로 읽고 배지는 '내가 막고 있는 것' 을 세지 못한다. ⑤ **판정은 한 파일이다**(`approval-policy.ts` · D-05). 같은 물음을 목록·결정·`SpecService` 셋이 각자 답하고 있었고, 그래서 목록은 승인할 수 있다고 말하고 결정은 거절하는 카드가 생겼다. `SpecService.approve/reject` 는 지웠다 — 테스트만 부르던 두 번째 구현이라, 남겨 두면 판정을 우회하는 경로가 코드에 남는다. ⑥ 카드가 **왜 못 누르는지**(`can_approve_reason`)와 직군(`assignee_role`)을 함께 싣는다.
>
> v1.11 변경(2026-09-07 — 승인이 나도 에이전트는 듣지 못했다, 개선 계획 둘째 스프린트): **REQ-API-133~135 신설 · EP-TASK-07·EP-SPEC-10·EP-APR-03 개정.** ① **A3 결정을 요청한 세션에게 돌려주는 채널이 없었다.** 서버→세션 방향의 보장 채널은 하트비트 하나인데(§2.4) 결재 결정이 거기 실리지 않아, 승인을 기다리던 에이전트는 승인이 나도 **영영 듣지 못했다** — `NERV_APPROVAL_REQUIRED` 는 "하트비트로 확인하라" 는 다음 행동을 주면서 그 결과를 싣지 않았다. `approval_decided` 를 `pending` 에 싣는다(질문 답변과 같은 1시간 창·같은 상한·같은 규율 — **전달로 소멸하지 않는다**). 순서는 `steer|stop` → `basis_superseded` → `approval_decided` → `question_answered` 다: 승인이 났으면 재개는 그 위에서 시작한다. ② **기다리는 세션은 기다린다고 말한다.** `awaiting_input` 을 세우는 자리가 질문 하나뿐이라 T2·T3 제출·critical 하향·플랜 승인을 올린 세션은 사람을 기다리는 동안 S5 에 `active` 로 보였다(FR-11 · agent-integration §5.3 ②). 결정이 나면 `decide()` 가 `active` 로 되돌리되, **다른 대기 사유(열린 blocking 질문 · 결정되지 않은 다른 결재)가 남아 있으면 깨우지 않는다.** ③ **답변에 `answered_by`·`answered_at` 을 싣는다** — `answered_by_user_id` 는 처음부터 저장됐는데 어느 응답에도 없었고, 스킬과 카탈로그는 "결정자를 확인" 하라고 말하고 있었다.
>
> v1.10 변경(2026-09-07 — 리스 없는 done 이 무검사였다, 개선 계획 둘째 스프린트): **REQ-API-129~132 신설 · EP-TASK-09 개정 · REQ-API-005 정정.** ① **세션의 리스 구속**. 문서 셋이 "유효한 리스 없는 `done` 은 거부" 를 약속하는데(agent-integration §2.7 · 백로그 E09-S05 수용 기준 · REQ-API-005) 서버는 활성 클레임이 **아예 없으면 판정을 건너뛰었다** — 리스가 없다는 것이 거부가 아니라 무검사였다. `in_progress`·`in_review`·`done` 셋은 이제 그 세션이 쥔 살아 있는 클레임을 요구한다. ② **사람의 `done` 은 넷에게만** — 활성 클레임 보유자·담당자·planner·admin(사람 결정). 사람에게 리스를 요구하지 않는 이유는 사람이 리스를 쥐는 문이 없기 때문이고, 아무에게나 열어 두지 않는 이유는 `done` 이 증적·스펙 영향을 채우고 닫는 상태이기 때문이다. ③ **`ready` 는 도착지가 아니라 판정이다** — 위임 명세 4요소(임포트 자리표시자는 **빈 것**이다)와 미해소 `blocks` 의존이 여기서 선다. 예전에는 `update()` 의 승격 경로만 그것을 보고 이 문은 통과시켰다: DB CHECK 가 막긴 했지만 돌아오는 것은 제약 이름뿐이라 무엇을 채워야 하는지 알 수 없었다. 통과하면 `blocked_reason`·`delegate_session_id` 를 지우고 `task.ready` 를 낸다. ④ **`claimed` 는 이 문으로 들어가지 않는다**(`transition_not_allowed`) — 그 상태를 만드는 길은 원자적 클레임 하나뿐이다. 상태만 써 넣으면 클레임 행 없이 `claimed` 인 Task 가 되고, 그것은 아무도 쥐지 않았는데 아무도 잡을 수 없는 Task 다. 같은 이유로 **활성 클레임이 걸린 Task 는 `ready`·`backlog` 로 옮기기 전에 놓는다**(`release_required`) — 상태만 바꾸면 클레임 행이 살아남아 다음 클레임이 부분 unique 위반(500)이 된다. ⑤ `details.reclaimable` 이 실물이 됐다 — 살아 있는 클레임이 없고 다시 잡으면 잡히는 상태(지금 `ready` 이거나 만료된 리스가 걸려 있음)일 때 참이다. 클레임 없이 `in_progress` 인 고아 Task 는 거짓이고, 그것은 사람이 되돌려야 한다는 뜻이다.
>
> v1.9 변경(2026-09-07 — 차단·결재가 남긴 사실이 다른 이름이었다, 개선 계획 둘째 스프린트): **REQ-API-128 신설 · §3.3 두 행 개정 · EP-APR-03 이벤트 열 정정.** ① **차단도 사실이다.** `claim.conflict_blocked` 는 알림 카탈로그에 critical 로, 화면 무효화 맵에도 올라 있는데 **내는 곳이 0** 이었다 — 막힌 쪽은 409 로 알지만 알아야 할 사람은 **먼저 잡고 있던 쪽**이다. 클레임 트랜잭션이 통째로 롤백되므로 이 이벤트는 **별도 트랜잭션**에서 낸다(같이 롤백되면 남는 것이 없다). 수신자도 그 사실을 따른다: 겹침 계열은 페이로드의 상대 쪽 사용자에게 간다. ② **결재는 결재다.** `decide` 가 대상과 무관하게 `question.answered` 를 냈다 — 스펙 승인·거절도 "질문에 답했다" 로 남아(실측 12건 중 10건) `<리소스>.<동사>` 규약이 깨졌고, 이벤트로 결재를 세면 질문이 함께 세어졌다. `approval.decided` 를 신설하고 `to_state` 에 결정을, 페이로드에 대상 종류·자기 승인 여부를 싣는다. 질문 답변의 `question.answered` 는 `QuestionService` 자기 자리에 그대로 있다. ③ **판정 불가도 남는다** — 게이트 예외 경로가 `gate.failopen` 을 실제로 낸다(D-14 가 약속한 자리인데 비어 있었다). ④ **자동 통과의 액터**: `approved_by_user_id` 가 NULL 인 것은 참이지만(사람이 승인하지 않았다) 이벤트 액터까지 NULL 이면 승인 실측 65건 중 56건이 누가 일으켰는지 말하지 못한다(FR-16). 액터는 **그 제출을 한 사람·세션**이다 — 승인자와 다른 물음이라 자리를 나눈다.
>
> v1.8 변경(2026-09-07 — 서버가 뺏은 것은 흔적을 남기지 않았다, 개선 계획 둘째 스프린트): **REQ-API-127 신설 · §3.3 발생 지점 개정.** ① **회수도 상태 전이다**(FR-16). 리스 만료 회수는 이벤트를 하나도 내지 않았고(실측 회수 3건 · `claim.%` 0건) `markStale` 은 제목이 "stale 전이 + 클레임 회수" 인데 **클레임을 아예 건드리지 않았다** — 세션이 사라져도 리스는 최대 30분 더 살아 있어 그 창에서 멀쩡한 클레임이 겹침으로 막혔다(리스 만료가 stale 임계와 같은 값이라는 **우연**에 기대고 있었다). 회수·해제 다섯 경로(만료·세션 종료·중단·stale·사람 해제)가 이제 `ClaimService` 한 곳을 쓰고 **`claim.released` 와 `task.ready` 둘**을 남긴다 — 뒤엣것을 빠뜨리면 보드는 큐가 늘어난 것을 모른다. **액터는 클레임을 쥐고 있던 세션**이다(사람 결정): 리스를 놓친 것은 그 세션의 일이고, 서버를 액터로 적으면 피드에서 그 세션의 줄이 끊긴다. ② **`stale` 회수 사유 신설**(마이그레이션 `0023`) — `expired` 는 시간이 지난 것이고 이것은 세션이 사라진 것이다. 0019·0021 이 두 번 세운 원칙("판정된 값은 판정한 이유대로 적는다")이 그대로 적용된다. `stop` 의 페이로드 사유도 DB 어휘(`stopped`)로 통일했다 — `stopped_by_human` 은 저장 어디에도 없는 이름이라 그 값으로 세는 소비자는 0을 센다. ③ **리스 상한**(`LEASE_TTL_SECONDS` = 1800)을 넘기면 **거절한다**(조용히 깎지 않는다 — REQ-API-112 와 같은 축). 판정은 도메인에 있고 zod·도구 스키마가 함께 막는다.
>
> v1.7 변경(2026-09-07 — 전표가 실물과 열 자리에서 달랐다, 개선 계획 첫 스프린트): **새 요구사항 없음 — 코드가 옳고 문서가 낡은 자리 열이다.** ① `EP-TASK-09` 요청 열이 `evidence{commit_sha, pr_url, test_ids}` 라는 **있어 본 적 없는 모양**을 적고 있었다 — 실물은 `TaskEvidenceInput{kind, locator}` 의 배열이고, `.strict()` 라 전표대로 보내면 400 이다. `spec_impact` 가 `done` 의 **필수 입력**이라는 사실도 같은 칸에 적는다(빠지면 게이트가 `missing`). ② **유령 질의 인자 일곱 자리** — 컨트롤러가 읽지 않는 것을 걷고(`EP-TASK-02` 의 role·spec_id · `EP-QST-01`·`EP-REQ-01` 의 cursor) 읽는데 없는 것을 더했다(`EP-REV-03`·`EP-REV-04` 의 cursor·limit · `EP-SPEC-19` 의 baseline · `EP-SES-05` 의 limit · `EP-SPEC-03` `include` 의 **`attachments`**). ③ `EP-REV-04` 의 판정 열에서 **`uncovered` 를 걷는다** — 실물은 `passed`/`pending` 둘뿐이고, 리뷰 세션이 없는 브랜치는 판정이 아니라 **행 자체가 없다**. ④ `EP-REV-05` 권한 `review:submit` → **`review:resolve`**(사람 결정 — 문서를 코드에 맞춘다). 그 귀결을 같은 칸에 적는다: developer·designer 는 `review:resolve` 가 없어 **자기 코드에 달린 발견에 답할 수 없다**. ⑤ 필드 이름 둘 — `EP-TOK-02` `expires` → **`expires_at`** · `EP-MBR-02` `user` → **`email`**(v0.94 가 여섯 자리를 고칠 때 남은 둘이다). ⑥ `EP-MIR-01` 응답 열의 **승인자** — 미러 frontmatter 에 그 필드는 없다(일곱 키뿐이고 §2.8 예시는 이미 맞았다). ⑦ **WS 인증 실패는 `connect_error` 가 아니다** — socket.io 예약어라 서버가 emit 하면 예외가 나고 그 예외가 연결 핸들러에서 나면 **프로세스가 죽는다**(실측: 미인증 탭 하나가 API 를 크래시 루프에 빠뜨렸다). 실물은 `nerv:error`(`WS_ERROR_EVENT`)다. ⑧ **SSE keep-alive 는 코멘트 라인이 아니다** — rxjs 로 흘리면 Nest 가 코멘트가 아니라 메시지로 내보내므로 `event: ping` · `data: {}` 다. `: ping` 을 기다리는 클라이언트는 25초마다 알 수 없는 메시지를 받는다. ⑨ **SLA·리마인더·만료는 집행되지 않는다** — "§2.3·§2.6 을 그대로 집행한다" 고 적었는데 집행되는 것은 §2.3 의 판정 다섯(+완화 둘)뿐이고, §2.6 의 T+4h·T+1d·T+2d 리마인더와 T+7d 만료는 **잡도 열도 비어 있다**(`approval.due_at` 에 쓰는 코드 0건 · 워커 잡 목록에 리마인더 없음). 이월로 표기한다 — 있다고 적힌 SLA 는 없는 SLA 보다 나쁘다. ⑩ **고정 ID 접두 잔재** — §2.2b·§2.9a·REQ-API-025 가 `SPC-`·`REQ-`·`TSK-` 접두를 검색·매칭 규칙으로 적는데, 표시 키 형식은 2026-08-23 에 `<PRJ>-<타입>-<base32 6자>` 로 바뀌었다([3.3 데이터 모델](../03-proposal/data-model.md) §5.1 · 실물 `CLV-T-ZWHNB0`). **규칙은 접두가 아니라 키 정확 일치**로 적고 형식의 정본을 링크로 가리킨다. 두 자리의 규칙은 이제 같다 — 웹훅은 `DISPLAY_KEY_PATTERN`, 검색은 키 모양 게이트 + 키 열 정확 일치다.
>
> v1.6 변경(2026-09-07 — 접는 것도 무시다, 개선 계획 첫 스프린트): **REQ-API-126 신설 · 전표 다섯 자리.** §1.4j 와 REQ-API-082 는 "모르는 값은 거절이지 무시가 아니다" 라고 못 박았는데, 표면 다섯 자리가 삼항으로 값을 **기본값에 접고** 있었다 — `EP-QST-01` 의 `status` · `EP-APR-01` 의 `state` · `EP-NTF-01` 의 `state` · `EP-CMT-01` 의 `status` · `EP-SPEC-18` 의 `direction`. 그래서 **`questions?status=cancelled` 가 열린 질문 목록을 200 으로** 돌려줬다: REQ-API-109 가 만든 상태를 조회할 길이 없으면서 부른 쪽은 걸러진 목록이라고 믿는다. 500 은 실패를 알리기라도 하는데 접기는 그렇지 않다. 판정을 도메인으로 옮기고(`assertVocab`) **서비스 시그니처의 리터럴 유니온을 `string` 으로 넓혔다** — 유니온은 컴파일러를 막지 HTTP·MCP 호출자를 막지 않는다. 곁들여 `EP-EVT-01` 의 `type` 도 카탈로그(`NERV_EVENT_NAMES`)로 검사한다: `?type=spec.aproved` 가 빈 목록 200 이면 오타가 "그런 일이 없었다" 로 읽힌다. `EP-SPEC-18` 은 **어휘 판정을 스펙 조회보다 앞**에 둔다 — 오타는 대상이 있든 없든 오타이고, 뒤에 두면 같은 요청이 대상에 따라 400 이 되기도 409 가 되기도 한다. 파생 어휘 둘(`APPROVAL_INBOX_STATES`·`SPEC_RELATION_DIRECTIONS`)을 `@nerv/schema` 에 두었다.
>
> v1.5 변경(2026-09-07 — 조직 경계가 판정마다 따로 새고 있었다, 개선 계획 첫 스프린트): **REQ-API-125 신설.** `project_id IS NULL` 은 "조직 단위 멤버십" 인데 **어느 조직인지를 보지 않는 자리가 셋 더** 있었다 — 알림 수신자 둘(지정 역할 큐·기본 admin·planner 큐)과 자기 승인 소규모 완화의 인원 count. 결과는 **다른 조직의 admin·planner 에게 이 프로젝트의 스펙 키·제목이 알림으로 가는 것**이고(FR-14 가 세운 경계가 통째로 새는 자리다), 소규모 완화 쪽은 반대 방향으로 틀린다: 승인할 수도 없는 남의 조직 사람이 "둘째 사람" 으로 세어지면 완화가 필요한 자리에서 꺼져 아무도 결재를 끝낼 수 없다. `assertMembership`(2026-08-24)·자기 승인 admin 판정(2026-09-02)이 이미 같은 실수를 고친 자리다 — **판정마다 따로 고쳐 온 것**이 이 결함의 모양이라, 새 판정을 쓸 때 `org_id = p.org_id AND (m.project_id = p.id OR m.project_id IS NULL)` 를 한 벌로 쓴다.
>
...
> v0.74 변경(2026-09-05 — 종류로도 거른다, 사람 결정): **REQ-API-093 신설.** `status` 와 **같은 축**이라 규칙도 같다(매칭 + 조상 · `matched` · `around` 와 배타). 둘을 함께 주면 AND 다. `area` 는 본문 없이 자리를 잡는 종류라 `type=vision,area` 가 곧 **트리의 뼈대**다 — 실측: clemvion 141편 → **17편**, sudoku 18편 → 5편. 스킬이 그것을 쓴다([4.6](plugin.md) §new 2단계: 자리를 고를 때 뼈대만 본다). 어휘는 `spec_type` 6종이고 검사는 `status` 와 같은 자리다.
...
> v0.60 변경(2026-09-03 — 배지의 판정을 서버로): EP-SPEC-03 응답에 `recheck{count,specs[]}` 를 더한다. 참조 갱신 배지의 판정이 화면에 있었고 그 규칙이 초안이면 거의 언제나 참이라 **오탐률 100%** 였다([4.5](screens.md) v0.62). 판정은 서버가 한다 — 이미 발행하고 있던 `spec.recheck_requested` 를 이 버전의 저장 시각과 견주고, 무엇 때문인지도 함께 싣는다.
...
> v0.37 변경(2026-08-30 — 사람 결정): §4 대응표에 **`nerv_task_get`·`nerv_task_create`** 두 행. MCP 표면이 REST 의 **부분집합**이었다 — 만들기·단건 읽기가 없어 에이전트는 자기가 클레임할 다음 것만 볼 수 있었다. MVP 도구 약속도 16 → 18 로 함께 고쳤다([4.1](scope.md) §4.2). `TaskService.get` 이 키·UUID 를 둘 다 받도록 정정(§1.4b — 옆 도구들과 같은 규칙).
...
**프로젝트 관리를 사람 전용으로 옮긴 이유**(사람 결정). 게이트 **면제**(EP-APR-04)는 이미 사람 전용이었다 — **다만 2026-09-07 까지 그것은 역할 문턱뿐이었다**(PAT 도 소유자 역할로 지나갔다). 아래 논증은 그 사실이 참이 된 지금 유효하다(REQ-API-123). 그런데 게이트 **정책 자체**를 낮추는 길이 토큰에 열려 있으면 그것이 면제의 우회로가 된다 — 면제를 받지 못하는 에이전트가 임계값을 내리면 되기 때문이다. 같은 축의 한쪽만 잠그는 것은 잠근 것이 아니다. 보관·복구를 함께 막는 것은 프로젝트를 목록에서 지우는 일이 같은 무게라서다.
...
### 1.3b 다른 오리진에서 부를 때 — CORS 와 세션 쿠키 (2026-09-20 신설)

화면과 API 가 다른 오리진에 설 수 있다([4.1 MVP 범위와 스택 확정](scope.md) §2.3). 그때 브라우저가 무엇을 허락받는지가 이 절이고, **설정의 정본은 [4.2 코드베이스와 배포](codebase.md) §5.2c**(REQ-CB-041·042)다 — 여기서는 대외 계약으로서의 모양만 적는다.

| 계약 | 값 | 비고 |
| --- | --- | --- |
| 허용 오리진 | `NERV_WEB_URL` + `NERV_TRUSTED_ORIGINS` 의 합집합 | **와일드카드 없음** — 세션 쿠키를 싣는 요청이라 `credentials: true` 와 `*` 는 함께 서지 않는다. CORS·better-auth·`/mcp` 가드가 같은 목록을 본다 |
| `Origin` 이 없는 요청 | 통과 · CORS 헤더 없음 | 에이전트·CLI·서버 대 서버는 CORS 의 대상이 아니다(PAT 경로 — §1.3) |
...
- **쿠키로 인증하는 쓰기는 서버가 `Origin` 을 대조한다**(2026-09-20 확정 · [4.2](codebase.md) §5.2c · REQ-CB-043). `/api/auth/*`(better-auth)와 `/api/v1`(세션 오리진 가드)이 같은 목록으로 판정하며, 목록 밖이거나 **헤더가 없으면** `403 NERV_FORBIDDEN` 이다. 면제는 둘이다 — **PAT 요청**(헤더에 직접 싣는 자격증명이라 CSRF 가 아니다)과 **읽기**(`GET`·`HEAD`·`OPTIONS` — 상태를 바꾸지 않고 응답은 CORS 가 막는다). 즉 브라우저 밖에서 쓰기를 부르는 소비자는 **PAT 를 쓴다**(§1.3 의 2경로 그대로다).
- `/mcp` 의 Origin 규칙은 여기 계약과 별개로 **가드가 최종 강제**한다(REQ-CB-013) — 목록은 같고, 판정은 통과/`403` 둘뿐이다.
...
| `NERV_PRECONDITION` | 400 / 409 | 400: zod 스키마 위반(`details.issues` — 어긴 규칙에 카탈로그 키가 달려 있으면 `message` 가 그 문구다 · REQ-API-191) · **입력 모양을 어긴 DB 제약**(§1.4a — 빠뜨린 값·허용되지 않는 값·너무 긴 값) · 409: 본문 지문(`base_hash`) 불일치, 게이트 전이 조건 미충족, 멱등 키 본문 불일치, **중복 값·없는 참조**(§1.4a) |
...
**해소한 뒤에 넘기는 것까지가 이 규칙이다**(2026-09-10 신설 — 사람 보고 · REQ-API-156). 참조를 UUID 로 바꾸는 코드가 있는 것과 **그 뒤의 모든 질의가 그것을 쓰는 것**은 다른 일이다. `nerv_task_update` 의 `done` 이 그 틈에 있었다: `transition()` 은 첫 줄에서 참조를 풀어 `taskId` 에 담고 `UPDATE task … WHERE id = ${taskId}` 는 옳게 쓰는데, **done 게이트에만 해소되지 않은 인자 묶음이 통째로 넘어갔다.** 키로 부른 `done` 은 게이트의 첫 질의(`evidence.task_id`)에서 22P02 로 터졌고, `approval`·`review_session` 두 자리는 `review_coverage` 를 켠 프로젝트에서만 실행돼 **잠복**해 있었다.

- **터지는 순서가 나쁘다.** 캐스팅은 게이트 **자체 검사보다 먼저** 터진다 — 스펙 영향도 증적도 없이 키로 `done` 을 시도해도 돌아오는 것이 "무엇이 빠졌다"(`kind: "done_gate"` · `missing`)가 아니라 형식 오류다.
- **그물에 걸려 400 으로 나간다.** REQ-API-106 이 22P02 를 500 에서 400 으로 내리며 "이것은 그물이지 검사가 아니다" 라고 적었는데(v0.86), 그 그물이 여기서는 **서버 결함을 호출자의 입력 오류로 보이게** 했다. 에이전트는 자기 인자를 고쳐 보다 포기한다 — 고칠 것이 자기 쪽에 없기 때문이다.
- **막는 것은 타입이다.** 게이트는 `(tx, taskId, projectId, specImpact)` 를 받는다 — 참조를 담은 인자 묶음을 아예 받지 않으므로 해소되지 않은 값이 닿을 길이 없다. 호출부에서 펴 넘기는 안도 같은 결함을 고치지만, 다음에 인자를 하나 더 넘길 때 같은 실수가 다시 가능하다.
- **회귀는 키 경로로 태운다.** 이 결함이 산 자리는 테스트의 틈이었다 — done 을 다루는 L2 가 전부 UUID 로만 전이해서, 도구 스키마가 광고하는 키 경로는 한 번도 지나가지 않았다. 기본 정책 하나, `review_coverage: true` 하나, 그리고 **게이트 미충족도 키로** 확인한다(`kind: "done_gate"` 가 와야 한다).
...
| REQ-API-156 | WHEN 전이·게이트 판정이 참조를 받으면 THE SYSTEM SHALL **해소된 UUID 만** 이후 질의에 넘긴다 — 같은 호출 안에서 한쪽은 해소된 값을, 다른 쪽은 원래 참조를 쓰지 않는다. WHEN 키로 `done` 을 부르면 THE SYSTEM SHALL UUID 로 부른 것과 같게 판정하고, 게이트가 미충족이면 형식 오류가 아니라 `kind: "done_gate"` 와 `missing` 으로 거부한다. WHERE 감사 이벤트가 주체 id 를 싣는 자리이면 THE SYSTEM SHALL 해소된 id 만 쓰고, 그 기록이 실패해도 **원래 오류를 덮지 않는다** |
...
**결정은 거두지 않는다**(2026-09-26 — 사람 결정 · 결정 철회 검토). 위의 한 트랜잭션이 문서를 옮기고 재브리핑을 걸고 기다리는 세션을 깨우므로, 결정 행을 되감는 철회는 이미 퍼진 것을 되감지 못한다 — 이벤트는 append-only 이고 승인된 본문은 불변이다. 바로잡는 것은 **새 결정**이다: 스펙은 코멘트나 새 초안, 계획은 세션 중단이나 클레임 놓기, 하향은 새 발견, 면제는 새 결정. 화면이 그 길을 카드마다 말한다([4.5](screens.md) REQ-WEB-239 — 재검토 트리거도 거기 적는다).
...
| 헤더 생략 | 허용하되 재시도 안전성은 클라이언트 책임. 웹 SPA는 폼 제출·클레임·결정 요청에 항상 부여한다 — **누름마다 새 키**다(대상의 ID·본문으로 만들지 않는다 · [4.5](screens.md) REQ-WEB-195). 대상에서 만든 키는 같은 사람의 두 번째 조작을 24시간 동안 첫 응답의 재생으로 만든다 |
...
경계는 **목록이 자라는가**다. 자라는 것 — `activity` · `event` · `notification` · `finding` · `task` · `session` — 은 `{items, next_cursor}` 봉투로 답한다. 유한한 것 — 한 문서에 매인 기준선·코멘트·관계·버전 — 은 커서 대신 **총계**(`{items, total}`)를 준다. **그 넷에는 상한이 없다**(2026-09-07 · REQ-API-155) — 상한이 있으면 `total` 은 총계가 아니라 자른 수이고, 그것이 곧 §1.6 이 없애려던 "이게 전부" 다. 넷 중 셋이 2026-09-07 까지 맨 배열이었다: 배열은 화면에서 "이게 전부인가" 를 물을 자리를 없앤다.
...
- **배지가 있는 자라는 목록은 셋을 준다**(2026-09-24 · REQ-API-166): `{items, next_cursor, total}`. 받은 요청(EP-APR-01)이 그 하나다 — 쪽을 나누는 순간 "목록 길이 = 전체 수" 가 깨지는데 그 수를 화면 세 자리가 쓴다(헤더 배지 · 홈의 인사 · "받은 요청 전체 N건"). 별도 엔드포인트로 세면 출처가 둘이 되고, **배지와 목록이 어긋나면 지울 수 없는 숫자가 남는다** — 알림 배지에서 이미 겪은 자리다([4.5](screens.md) REQ-WEB-035). 같은 질의에서 같은 조건으로 센다. 받은 요청 대기는 넷째로 `actionable_total` — 그중 **내가 누를 수 있는 수** — 를 싣고, 세 자리는 그 수를 쓴다(2026-09-25 · 사람 결정 D2 · REQ-API-184 · [4.5](screens.md) REQ-WEB-217). 전체만 주면 내가 요청한 카드까지 배지에 들어, 할 일을 다 해도 0 이 되지 않는다.
...
- **커서의 시각은 DB 가 준 정밀도 그대로 싣는다**(2026-09-10 · REQ-API-124 의 같은 뜻). `Date` 로 왕복시키면(`new Date(...).toISOString()`) Postgres 의 µs 가 ms 로 잘리고, 그 순간 `=` 동률 판정이 깨진다 — 경계 행이 `.41567` 인데 커서에 `.415` 가 실리면 같은 ms 안의 `.41523` 은 `< .415` 도 `= .415` 도 아니어서 **영영 나오지 않는다.** 작업 목록이 그랬다(실측 2026-09-10 — 같은 ms 에 µs 만 다른 다섯 건 중 **둘만** 나왔다). 만드는 쪽은 드라이버 텍스트를 그대로 싣고(`::text` 로 못 박는다), 읽는 쪽은 `::timestamptz`·`::uuid`·`::enum` **캐스팅 앞에서** 형식을 거른다 — 거르지 않으면 낡은 커서가 22007·22P02 로 화면을 깨뜨린다.
...
- **조직이 경계다**(2026-08-24 정정). 그 합집합은 **같은 조직 안에서만** 이뤄진다. `assertMembership` 이 `project_id IS NULL` 만 보고 `org_id` 를 보지 않아, A 조직의 조직 단위 admin 이 **B 조직의 프로젝트에서도 admin** 이었다(실측·L2 회귀). 이 판정은 REST·WS join·SSE 가 함께 쓰는 한 곳이라 여기가 새면 세 표면이 함께 샌다. **같은 규칙이 멤버십을 세는 모든 자리에 적용된다**(2026-09-07 · REQ-API-125) — 알림 수신자 둘과 자기 승인의 소규모 완화 count 가 그 규칙 밖에 있었다.
...
| EP-PRJ-02 | `POST /api/v1/orgs/{org}/projects` | **조직 admin**(§2.1b · REQ-API-171) | `ProjectCreateInput` | `ProjectResult` | ★`project.created` |
...
| REQ-API-187 | WHEN 비밀번호 재설정 요청(`/api/auth/request-password-reset`)이 오면 THE SYSTEM SHALL 그 주소의 계정이 있을 때만 재설정 메일을 요청한 화면의 언어로 줄 세우되, 계정이 있든 없든 같은 상태와 같은 본문으로 답한다. WHERE 메일 발신이 꺼진 배치면 THE SYSTEM SHALL 재설정을 켜지 않고 `RESET_PASSWORD_DISABLED` 로 거절한다. WHEN 메일의 링크가 열리면 THE SYSTEM SHALL 토큰을 쓰지 않은 채 화면의 `/reset-password` 로 보내되 살아 있으면 토큰을, 아니면 `error=INVALID_TOKEN` 을 싣는다. WHEN 새 비밀번호가 토큰과 함께 오면 THE SYSTEM SHALL 토큰이 살아 있고 처음 쓰일 때만 바꾸고, 그 사람의 모든 세션을 끊으며, 이메일을 확인된 것으로 적는다. THE SYSTEM SHALL 토큰의 수명을 `PASSWORD_RESET_TTL_MINUTES` 로 두고, 요청과 재설정에 로그인과 같은 IP당 한도를 건다 |

#### 2.1c 조직을 가로지르는 목록은 범위를 이름으로 싣는다 (2026-09-24 신설 — 조직·프로젝트 경계 점검)

**받은 요청·알림·내 토큰은 조직을 가로지르는데 조직을 싣지 않았다.** 화면은 줄마다 `project_slug` 만 그릴 수 있었고, 두 조직에 같은 slug 가 있으면 둘을 가를 길이 없었다(REQ-API-152 가 요청 쪽에서 막은 그 모호함이 응답 쪽에 남아 있었다). 프로젝트 **이름**도 목록마다 있거나 없었다 — 알림은 주는데 화면이 쓰지 않았고, 멤버·초대는 주지 않았다.

추가만 하는 변경이라 기존 소비자(스킬·CLI)는 그대로다. 화면이 이 값들을 한 모양으로 그리는 규칙은 [4.5](screens.md) REQ-WEB-192 다.

| ID | 수용 기준(EARS) |
| --- | --- |
| REQ-API-170 | WHEN 조직을 가로지르는 목록 — 받은 요청(EP-APR-01)·알림(EP-NTF-01)·내 토큰(EP-TOK-01) — 을 돌려주면 THE SYSTEM SHALL 항목마다 `org_slug`·`org_name` 과 `project_name` 을 싣는다. WHEN 멤버·초대 목록과 초대 미리보기(EP-INV-04)를 돌려주면 THE SYSTEM SHALL `project_name` 을 싣는다 — 화면이 slug 가 아니라 이름을 그리고, 같은 slug 가 두 조직에 있어도 가를 수 있게 한다 |

### 2.2 스펙·버전·코멘트 (S3)

| ID | 메서드 · 경로 | 권한 | 요청 | 응답 | 발생 이벤트 |
| --- | --- | --- | --- | --- | --- |
| EP-SPEC-01 | `GET /api/v1/projects/{proj}/specs/tree` | 전 역할(`spec:read`) | `SpecTreeQuery`(root, depth, status, type, around, hops, include_relations, **baseline**(v0.79 배선 · REQ-API-098), include_archived — 기본 false, REQ-API-022 · root·depth 는 REQ-API-090 · status·type 은 REQ-API-092·093) | `SpecTreeResult`(id·title·type·문서 상태·현재 버전·`archived_at`) · 노드마다 `updated_at`(그 버전이 마지막으로 바뀐 시각) · `open_comments`(열린 코멘트 수 — REQ-API-183 · EP-SPEC-19 의 노드도 같다) | — |
| EP-SPEC-02 | `GET /api/v1/projects/{proj}/specs/search` | 전 역할 | `SpecSearchQuery`(**q**(질의 — 인자 이름은 `query` 가 아니다), type·status(쉼표 목록 — 2026-09-05 배선 · REQ-API-099. 어휘 밖은 400), requirement_id(**그 요구사항이 속한 스펙으로 좁힌다** — 2026-09-05 배선 · REQ-API-110. 키 또는 UUID, 없는 요구사항은 빈 결과가 아니라 409), **references**(이 스펙을 참조하는 문서만 — **자르기 전에 거른다**, 2026-09-07 정정 · REQ-API-154), include_archived — 기본 false(REQ-API-022), limit) | `SpecSearchResult`(고정 ID + 앵커 + 스니펫 + 관련도, **`related[]`** 1-hop 관계 확장 그룹, **`degraded?`** — 파이프라인은 §2.2b) | — |
| EP-SPEC-03 | `GET /api/v1/projects/{proj}/specs/{spec}` | 전 역할 | `SpecGetQuery`(**`v`** — 전표가 오래 `version` 이라 적었는데 실제 파라미터는 `v` 다: `?version=3` 은 조용히 무시되고 최신본이 온다. 기본 approved 최신 — Task 컨텍스트에서는 기준 버전 지정, `include` 쉼표 구분 — **구현: `tasks`·`comments`·`attachments`**(`requirements` 는 늘 실린다. 목록 밖의 이름은 400 이다 — 2026-09-04 에는 `include=["attachments"]` 가 `ok:true` 로 오고 응답에 첨부가 없었다). `relations` 요약은 Phase 2. **`baseline`**(그 세트가 이 문서에 묶어 둔 버전 — `version` 과 배타, REQ-API-087)) | `SpecGetResult`(+`basis_superseded?` — 요청 버전이 superseded면 최신 approved 번호와 함께 표시, +`recheck{count,specs[]}` — 이 버전을 마지막으로 쓴 뒤 온 참조 갱신 신호와 그 출처 스펙 키, REQ-WEB-037) · **`pending_approval_id`** — 이 버전을 기다리는 결정되지 않은 결재(없으면 `null` · REQ-API-182) | — |
| EP-SPEC-04 | `GET /api/v1/projects/{proj}/specs/{spec}/versions` | 전 역할 | — | `{items, total}`(유한 목록 — §1.6 · 2026-09-07 배선 · REQ-API-155. 그전에는 맨 배열이었다) — `SpecVersionSummary` | — |
...
| EP-SPEC-10 | `POST /api/v1/projects/{proj}/spec-versions/{ver}/submit` | 작성자 본인 또는 planner·admin — **2026-09-07 부터 실제로 판정한다**(REQ-API-139 · 그전에는 `spec:draft` 만 봐서 아무나 남의 초안을 제출할 수 있었고, 그것이 자기 승인의 앞문이었다) | **본문 없음** — `reviewer_hint`·`note` 는 2026-09-03 에 걷었다(리뷰어 지정은 게이트가 §6.3 규칙으로 정하고, 변경 요약은 초안의 `change_summary_md` 가 이미 나른다). 이 행이 그 뒤로도 둘을 적고 있었고 2026-09-05 에 스키마 대조 검사가 짚었다 | `SpecSubmitResult`(approval_id[], 지정 리뷰어·SLA) — **세션이 올린 T2·T3 제출은 그 세션을 `awaiting_input` 으로 세운다**(2026-09-07 · REQ-API-134). 자동 통과(T0·T1)와 사람 제출은 세션을 건드리지 않는다. T2·T3 의 `web_url` 은 받은 요청의 **그 카드**(`/inbox?focus=<approval_id>` — `NERV_WEB_URL` 이 있으면 절대 주소 · REQ-API-177) | `spec.submitted` + `approval.requested` |
| EP-CMT-01 | `GET /api/v1/projects/{proj}/specs/{spec}/comments` | 전 역할 | `CommentListQuery`(status: open/resolved) | `{items, total}`(유한 목록 — §1.6 · 2026-09-07 배선 · REQ-API-155) — `CommentResult` · 줄마다 `author_name` · 에이전트가 단 것이면 `author_hostname`·`author_agent_type` · `resolved_by_name`(REQ-API-183) | — |
...
| EP-SPEC-11 | `GET /api/v1/projects/{proj}/baselines` | 전 역할(`spec:read`) | `BaselineListQuery` — 질의 인자 없음 | `{items, total}`(유한 목록 — §1.6 · 2026-09-07 배선 · REQ-API-155. 전표가 오래 "배열이다 · §1.6 예외" 라 적었는데 §1.6 에 그 예외는 없었다) — `BaselineSummary`(name·note·항목 수·created_by·created_at) | — |
...
| EP-SPEC-18 | `GET /api/v1/projects/{proj}/specs/{spec}/relations` | 전 역할(`spec:read`) | `SpecRelationQuery`(direction: out/in/both 기본 both, kind?) — **상한이 없다**(2026-09-07 · REQ-API-155: 관계 탭·영향 미리보기의 목적은 완전한 역참조 목록이다). 응답은 `{items, total}`(커서 아님 — §1.6 유한 목록) | `Page<SpecRelationEntry>`(kind·방향·상대 스펙 id/key/title/문서 상태/현재 버전) — **역참조(backlink)가 1급이다**: 수정 전 "누가 나를 참조하나"의 조회 경로, S3 관계 패널([4.5 화면 명세](screens.md) §2.4)과 영향 미리보기의 데이터 소스 | — |
| EP-SPEC-19 | `GET /api/v1/projects/{proj}/specs/graph` | 전 역할(`spec:read`) | `SpecGraphQuery`(include_archived — 기본 false, **`baseline`** — 그 세트에 담긴 문서만 그때 핀된 버전으로. EP-SPEC-01 과 같은 판정이고 REQ-API-098 이 요구한 것인데 전표가 이 행에만 적지 않았다) | `SpecGraphResult`(`nodes[]` 트리와 같은 모양 + `edges[]` from_id·to_id·kind — **이 순서로 정렬해 준다**(2026-09-27 · [4.5](screens.md) REQ-WEB-245: 순서가 바뀌면 배치가 바뀐다)) — 전역 관계 그래프를 **한 응답**으로. 둘을 나눠 받으면 그 사이의 변화가 끝점 없는 간선으로 남는다. 화면 정본 [4.5](screens.md) §2.4a | — |
...
| 단계 | 동작 | 구현 축 |
| --- | --- | --- |
| ① ID 직행 | 질의에 **고정 ID 하나가 통째로 들어 있으면** 그 리소스를 최상위 반환 — 전문 검색을 거치지 않는다. 매칭은 **키 정확 일치**(`spec.key` 또는 `requirement.ref`)이지 접두 규칙이 아니다: 표시 키 형식은 `<project.key>-<타입>-<base32 6자>`(예: `CLV-T-ZWHNB0`)이고 접두는 **프로젝트가 정하는 값**이라 규칙에 박을 수 없다 — 형식의 정본은 [데이터 모델](../03-proposal/data-model.md) §5.1 이다(2026-09-07 정정 — 전표가 `SPC-`·`REQ-`·`TSK-` 접두를 적고 있었고, 그것은 2026-08-23 이전 표기다. 실데이터에서 그 접두에 맞는 키는 **task 0/487 · spec 2/159** 라, 사람이 실제 키를 쳐도 이 경로가 한 번도 걸리지 않았다) | 키 정확 일치 |
...
| ID | 메서드 · 경로 | 권한 | 요청 | 응답 | 발생 이벤트 |
| --- | --- | --- | --- | --- | --- |
| EP-REQ-01 | `GET /api/v1/projects/{proj}/requirements` | 전 역할 | `RequirementListQuery`(spec, impl_status, **`reverify`**(=`1` 이면 다시 검증이 필요한 것만 — 2026-09-26 · REQ-API-190) — **커서 없음**. 전표가 그 사실을 적어 두고 같은 괄호에 `cursor` 를 함께 적고 있었다) | `Page<RequirementResult>`(`ref`·`statement_md`·`impl_status`·연결 Task/Evidence 수 · **`reverify_required`**·`reverify_locator`(앞선 서명의 테스트 — "영향 없음 확인" 이 다시 서명할 대상)·`statement_changed_at`·`verified_at`) | — |
| EP-REQ-02 | `GET /api/v1/projects/{proj}/requirements/{ref}` | 전 역할 | — | `RequirementDetailResult`(버전 이력 + 파생 Task + Evidence — **목록과 같은 범위**: 요구사항에 직접 붙은 것과 파생 Task 의 것, 줄마다 `signed`·`stale` · `reverify_required` · 2026-09-26) | — |
...
| ID | 메서드 · 경로 | 권한 | 요청 | 응답 | 발생 이벤트 |
| --- | --- | --- | --- | --- | --- |
| EP-TASK-01 | `GET /api/v1/projects/{proj}/tasks` | 전 역할 | `TaskListQuery`(status[], assignee, spec, **`ai`**(=`1` 이면 `delegate_session_id` 가 있는 것만 — 에이전트 세션이 쥔 것이지 사람이 담당인 것이 아니다 · REQ-API-122) — **`priority` 필터는 없다**(2026-09-06 정정: 서비스가 그 인자를 받지 않는다), **include_archived**(기본 false — `done_at` 이 `TASK_DONE_WINDOW_DAYS` 를 지난 done 을 포함, [4.5 화면 명세](screens.md) §2.5), cursor, limit) | `Page<TaskSummary>`(보드 레인용 — 정렬 `priority ASC, updated_at DESC, id ASC`. `delegation_complete` 는 승격·전이와 같은 판정이다 — 임포트 자리표시자는 빈 것 · REQ-API-175) · 활성 클레임의 실행 세션 `claim_hostname`·`claim_agent_type`·`claim_session_state`(REQ-API-180) | — |
| EP-TASK-02 | `GET /api/v1/projects/{proj}/tasks/next` | task:claim 보유 역할 | `TaskNextQuery`(**`limit` 하나뿐**이다 — 전표가 오래 적던 `role`·`spec_id` 는 컨트롤러가 읽지 않는다. 질의 인자는 `.strict()` 를 지나지 않으므로 보내도 400 이 아니라 **조용히 버려지고**, 부른 쪽은 걸러진 큐를 받았다고 믿는다) | `TaskNextResult`(ready 후보 + **위임 명세 4요소** + 권장 scope) | — |
| EP-TASK-03 | `POST /api/v1/projects/{proj}/tasks` | planner·developer·admin ●, qa ○ **AND `task:update`**(MCP `nerv_task_create` 와 같은 권한 — §1.3b · 역할 목록은 `TASK_CREATE_ROLES` — 화면이 같은 값을 본다) | `TaskCreateInput`(title, body_md, source_spec_version_id, source_requirement_id, **baseline**(이름 — 주변 문서까지 포함한 기준 세트), 위임 명세 4필드, priority) | `TaskResult`(status=backlog) | ★`task.created` |
| EP-TASK-04 | `GET /api/v1/projects/{proj}/tasks/{task}` | 전 역할 | — | `TaskDetailResult`(위임 명세·활성 클레임·의존·Evidence(**`repo` 포함** — 2026-09-10 · REQ-API-157. 열은 처음부터 있었고 웹훅이 채우는데 고르지 않았다)·**`blocked_resolution`** — 막힌 Task 에만 실린다. `{reason, source, satisfied, pending[]}` 이고 **저장하는 열이 아니라 파생**이다(REQ-API-118). `satisfied: null` 은 "아니다" 가 아니라 **서버가 판정할 수 없다**는 뜻이다) · 담당 이름 `assignee_name`(REQ-API-180) | — |
| EP-TASK-05 | `PATCH /api/v1/projects/{proj}/tasks/{task}` | planner·developer·admin **AND `task:update`**(역할 목록은 `TASK_EDIT_ROLES`) | `TaskUpdateInput`(위임 명세·priority·의존·**`rebrief`** — `true` 면 기준 SpecVersion 을 최신 승인본으로 옮기고 재브리핑 표시를 지운다 · REQ-API-121) | `TaskResult` — 위임 명세 4요소 충족 + 의존 해소 시 서버가 `ready` 승격 | `task.ready`(승격 시) |
...
| EP-TASK-07 | `POST /api/v1/projects/{proj}/claims/{claim}/heartbeat` | 클레임 보유자 | `HeartbeatInput`(progress, stats{added, removed, files}) | `HeartbeatResult`(새 `lease_expires_at` + `pending` — **네 종류**(`steer|stop` → `basis_superseded` → `approval_decided`(2026-09-07 · REQ-API-133) → `question_answered`)이고 지시만 한 번, 나머지 셋은 1시간 창 동안 다시 실린다 + **`scope_overlaps`** — 지금 내 범위와 겹치는 활성 클레임 수(block·warn만). 클레임 응답의 겹침은 *잡던 순간*의 사실이라 statusline 은 이 값을 쓴다 · REQ-API-116) | — (이벤트 없음 — `last_heartbeat_at` 갱신만) |
...
| EP-TASK-09 | `POST /api/v1/projects/{proj}/tasks/{task}/transition` | 담당자·planner·admin(`task:update`) — **`done` 은 넷에게만**(활성 클레임 보유자·담당자·planner·admin · 2026-09-07 · REQ-API-130) | `TaskTransitionInput`(status — 목표는 **여섯**(`backlog`·`ready`·`in_progress`·`in_review`·`done`·`blocked` · `TASK_TRANSITION_TARGETS`)이고 **`claimed` 는 없다**(`nerv_task_claim` 만이 만든다 — REQ-API-132), `spec_impact`(jsonb — **`done` 의 필수 입력**이다: 빠지거나 빈 객체면 게이트가 `missing` 으로 막는다. "영향 없음" 도 sentinel 로 **선언**해야 통과한다), `evidence[]` — **`TaskEvidenceInput{kind, locator}` 의 배열**이다(전표가 오래 `{commit_sha, pr_url, test_ids}` 라 적었는데 그런 모양은 있어 본 적이 없다. 한 Task 가 커밋·PR·테스트를 여럿 남기므로 목록이고, 어휘는 `evidence_kind` 다 — REQ-API-056), blocked_reason — **어휘 4종**(`awaiting_answer`·`dependency_broken`·`spec_conflict`·`external`)이고 `@nerv/schema` 의 `BLOCKED_REASONS` 가 정본이다. ~~`note`~~ 는 **2026-09-06 걷었다** — 받고 버려지던 필드라 이제 `.strict()` 가 400 을 낸다) | `TaskResult` 또는 409(게이트 거부 사유 — FR-10). 거절 어휘 다섯(2026-09-07): `transition_not_allowed` · `no_active_claim` · `release_required` · `delegation_spec_incomplete` · `dependencies_pending` | `task.blocked` · `task.done` · **`ready` 도착이면 `task.ready`**(2026-09-07) · 기타 전이는 ★`task.updated`(payload에 from/to) |
...
| ID | 메서드 · 경로 | 권한 | 요청 | 응답 | 발생 이벤트 |
| --- | --- | --- | --- | --- | --- |
| EP-SES-01 | `GET /api/v1/projects/{proj}/sessions` | 전 역할 | `SessionListQuery`(state[] · **`cursor`·`limit`**(2026-09-07 정정 — v1.2 가 봉투를 준 뒤로 커서를 읽는다. `user` 는 여전히 읽지 않는다). 응답에 `summary`(상태별 수 여섯)가 함께 실린다) — 정렬은 `last_heartbeat_at DESC NULLS LAST, started_at DESC, id DESC` 이고 커서가 그 셋을 담는다(REQ-API-124) | `Page<SessionSummary>`(user·hostname·agent_type·state·현재 Task·diff·last_heartbeat — [데이터 모델](../03-proposal/data-model.md) §4.2 질의) — 활성 클레임이 없으면 `last_task_key`·`last_task_title`·`last_claim_status`·`last_release_reason`, 입력 대기(`awaiting_input`)면 `waiting_question_id`·`waiting_question_title`(없으면 `waiting_approval_id`)도 싣는다(REQ-API-179) | — |
...
| ID | 메서드 · 경로 | 권한 | 요청 | 응답 | 발생 이벤트 |
| --- | --- | --- | --- | --- | --- |
| EP-APR-01 | `GET /api/v1/approvals` | 로그인 사용자(본인 관련만) | `ApprovalListQuery`(state: pending/decided, project, **`cursor`·`limit`**(2026-09-24 · REQ-API-166) — `expired`·`subject_type`·`subject_id` 는 읽지 않는다) | `{items, next_cursor, total, actionable_total}`(§1.6 — `actionable_total` 은 대기에만 온다 · REQ-API-184. 대기는 **누를 수 있는 것부터, 그 안에서 오래 기다린 것부터**, 처리됨은 **방금 결정한 것부터**) · `ApprovalCard`(대상 원문 델타·영향 분석 포함 — [스펙 워크플로우와 거버넌스](../03-proposal/spec-workflow.md) §6.4 카드 3유형). **내 큐만 온다**(2026-09-07 · REQ-API-137 — 지정 승인자·직군 슬롯·기본 큐(`approval:decide`)·admin). 카드는 `can_approve` 와 함께 **`can_approve_reason`**(`author`·`session_owner`·`self_requested`·`missing_role`·`not_in_role_queue`·`not_assignee`·`already_approved`)·`assignee_role` 을 싣는다. **일괄 판정도 함께 온다**(2026-09-22 · REQ-API-163): `can_bulk_approve` 와 `bulk_block_reason`(`bulk_quorum`·`bulk_gate_bypass` 또는 단건이 막히는 이유) — 화면이 저위험 규칙을 다시 구현하면 두 벌이 되고, admin 완화가 낀 규칙은 두 벌이 되는 순간 한쪽만 고쳐진다. **무엇을·누가·왜 도 함께 온다**(2026-09-24 · REQ-API-177): `requested_hostname`·`requested_agent_type`·`session_waiting` · 스펙의 `change_summary_md`·`gate_tier` · 플랜의 `task_key`·`task_title` · 발견의 `finding_id`·`finding_title`·`finding_severity` · **왜 그 티어인가도 온다**(2026-09-26 · REQ-API-188): 스펙 카드의 `gate_score`(4축 합계) · `gate_axes`(`side_effect`·`sensitivity`·`reversibility`·`blast_radius` 축별 점수) · `gate_signals`(티어를 올린 신호 — 발동한 것만) · `gate_evidence`(신호의 근거 — 재시도 신호가 가리키는 에스컬레이션 · 2026-09-26 · REQ-API-189) · **질문은 `session_id` 를 싣는다**(2026-09-26 · [4.5](screens.md) REQ-WEB-239 — 답을 보낸 토스트가 그 세션으로 데려간다) | — |
| EP-APR-02 | `GET /api/v1/approvals/{id}` | 관련자 | — | `ApprovalDetailResult` — 결정됐으면 `decided_by`(이름)도 싣는다: 받은 요청이 "찾던 카드는 이미 처리됐다" 를 말하는 자리다(REQ-API-177) | — |
| EP-APR-03 | `POST /api/v1/approvals/{id}/decision` | 지정 승인자·해당 역할 큐 — **사람 전용**, 지시자≠승인자 판정([스펙 워크플로우와 거버넌스](../03-proposal/spec-workflow.md) §2.3 5규칙) | `ApprovalDecisionInput`(decision: approve/reject/comment, **comment**, **seen_content_hash**(무엇을 보고 승인했는가 — 본문이 그 뒤 바뀌었으면 그 승인은 다른 문서에 대한 것이라 막힌다)) | `ApprovalResult`(승인이면 **`quorum{given,required,satisfied}`** 를 함께 준다 — `satisfied:false` 면 결재는 남았지만 **문서는 움직이지 않았다**는 뜻이다 · 2026-09-07 · REQ-API-140) — 요청이 세션에서 왔으면 그 세션을 `active` 로 되돌리고(다른 대기 사유가 없을 때만) 결정을 하트비트 역채널에 싣는다(2026-09-07 · REQ-API-133·134) | ★`approval.decided`(결정 자체 — 2026-09-07 · REQ-API-128) + 대상이 spec_version이면 `spec.approved`(+이전 버전 `spec.superseded`) / `spec.rejected` |
| EP-APR-04 | `POST /api/v1/projects/{proj}/gates/bypass` | admin ●, planner(스펙 계열)·developer(코드 계열) ○ — **사람 전용**(REQ-API-123 · 판정은 도메인. 역할 문턱은 PAT 도 지난다) | `GateBypassInput`(**subject_id**, **reason**) — 유효 시간은 받지 않는다: 면제도 결재 레코드라 되돌리는 것은 만료가 아니라 새 결정이다 | `ApprovalResult`(is_bypass=true) | `gate.bypassed` |
| EP-APR-05 | `GET /api/v1/projects/{proj}/inbox` | 로그인 사용자(본인 관련만) — **사람 전용**(REQ-API-123 — 전역 EP-APR-01 과 같은 규칙) | `state`·`cursor`·`limit` — EP-APR-01 과 같다 | **EP-APR-01 을 그 프로젝트로 좁힌 것**(결재 + 질문 · 같은 봉투 `{items, next_cursor, total}`) — S7 이 프로젝트 안에서 읽는 곳 | 질의를 따로 들고 있던 동안 같은 질문에 다른 답을 냈다(REQ-API-167) |
| EP-APR-06 | `POST /api/v1/approvals/decisions` | 지정 승인자·해당 역할 큐 — **사람 전용**(EP-APR-03 과 같은 판정. 표면은 `decide()` 를 건마다 부르는 얇은 루프이고 판정을 다시 하지 않는다 · D-05) | `ApprovalBulkDecisionInput`(decision: **approve/reject 둘뿐**(`comment` 는 어휘 밖이다), comment(전 건에 같은 사유가 남는다), **items[]{id, seen_content_hash}** — 지문은 **건마다** 싣는다 · 상한 **50** — 넘기면 400 과 "한 번에 50건까지 결정할 수 있습니다" · REQ-API-191) | `{ok, batch_id, decided, failed, results[]{id, ok, code?, kind?, message?, quorum?}}` — **200 이 전부 성공을 뜻하지 않는다**: 건마다 제 트랜잭션이라 부분 실패가 정상이고, 실패는 `kind`(`already_decided`·`stale_approval`·`bulk_quorum`·`bulk_gate_bypass`·`not_found` 등)로 갈린다. **승인은 저위험만 지나간다**(`can_bulk_approve` — 정족수 2 이상과 `gate_bypass` 는 빠지고 **admin 은 둘 다 면제**다 · 2026-09-22 사람 결정). 거절은 고른 것 전부다 — 서버가 막는 것은 승인뿐이다 | 건마다 ★`approval.decided`(payload 에 **`bulk:true`·`batch_id`**) |
| EP-QST-01 | `GET /api/v1/projects/{proj}/questions` | 전 역할 | `QuestionListQuery`(**`status` 하나뿐 · 커서 없음** — 어휘는 `question_status` 넷(`open`·`answered`·`cancelled`·`expired`)이고 기본은 `open`, 밖의 값은 400 이다(REQ-API-126). 전표가 적던 `cursor` 는 컨트롤러에 없다) | `Page<QuestionResult>`(선택지·대기 세션·경과·**출처**(`spec_key`·`task_key`·`finding_id`)·`escalate`) | — |
| EP-QST-02 | `POST /api/v1/projects/{proj}/questions/{id}/answer` | 대상 역할 또는 지정자 — **사람 전용**(REQ-API-123 · 권한 `spec:read` 는 모든 PAT 가 가지므로 판정은 도메인에 있다) | `QuestionAnswerInput`(answer_key 또는 answer_md) | `QuestionResult`(status=answered) — 전용 경로 `QuestionService.answer`(결재 `decide` 와 다른 경로다) | ★`question.answered` |
...
**승인 유효성 판정**은 [스펙 워크플로우와 거버넌스](../03-proposal/spec-workflow.md) §2.3 정본을 서버가 그대로 집행한다 — 자기 승인 거부(요청자 = 승인자) · 에이전트 영구 불가(`assertHuman`) · content hash 불일치 = stale 승인 거부(`seen_content_hash`) · 그리고 같은 절의 **완화 둘**(프로젝트에 승인 가능한 사람이 둘 미만이면 자동 완화 · admin 은 허용하되 감사에 남긴다). 판정은 화면이 아니라 도메인 한 곳이고, 화면은 그 결과 불리언(`can_approve`)을 그대로 읽는다(D-05).

**SLA·리마인더·만료는 아직 집행되지 않는다**(2026-09-07 정정 — 이 문단이 오래 "§2.3·§2.6 을 그대로 집행한다" 고 적었다). 같은 문서 §2.6 의 표(T+4h 리마인더 1회 · T+1d 2회 + 대체 승인자 · T+2d admin 에스컬레이션 · **T+7d 만료 후 draft 복귀**)를 만드는 코드가 없다: `approval.due_at` 열은 있지만 **쓰는 곳이 한 곳도 없고**, 워커 잡 목록(`notification`·`retention`·`partition`·`lease-reaper`·`session-stale`·`embedding`·`export`)에 리마인더도 만료도 없다. **이월**로 표기한다 — 있다고 적힌 SLA 는 사람이 그것을 믿고 기다리게 하므로 없는 SLA 보다 나쁘다. MVP 받은 요청 카드는 스펙 승인·플랜·질문 3유형이고 CR·에스컬레이션 카드는 P2다([로드맵](../03-proposal/roadmap.md) FR-11).
...
| EP-REV-04 | `GET /api/v1/projects/{proj}/gates/reviews` | `spec:read` | `GateCoverageQuery`(**`limit`** — 기본 20·최대 200, §1.6 의 예외 둘 중 하나) | `GateCoverageResult`(`{items, total}` — 브랜치별로 branch, 커버 리뷰(head_sha·round_no·완료 시각), 해소 `resolved/total`, 판정 **`passed`/`pending` 둘뿐**, 면제(`is_bypass` 결재의 사람·시각·사유)) | — |
...
| ID | 메서드 · 경로 | 권한 | 요청 | 응답 | 발생 이벤트 |
| --- | --- | --- | --- | --- | --- |
| EP-EVT-01 | `GET /api/v1/projects/{proj}/events` | 전 역할(`spec:read`) — `audit:read` 라는 권한은 **존재한 적 없다**(2026-09-05 정정) | `EventListQuery`(type, subject_id, **`before`·`limit`** — `since`·`subject_type` 은 읽지 않고 커서 이름은 `before` 다, 2026-09-06 정정. 값은 **불투명**이고 `(occurred_at, id)` 를 담는다 · REQ-API-124) — `occurred_at DESC, id DESC` | `Page<EventResult>`(actor{user, session, is_agent}·from/to state·payload — [데이터 모델](../03-proposal/data-model.md) §2.9) · **무엇에 일어났나**(REQ-API-181): 대상 `spec_key`·`spec_title`·`version_no` · `task_key`·`task_title` · `finding_id`·`finding_title` · `question_title` · `review_branch` · `session_hostname`·`session_agent_type` — 결재·질문·클레임·리뷰 세션이 대상이면 그 너머의 것을 싣는다. 행위자 `actor_user_id`·`actor_session_id`(화면이 잇달아 같은 일을 접는 기준) | — |
| EP-NTF-01 | `GET /api/v1/me/notifications` | 본인 | `NotificationListQuery`(state: unread/read, **`before`·`limit`** — 커서 이름은 `before` 다. 값은 불투명하고 `(created_at, id)` 를 담는다 · REQ-API-124) | `Page<NotificationResult>` — 대상이 스펙 버전이면 `version_no` 를 함께 싣는다(아니면 `null` · REQ-API-159). 결재 요청·질문 알림은 그 요청이 닫혔으면 `resolution`(`approve`·`reject`·`comment`·`answered`·`cancelled`)과 `resolved_by` 를 싣는다(REQ-API-176) · 대상 칸은 EP-EVT-01 과 **같은 규칙**이다 — 결재 요청·질문 알림도 그 스펙 버전·작업·발견·질문 제목을 싣는다(REQ-API-181) | — |
...
| EP-COV-01 | `GET /api/v1/projects/{proj}/coverage` | 전 역할 | `CoverageQuery`(spec) | `CoverageResult`(스펙별 구현/검증 커버리지·증적 결손·빈 약속·**다시 검증 필요**(`reverify_required` · 2026-09-26) — [스펙 워크플로우와 거버넌스](../03-proposal/spec-workflow.md) §5.5 산식) | — |
...
| 메일 | **자동으로 나간다**(2026-09-22 · 사람 결정). 초대 행과 메일이 **한 트랜잭션**에 들어가고([4.3](database.md) §2.17 `email_outbox`), 워커가 보낸다. **토큰 원문은 응답에 그대로 남긴다** — 메일이 꺼졌거나 막힌 배치에서도 [복사] 경로가 살아 있어야 한다. 응답의 `queued` 가 둘을 가른다: 화면은 "메일을 보냈습니다" 와 "링크를 전달하세요" 를 갈라 말해야 하고, 둘을 같은 문구로 덮으면 한쪽은 반드시 거짓이다 |
...
- 인증 실패 시 **`nerv:error` 이벤트**에 `{code: "NERV_UNAUTHENTICATED"}`를 실어 보내고 즉시 끊는다. 이름의 정본은 `@nerv/schema` 의 **`WS_ERROR_EVENT`** 이고 클라이언트도 그 상수를 읽는다(REQ-CB-006).
  - **`connect_error` 가 아닌 이유**(2026-09-07 정정 — 전표가 오래 그 이름을 적었다): socket.io 의 `connect_error` 는 **예약어라 서버가 emit 할 수 없다.** emit 하면 예외를 던지고, 연결 핸들러에서 던진 예외는 프로세스를 죽인다 — 실측으로 **미인증 브라우저 탭 하나가 API 를 크래시 루프에 빠뜨렸다.** 클라이언트 쪽 `connect_error` 는 그대로 있다(전송 실패·핸드셰이크 거절에 socket.io 가 스스로 낸다); 서버가 사유를 실어 보내는 길이 별도 이름인 것이다.
...
| `task.ready` · `task.claimed` · `task.blocked` · `task.done` | Task 축 전이(§1.4·§6.3). `task.ready` 는 **회수·해제 다섯 경로 전부**에서 `claim.released` 와 짝으로 난다(REQ-API-127) | `project:{id}` | P0~P1 |
...
| `claim.conflict_warn` · `claim.conflict_blocked` | 클레임 겹침 판정(§4.4). **차단도 낸다**(2026-09-07 · REQ-API-128) — 클레임이 롤백되므로 별도 트랜잭션이다 | `project:{id}` + 양쪽 세션 소유자 `user:{id}` | P0 |
| ★`claim.released` | EP-TASK-08 · EP-SES-04(stop) · `nerv_session_end` · **리스 만료 회수 · stale 회수**(2026-09-07 · REQ-API-127 — 다섯 경로가 같은 함수를 쓴다) | `project:{id}` | P0 |
...
| ★`approval.decided` | EP-APR-03 — 승인·거절·코멘트 모두(2026-09-07 · REQ-API-128 — 그 전에는 이 자리가 `question.answered` 였다) | 요청자 `user:{id}` + `project:{id}` | P1 |
| ★`question.answered` | EP-QST-02 **만**(2026-09-07 정정 — 결재는 위 행이다) | 요청 세션 소유자 `user:{id}` + `project:{id}` | P1 |
...
- keep-alive — WS는 socket.io 기본 ping(주기 25초·타임아웃 20초)을 그대로 쓰고, SSE는 25초 주기의 **`event: ping` 메시지**(`data: {}` · `id:` 는 빈 문자열)를 송신한다. **에이전트 하트비트 60초와는 무관하다** — 그것은 MCP 클레임 리스 규약이다.
...
- **keep-alive**: 25초 주기(`NERV_SSE_KEEPALIVE_MS` 로 바꾼다 — [4.2](codebase.md) §5.2 · REQ-CB-035)의 **`event: ping` 메시지**다 — `data: {}` 이고 `id:` 는 빈 문자열이며, 클라이언트는 이 타입을 무시한다. **코멘트 라인(`: ping`)이 아니다**(2026-09-07 정정 — 전표가 §3.4·이 절·REQ-API-014 세 자리에서 그렇게 적고 있었다): 스트림을 rxjs 로 흘리므로 Nest 가 코멘트가 아니라 메시지로 내보낸다. `: ping` 을 기다리도록 쓴 클라이언트는 25초마다 알 수 없는 이벤트를 받는다. 프록시 계층의 버퍼링 해제·타임아웃은 [4.2 코드베이스와 배포](codebase.md) §5.4(nginx)·§6.3(Ingress)이 정본이다.
...
| `nerv_spec_search` | A1 | `SpecService.search` | EP-SPEC-02 | 하이브리드 파이프라인 §2.2b — 두 표면 동일. `snippet` 은 본문에서 잘라 온 조각이라 `<nerv:text kind="snippet">` 경계로 나간다(REQ-API-153). `related[]`(관계 확장)·`degraded` 표기 포함(정의 정본 [3.4](../03-proposal/agent-integration.md) §2.3, 2026-08-22 갱신) |
| `nerv_spec_get` | A1 | `SpecService.get` | EP-SPEC-03 | **사용자 생성 본문 셋이 비신뢰 경계로 감싸여 나간다**(2026-09-07 실물화 · REQ-API-153 — `body_md` · `requirements[].statement_md` · `comments[].body_md`. [에이전트 연동 설계](../03-proposal/agent-integration.md) §6.3) — **MCP 표면에서만**이고 REST·미러는 감싸지 않는다. `version`/`baseline` 인자와 `basis_superseded` 표시는 두 표면 동일(기준 버전 규약 — 같은 문서 §2.4) |
...
| REQ-API-005 | WHEN 세션 경로가 리스 구속 목표(`in_progress`·`in_review`·`done`)로 전이를 시도하는데 그 세션이 쥔 살아 있는 클레임이 없으면 THE SYSTEM SHALL HTTP 409 `NERV_LEASE_EXPIRED`(`kind`: `no_active_claim` 또는 `lease_expired`)를 반환하고 `details.reclaimable` — 살아 있는 클레임이 없고 다시 잡으면 잡히는 상태인가 — 를 함께 싣는다(2026-09-07 정정 · REQ-API-129) | 리스 만료 후 done 전이 거부([에이전트 연동 설계](../03-proposal/agent-integration.md) §2.7 리스 규약) |
...
| REQ-API-009 | WHEN 미인증 소켓이 `/ws`에 연결을 시도하면 THE SYSTEM SHALL **`WS_ERROR_EVENT`(`nerv:error`)** 에 `{code:"NERV_UNAUTHENTICATED"}`를 실어 보낸 뒤 끊고(`connect_error` 는 socket.io 예약어라 서버가 emit 할 수 없다 — 2026-09-07 정정), WHEN 비멤버가 `project:{id}` join을 시도하면 THE SYSTEM SHALL ack `{ok:false, code:"NERV_FORBIDDEN"}`을 반환하며 룸에 넣지 않는다 | 비멤버 join 후 이벤트 미수신 확인 |
...
| REQ-API-014 | WHILE SSE 스트림이 열려 있는 동안, THE SYSTEM SHALL 25초 주기의 **`event: ping` 메시지**(`data: {}`)를 송신해 프록시 유휴 타임아웃을 방지한다 — 코멘트 라인이 아니다(2026-09-07 정정) | 60초 무이벤트 구간에서 `ping` 타입 2회 이상 수신 |
...
| REQ-API-111 | WHEN 사람 전용 동작(프로젝트 설정·보관·복구 · 스펙 메타·보관·복구 · 기준선 생성 · steer · 받은 요청 조회·결정 · **질문 답변 · 게이트 면제 · 프로젝트 받은 요청**(2026-09-07 · REQ-API-123))이 요청되면 THE SYSTEM SHALL **도메인 서비스에서** 주체가 에이전트인지 판정해 `NERV_HUMAN_ONLY` 로 거절하고 `details.action` 과 대신 갈 `web_url` 을 함께 준다 — 판정이 표면에만 있으면 새 표면이 생길 때 조용히 열린다(2026-09-05) | 라우트 **10종** 각 1건(2026-09-07 개정) · 게이트를 서비스에서 빼면 전부 실패 |
...
| REQ-API-125 | WHEN 멤버십을 세거나 수신자를 고르는 판정이 조직 단위 멤버십(`project_id IS NULL`)을 포함하면 THE SYSTEM SHALL **그 프로젝트가 속한 조직의 것만** 센다 — 알림 수신자(지정 역할 큐·기본 admin·planner 큐)와 자기 승인 소규모 완화의 인원 count 가 그 규칙 밖에 있었다 | 다른 조직의 조직 단위 planner 가 있는 상태에서 `question.created` 를 파생시키면 그 사람의 `notification` 은 0행이고 같은 조직의 planner 는 1행 이상 |
| REQ-API-126 | WHEN 목록 질의의 어휘 인자(`EP-QST-01` status · `EP-APR-01` state · `EP-NTF-01` state · `EP-CMT-01` status · `EP-SPEC-18` direction · `EP-EVT-01` type)에 어휘 밖의 값이 오면 THE SYSTEM SHALL 400 `invalid_input` 으로 거절하고 허용 목록을 준다 — **기본값으로 접지 않는다**. 판정은 도메인 서비스에 있고 시그니처는 `string` 이다(리터럴 유니온은 HTTP·MCP 호출자를 막지 못한다) | 여섯 자리 각 400 + `details.allowed` · `questions?status=cancelled` 가 취소된 것만 준다(열린 목록이 아니다) · `EP-SPEC-18` 은 없는 스펙에도 400(409 가 아니다) |
| REQ-API-127 | WHEN 클레임이 회수·해제되면(만료 · 세션 종료 · 중단 · stale · 사람 해제) THE SYSTEM SHALL `claim.released` 를 남기고, Task 가 `ready` 로 돌아갔으면 `task.ready` 를 **함께** 남긴다 — 액터는 그 클레임을 쥐고 있던 세션이다. WHEN 세션이 `stale` 로 전이되면 THE SYSTEM SHALL 그 세션의 활성 클레임을 `stale` 사유로 회수한다(리스 만료를 기다리지 않는다). WHEN 요청한 `lease_seconds` 가 상한을 넘으면 THE SYSTEM SHALL 거절한다(조용히 줄이지 않는다) | 만료 회수 뒤 이벤트 둘·액터가 그 세션 · 리스가 남아 있어도 stale 이면 회수되고 사유가 `stale` · 상한 초과는 400 |
| REQ-API-128 | WHEN 겹침 `block` 으로 클레임이 거부되면 THE SYSTEM SHALL `claim.conflict_blocked` 를 **클레임 롤백과 무관하게** 남기고 먼저 잡고 있던 쪽에 알린다. WHEN 결재가 결정되면 THE SYSTEM SHALL `approval.decided` 를 남긴다(대상이 질문이어도 `question.answered` 를 쓰지 않는다). WHEN 게이트가 판정에 실패해 통과시키면 THE SYSTEM SHALL `gate.failopen` 을 남긴다. WHERE 승인이 자동 통과라 승인자가 없으면 THE SYSTEM SHALL 이벤트 액터에 **제출한 주체**를 적는다 | 차단 뒤 이벤트 1건·알림은 선점자에게 · 결정 뒤 `approval.decided` 1건이고 `question.answered` 0건 · 자동 통과의 액터가 제출자이고 승인자는 NULL |
| REQ-API-129 | WHEN 세션(에이전트) 경로가 `in_progress`·`in_review`·`done` 으로 전이를 시도하면 THE SYSTEM SHALL 그 세션이 쥔 살아 있는 클레임이 없는 한 409 `NERV_LEASE_EXPIRED`(`kind` 는 `no_active_claim` 또는 `lease_expired`, `reclaimable` 동반)로 거부한다 | 클레임 없는 세션의 done 거부·`reclaimable` 참 · 만료 리스 거부 · 고아 `in_progress` 는 `reclaimable` 거짓 |
| REQ-API-130 | WHEN 사람 경로가 `done` 으로 전이를 시도하면 THE SYSTEM SHALL 호출자가 활성 클레임 보유자·`assignee_user_id`·planner·admin 중 하나가 아닌 한 403 `NERV_FORBIDDEN`(`not_assignee`)로 거부한다 | 무관한 developer 거부 · 담당자 통과 · planner 통과 |
| REQ-API-131 | WHEN `ready` 로 전이를 시도하면 THE SYSTEM SHALL 위임 명세 4요소(임포트 자리표시자는 **빈 것**이다) 누락 이름과 미해소 `blocks` 의존 키를 `details` 에 실어 409 로 거부하고, 통과하면 `blocked_reason`·`delegate_session_id` 를 지우고 `task.ready` 를 발행한다 | 누락 필드 이름 반환 · 자리표시자 거부 · 미해소 의존 키 반환 · 통과 시 사유 삭제와 이벤트 |
...
| REQ-API-179 | WHEN 세션 목록(EP-SES-01)을 돌려주면 THE SYSTEM SHALL 활성 클레임이 없는 세션에는 마지막으로 쥔 작업의 키·제목과 그 클레임이 끝난 방식(상태·해제 사유)을 싣고, `awaiting_input` 세션에는 그 세션의 열린 질문(id·제목)을, 열린 질문이 없으면 그 세션이 올린 결정 전 결재의 id 를 싣는다 |
| REQ-API-180 | WHEN 작업 목록(EP-TASK-01)을 돌려주면 THE SYSTEM SHALL 활성 클레임이 에이전트 세션의 것이면 그 세션의 기계·에이전트 종류·상태를 싣는다. WHEN 작업 상세(EP-TASK-04)를 돌려주면 THE SYSTEM SHALL 담당의 이름을 싣는다. WHEN 발견 목록(EP-REV-03)을 돌려주면 THE SYSTEM SHALL 발견마다 그 리뷰가 이어진 작업의 키와 그 발견을 올린 작업의 키를 싣고, `branch` 가 오면 마지막 라운드의 브랜치로 목록과 facet 을 함께 거른다. WHEN 이미 올린 발견을 다시 올리면(EP-REV-07) THE SYSTEM SHALL 새로 만들지 않고 이미 만든 작업의 키를 돌려준다 |
| REQ-API-181 | WHEN 이벤트 피드(EP-EVT-01)나 알림 목록(EP-NTF-01)을 돌려주면 THE SYSTEM SHALL 이벤트의 대상을 따라가 — 결재는 그 스펙 버전·플랜의 작업·발견으로, 질문은 그 제목과 붙은 작업·스펙으로, 클레임·리뷰 세션은 그 작업으로 — 스펙의 키·제목·버전, 작업의 키·제목, 발견의 식별자·제목, 질문의 제목, 리뷰 브랜치, 대상 세션의 기계·에이전트 종류를 두 목록이 같은 규칙으로 싣는다. WHEN 이벤트 피드를 돌려주면 THE SYSTEM SHALL 행위자의 사용자·세션 식별자를 싣는다 |
...
| REQ-API-188 | WHEN 스펙 버전을 제출하면(EP-SPEC-10 · `nerv_spec_submit_review`) THE SYSTEM SHALL 게이트 판정의 4축 합계(`gate_score`)·축별 점수(`gate_axes`)·티어를 올린 신호(`gate_signals` — 발동한 것만)를 `spec.submitted` 와 슬롯마다의 `approval.requested` 에 싣고, WHEN 받은 요청 목록(EP-APR-01 · EP-APR-05)을 돌려주면 THE SYSTEM SHALL 스펙 결재 카드에 그 셋을 싣는다 — 그 칸이 생기기 전의 요청은 비워 둔다 |
...
| REQ-API-135 | WHEN 질문 응답(생성·폴링·역채널)을 돌려주면 THE SYSTEM SHALL `answered_by`(표시 이름)와 `answered_at` 을 함께 싣는다 | 답변 뒤 두 값이 응답과 하트비트에 있다 · 새 질문에서는 둘 다 null |
| REQ-API-136 | WHEN 결재를 승인하려는 사람이 그 대상의 요청자·작성자·작성 세션 소유자 중 하나이면 THE SYSTEM SHALL 403 `NERV_FORBIDDEN`(`self_approval` · `self_kind`)로 거부한다. WHERE 그 카드를 승인할 수 있는 다른 사람이 0명이거나 호출자가 admin 이면 THE SYSTEM SHALL 허용하고 그 사실을 이벤트에 남긴다 | 남이 제출한 초안을 작성자가 승인하면 `self_kind='author'` · 세션 소유자는 `session_owner` · 거절·코멘트는 허용 |
| REQ-API-137 | WHEN 받은 요청 목록(EP-APR-01)을 조회하면 THE SYSTEM SHALL 지정 승인자·직군 슬롯·기본 큐·admin 중 하나에 해당하는 카드만 반환하고, 각 카드에 `can_approve` 와 그 이유를 싣는다 | viewer·qa 의 목록은 0건 · 직군 슬롯은 그 직군에게만 · 이유가 카드에 실린다 |
| REQ-API-138 | WHERE 결재 행에 직군 슬롯(`assignee_role`)이 있으면 THE SYSTEM SHALL 그 역할 보유자와 admin 에게만 결정을 허용하고 나머지에게는 403(`not_in_role_queue`)을 반환한다 | designer 슬롯을 planner 가 내리면 403 · designer 는 통과 |
| REQ-API-139 | WHEN 스펙 검토 요청(EP-SPEC-10)이 오면 THE SYSTEM SHALL 호출자가 그 버전의 작성자이거나 planner·admin 이 아닌 한 403 `NERV_FORBIDDEN`(`not_author`)로 거부한다 | 무관한 developer 는 403 이고 문서는 draft 그대로 · planner 는 남의 초안도 제출 · 작성자 본인은 역할과 무관하게 통과 |
| REQ-API-140 | WHEN 티어가 요구하는 승인자 수가 2 이상인 스펙이 제출되면 THE SYSTEM SHALL 그 수만큼 결재 슬롯을 만들고(둘째부터는 문서 타입의 직군 큐, 그 역할의 멤버가 없으면 기본 큐로 강등), WHEN 결정이 오면 THE SYSTEM SHALL 이번 라운드에 `approve` 를 낸 **서로 다른 사용자** 수가 필요 수에 닿을 때만 문서를 전이하고 응답에 `quorum` 을 싣는다. WHERE 승인 가능한 사람이 필요 수보다 적으면 THE SYSTEM SHALL 그 수만큼만 세우고 `quorum_relaxed` 를 남긴다. 서로 다른 사용자는 **결정한 사람**으로 센다. WHEN 라운드가 거절·코멘트로 닫혀 결정되지 않은 슬롯이 남은 채 다시 제출되면 THE SYSTEM SHALL 그 슬롯을 이번 라운드의 요청으로 다시 찍어(요청 시각 · 요청자) 필요 수에 넣는다(2026-09-26) | T3 제출은 슬롯 2 · 첫 승인은 `in_review` 유지와 `satisfied:false` · 둘째 승인에 approved 와 `spec.approved` 1건 · 같은 사람의 둘째 승인은 403 `already_approved` · 동시 승인에도 전이 1회 · 거절 뒤 재제출은 0부터 |
...
| REQ-API-146 | WHERE 프로젝트 정책 `done_gate.evidence_source` 가 `ci_or_human` 이면 THE SYSTEM SHALL 에이전트가 올린 증적을 세지 않고, WHERE `done_gate.review_coverage` 가 참이면 THE SYSTEM SHALL 그 Task 를 지난 리뷰 라운드와 열린 critical 0 을 함께 요구한다. WHERE 그 Task 에 게이트 면제 결재가 있으면 THE SYSTEM SHALL 리뷰 조건을 넘어간다 | 기본은 오늘과 같다 · `ci_or_human` 에서 agent 증적만이면 거부 · 리뷰 없으면 거부 · 면제는 통과 |
| REQ-API-147 | WHEN 증적이 등록되면 THE SYSTEM SHALL `locator` 의 형식을 검사한다(커밋은 7~40 hex, PR 은 절대 URL, 그 밖은 비어 있지 않을 것) — 어긋나면 400 `invalid_input`(`field: locator`) | `commit` 에 문장을 넣으면 400 |
...
| REQ-API-158 | WHEN 프로젝트를 읽으면 THE SYSTEM SHALL `repo_host` 를 함께 주고, 값이 없는 행은 `github` 으로 읽는다(`NOT NULL DEFAULT`). WHEN 프로젝트 설정이 `repo_host` 를 받으면 THE SYSTEM SHALL 어휘(`REPO_HOSTS`) 안의 값만 저장하고 밖의 값은 400 `NERV_PRECONDITION` 으로 거절하며, **보내지 않으면** 지금 값을 건드리지 않는다. WHERE 이 값이 쓰이는 곳이면 THE SYSTEM SHALL 주소의 **모양**만 정한다 — 서버는 이 값으로 어떤 저장소에도 접속하지 않는다 | 기본 github · gitlab 저장 · 어휘 밖 400 · 안 보내면 유지 |
| REQ-API-159 | WHEN 알림 목록(EP-NTF-01)을 주면 THE SYSTEM SHALL 대상이 스펙 버전인 알림에 그 버전의 `version_no` 를 함께 싣고, WHERE 대상이 스펙 버전이 아니면 `null` 로 준다 — **추정해 채우지 않는다**(있지도 않은 diff 로 화면을 보내게 된다) | 승인 알림 v2 · 재검토 알림 null |
| REQ-API-160 | WHEN 토큰을 발급하면(EP-TOK-02) THE SYSTEM SHALL 응답에 **그 토큰이 묶인 프로젝트**(slug·이름)와 이름·권한·만료를 원문과 함께 싣는다 — 원문은 이 응답에서 한 번만 나가고 목록에는 접두만 남으므로, 그 한 번이 자기를 설명하지 않으면 받은 사람은 **무엇에 쓰는 토큰인지 되짚을 길이 없다**. WHEN admin 이 조직 전체 토큰을 조회하면(EP-TOK-04) THE SYSTEM SHALL 각 행에 프로젝트 slug 와 **이름**을 함께 싣는다(slug 만으로는 표가 사람 말로 읽히지 않는다) | 발급 응답의 `project.slug` = 그 토큰의 `project_id` 가 가리키는 프로젝트 |
| REQ-API-161 | WHEN 요청을 **그 표면에서는 할 수 없다**는 이유로 거절하면 THE SYSTEM SHALL 거절과 함께 **어디로 가면 되는지**를 응답에 싣는다 — 메타 변경(`meta_change_not_allowed`)은 그 문서의 `web_url` 이다. 거절만 남기면 부르는 쪽에 남는 선택지가 없고, 에이전트는 사람에게 청할 주소조차 갖지 못한다 | 메타를 바꾼 `nerv_spec_draft_upsert` 의 409 가 `web_url` 을 담는다 |
...
| REQ-API-166 | WHEN 받은 요청 목록(EP-APR-01)을 조회하면 THE SYSTEM SHALL `{items, next_cursor, total}` 봉투로 답하고 `cursor`·`limit` 를 읽으며, **대기는 요청이 오래된 순**(`requested_at ASC, id` — 2026-09-25 개정: 그 앞에 **누를 수 있는가**가 첫째 키로 선다, REQ-API-184) · **처리됨은 결정이 최근인 순**(`decided_at DESC, id`)으로 정렬한다. `total` 은 쪽이 아니라 **같은 조건의 전체 수**다 — 배지와 목록이 다른 수를 세면 지울 수 없는 숫자가 남는다([4.5](screens.md) REQ-WEB-035). 대기 목록은 결재와 질문 두 소스를 **같은 정렬 키로** 병합한다 — 전표는 처음부터 `Page<ApprovalCard>` 라 적었는데 서비스는 맨 배열에 `LIMIT 100` 이었고, 그 상한이 **오래 기다린 쪽**을 잘랐다(2026-09-24 실측: 120건 중 가장 오래 기다린 20건이 목록에서 통째로 빠졌다 — 화면이 존재하는 이유를 상한이 뒤집고 있었다) |
| REQ-API-167 | WHEN 프로젝트 소속 받은 요청(EP-APR-05)을 조회하면 THE SYSTEM SHALL **전역 목록(EP-APR-01)을 그 프로젝트로 좁힌 것**을 같은 봉투·같은 열로 반환한다 — 질문을 함께 싣고, 보관한 프로젝트는 비우며, `state`·`cursor`·`limit` 를 같게 읽는다. 질의가 두 벌이던 동안 **같은 질문에 다른 답**을 냈다(2026-09-24 실측 · 결재 3 + 질문 2): 프로젝트 3건(결재만) · 전역 5건, 보관한 뒤 프로젝트 3건 · 전역 0건, 그리고 카드가 필요로 하는 `waiting_seconds`·`project_slug`·`spec_key` 가 없어 **같은 컴포넌트가 그릴 수 없었다** |
...
| REQ-API-153 | WHEN MCP 표면이 사용자 생성 본문(`nerv_spec_get` 의 `body_md`·`requirements[].statement_md`·`comments[].body_md`)을 돌려주면 THE SYSTEM SHALL 필드 값 **전체**를 `<nerv:spec id version trust="untrusted">…</nerv:spec>` 경계로 감싸고, 스펙이 아닌 사용자 생성 텍스트(`nerv_spec_search` 의 `snippet` · 질문 답변과 하트비트 `pending[]` 의 `answer_md`)는 `<nerv:text kind="…" trust="untrusted">` 로 감싼다. WHERE 표면이 REST 또는 markdown 미러이면 THE SYSTEM SHALL 감싸지 않는다 — 사람과 화면이 읽는 곳에서 태그는 본문이 된다. WHERE 대상이 사람의 지시(하트비트 `instructions`)이면 THE SYSTEM SHALL 감싸지 않는다 — 그것은 따라야 할 것이다. WHEN `nerv_spec_draft_upsert`·EP-SPEC-08 의 본문이 그 경계로 통째로 감싸여 오면 THE SYSTEM SHALL 400 `NERV_PRECONDITION`(`kind:"invalid_input"` · `field:"body_md"` · `reason:"wrapped_body"`)으로 거절한다 — 조용히 벗기면 그 경계는 아무것도 뜻하지 않게 된다 | untrusted.spec L1 6건 · mcp.spec L2 4건 · REST·미러 미래핑 2건 |
...
| REQ-API-097 | WHEN 파생 Task 가 클레임되거나 전이하거나 그 요구사항에 증적이 등록되면 THE SYSTEM SHALL 그 Requirement 의 `impl_status` 를 **다시 파생한다** — 하나 이상이 `claimed`/`in_progress`/`in_review` 면 `in_progress`, 전부 `done` **이고** 증적(요구사항 또는 그 파생 Task 에 붙은 것 — 낡은 것(`stale`)은 빼고) 1건 이상이면 `implemented`, `done` 이 있지만 아직 전부가 아니거나 증적이 없으면 `in_progress`, 그 밖에는 `unimplemented` 다. WHILE 이미 `verified` 인 동안 THE SYSTEM SHALL 값을 바꾸지 않는다(한번 선 검증은 발견이 열려도 내리지 않는다 · REQ-API-141) — 문장이 바뀌어 지금 문장의 서명이 없을 때만 다시 파생한다(REQ-API-190). WHILE 파생 Task 가 하나도 없는 동안 THE SYSTEM SHALL 값을 바꾸지 않는다 — 임포터가 문서에서 읽어 넣은 값을 쓸어버리지 않기 위해서다 |
...
| REQ-API-021 | WHEN 기존 `spec_id`를 지정한 `nerv_spec_draft_upsert`에 현재 값과 다른 `parent_id`/`type`/`title`이 오면 THE SYSTEM SHALL 409 `NERV_PRECONDITION`(`details.kind="meta_change_not_allowed"`)을 반환하고 본문도 저장하지 않으며, **그 문서의 `web_url` 을 함께 싣는다**(2026-09-22 개정 · REQ-API-161). WHEN 같은 값이 오면 THE SYSTEM SHALL 정상 처리한다 | 다른 title 1케이스 + 동일 메타 재호출 1케이스 |
...
| REQ-API-025 | WHEN 검색 질의에 고정 ID 가 통째로 들어 있으면 THE SYSTEM SHALL **키 정확 일치**(`spec.key`·`requirement.ref`)로 그 리소스를 최상위 직행 반환하고, 그 외 질의는 렉시컬+벡터 RRF 병합 순위와 `related[]` 분리 그룹으로 응답한다(§2.2b) — REST와 MCP 두 표면의 결과가 동일하다. 접두를 규칙으로 삼지 않는다: 표시 키의 접두는 프로젝트가 정하는 값이고 형식의 정본은 [데이터 모델](../03-proposal/data-model.md) §5.1 이다(2026-09-07 개정 — 앞선 `SPC-`·`REQ-`·`TSK-` 접두 규칙을 대체한다) | 현행 형식 키(`<PRJ>-S-…`) 질의·한국어 질의·의미 질의 각 1건을 두 표면에서 실행해 대조 |
