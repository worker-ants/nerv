...
>
> 문서 버전 v0.72 · 2026-09-05 · HTML 판: [api.html](../html/api.html)
>
...
| ID | 메서드 · 경로 | 권한 | 요청 | 응답 | 발생 이벤트 |
| --- | --- | --- | --- | --- | --- |
| EP-SPEC-01 | `GET /api/v1/projects/{proj}/specs/tree` | 전 역할(`spec:read`) | `SpecTreeQuery`(root, depth, status, include_archived — 기본 false, REQ-API-022 · root·depth 는 REQ-API-090. **`status` 는 아직 어느 표면도 소비하지 않는다** — 2026-09-05 확인, 빼거나 구현할지는 사람의 결정을 기다린다) | `SpecTreeResult`(id·title·type·문서 상태·현재 버전·`archived_at`) | — |
