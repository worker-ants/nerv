...
>
> 문서 버전 v1.54 · 2026-09-24 · HTML 파생본: [api.html](../html/api.html)
>
...
| EP-SPEC-10 | `POST /api/v1/projects/{proj}/spec-versions/{ver}/submit` | 작성자 본인 또는 planner·admin — **2026-09-07 부터 실제로 판정한다**(REQ-API-139 · 그전에는 `spec:draft` 만 봐서 아무나 남의 초안을 제출할 수 있었고, 그것이 자기 승인의 앞문이었다) | **본문 없음** — `reviewer_hint`·`note` 는 2026-09-03 에 걷었다(리뷰어 지정은 게이트가 §6.3 규칙으로 정하고, 변경 요약은 초안의 `change_summary_md` 가 이미 나른다). 이 행이 그 뒤로도 둘을 적고 있었고 2026-09-05 에 스키마 대조 검사가 짚었다 | `SpecSubmitResult`(approval_id[], 지정 리뷰어·SLA) — **세션이 올린 T2·T3 제출은 그 세션을 `awaiting_input` 으로 세운다**(2026-09-07 · REQ-API-134). 자동 통과(T0·T1)와 사람 제출은 세션을 건드리지 않는다 | `spec.submitted` + `approval.requested` |
...
| EP-NTF-01 | `GET /api/v1/me/notifications` | 본인 | `NotificationListQuery`(state: unread/read, **`before`·`limit`** — 커서 이름은 `before` 다. 값은 불투명하고 `(created_at, id)` 를 담는다 · REQ-API-124) | `Page<NotificationResult>` — 대상이 스펙 버전이면 `version_no` 를 함께 싣는다(아니면 `null` · REQ-API-159) | — |
