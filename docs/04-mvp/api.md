...
>
> 문서 버전 v1.61 · 2026-09-24 · HTML 파생본: [api.html](../html/api.html)
>
...
- **배지가 있는 자라는 목록은 셋을 준다**(2026-09-24 · REQ-API-166): `{items, next_cursor, total}`. 받은 요청(EP-APR-01)이 그 하나다 — 쪽을 나누는 순간 "목록 길이 = 전체 수" 가 깨지는데 그 수를 화면 세 자리가 쓴다(헤더 배지 · 홈의 인사 · "받은 요청 전체 N건"). 별도 엔드포인트로 세면 출처가 둘이 되고, **배지와 목록이 어긋나면 지울 수 없는 숫자가 남는다** — 알림 배지에서 이미 겪은 자리다([4.5](screens.md) REQ-WEB-035). 같은 질의에서 같은 조건으로 센다.
...
| REQ-API-166 | WHEN 받은 요청 목록(EP-APR-01)을 조회하면 THE SYSTEM SHALL `{items, next_cursor, total}` 봉투로 답하고 `cursor`·`limit` 를 읽으며, **대기는 요청이 오래된 순**(`requested_at ASC, id`) · **처리됨은 결정이 최근인 순**(`decided_at DESC, id`)으로 정렬한다. `total` 은 쪽이 아니라 **같은 조건의 전체 수**다 — 배지와 목록이 다른 수를 세면 지울 수 없는 숫자가 남는다([4.5](screens.md) REQ-WEB-035). 대기 목록은 결재와 질문 두 소스를 **같은 정렬 키로** 병합한다 — 전표는 처음부터 `Page<ApprovalCard>` 라 적었는데 서비스는 맨 배열에 `LIMIT 100` 이었고, 그 상한이 **오래 기다린 쪽**을 잘랐다(2026-09-24 실측: 120건 중 가장 오래 기다린 20건이 목록에서 통째로 빠졌다 — 화면이 존재하는 이유를 상한이 뒤집고 있었다) |
