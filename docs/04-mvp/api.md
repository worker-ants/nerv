...
>
> 문서 버전 v1.58 · 2026-09-24 · HTML 파생본: [api.html](../html/api.html)
>
...
| EP-NTF-01 | `GET /api/v1/me/notifications` | 본인 | `NotificationListQuery`(state: unread/read, **`before`·`limit`** — 커서 이름은 `before` 다. 값은 불투명하고 `(created_at, id)` 를 담는다 · REQ-API-124) | `Page<NotificationResult>` — 대상이 스펙 버전이면 `version_no` 를 함께 싣는다(아니면 `null` · REQ-API-159). 결재 요청·질문 알림은 그 요청이 닫혔으면 `resolution`(`approve`·`reject`·`comment`·`answered`·`cancelled`)과 `resolved_by` 를 싣는다(REQ-API-176) | — |
