...
>
> 문서 버전 v1.33 · 2026-09-20 · HTML 파생본: [api.html](../html/api.html)
>
...
- **오리진 검증은 `/api/auth/*` 가 한다.** 쿠키를 실은 상태 변경 요청의 `Origin` 이 목록 밖이면 `403` 이다(better-auth). `/api/v1` 의 CSRF 방어선은 브라우저의 CORS 와 `SameSite=Lax` 이며, 서버가 그 표면에서도 `Origin` 을 대조할 것인가는 **아직 결정하지 않았다**([4.2](codebase.md) §5.2c 의 "한계").
