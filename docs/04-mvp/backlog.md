...
| E14-S04 | **1·2단계** — ① `NERV_WEB_URL`·`NERV_API_URL` 둘 신설, 옛 이름 걷기와 **기동 거부**(`common/origins.ts` · `origins.spec.ts` · REQ-CB-036·037), 여섯 소비자 배정, compose·k8s 두 키, 전표 게이트가 걷힌 이름을 센다 ② 허용 오리진 **한 목록**(`allowedOriginsFromEnv` = `NERV_WEB_URL` + `NERV_TRUSTED_ORIGINS`)을 CORS(`@fastify/cors`)·better-auth·`/mcp` 가드가 함께 본다(REQ-CB-041 — 2026-09-14 에 적어 둔 가드의 비대칭이 여기서 닫혔다), 세션 쿠키의 `Domain` 손잡이와 틀린 값의 기동 거부(`NERV_COOKIE_DOMAIN` · REQ-CB-042), better-auth 의 테스트 모드 오리진 검증 끄기를 **명시로 껐다**(그 전에는 L2 가 방어선을 끈 채 초록을 봤다), L2 가 브라우저 없이 프리플라이트 응답·`Set-Cookie` 의 `Domain`·목록 밖 오리진의 거절을 센다(`cors-cookie.spec.ts`) | **3·4단계**([4.1](scope.md) §2.3) — ③ 웹의 런타임 설정 로더(`import.meta.env` 사용 0건 · `api.ts`·`session.ts`·`ws.ts` 가 여전히 상대 경로다) ④ DNS·인증서·Ingress host·CDN 과 `base/web/` 을 오버레이로 내리는 일, `plugin/hooks/hooks.http.json` 의 하드코딩된 주소(`plugin.json` 버전과 함께). 여기에 **앞문 nginx 의 한계**가 딸려 있다: `map` 은 오리진 **하나**(`NERV_WEB_URL`)만 받는다 — envsubst 가 변수를 한 줄로만 치환하고 같은 값이 두 줄이면 nginx 가 기동을 거부한다. 최종 강제는 앱 가드라 실해는 없고(REQ-CB-013), 두 오리진을 앞문도 보게 하려면 `map` 대신 다른 판정이 필요하다 |
