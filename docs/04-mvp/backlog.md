...
>
> 문서 버전 v0.85 · 2026-09-20 · HTML 파생본: [backlog.html](../html/backlog.html)
>
...
| E14-S04 | **1·2·3단계 + 4단계의 k8s 지형** — ① `NERV_WEB_URL`·`NERV_API_URL` 둘 신설, 옛 이름 걷기와 **기동 거부**(`common/origins.ts` · `origins.spec.ts` · REQ-CB-036·037), 여섯 소비자 배정, compose·k8s 두 키, 전표 게이트가 걷힌 이름을 센다 ② 허용 오리진 **한 목록**(`allowedOriginsFromEnv` = `NERV_WEB_URL` + `NERV_TRUSTED_ORIGINS`)을 CORS(`@fastify/cors`)·better-auth·`/mcp` 가드가 함께 본다(REQ-CB-041 — 2026-09-14 에 적어 둔 가드의 비대칭이 여기서 닫혔다), 세션 쿠키의 `Domain` 손잡이와 틀린 값의 기동 거부(`NERV_COOKIE_DOMAIN` · REQ-CB-042), better-auth 의 테스트 모드 오리진 검증 끄기를 **명시로 껐다**(그 전에는 L2 가 방어선을 끈 채 초록을 봤다), L2 가 브라우저 없이 프리플라이트 응답·`Set-Cookie` 의 `Domain`·목록 밖 오리진의 거절을 센다(`cors-cookie.spec.ts`) ③ 화면이 API 주소를 **런타임 설정**에서 읽는다(`lib/config.ts` · 앞문의 `location = /config.json` · REQ-CB-044) — 주소를 쓰는 자리 넷(REST·`/api/auth/*`·WebSocket·첨부 업로드)이 그 값을 쓰고, **파일이 없으면 같은 오리진**이라 개발 루프와 한 호스트 배치는 그대로다. L1 이 폴백 전수를, L3 가 앞문의 실물 응답을 센다 ④-a **k8s 지형**(REQ-CB-045) — base 는 `api.` 호스트의 표면 여섯만 갖고, 화면의 `/` 규칙과 웹 파드는 `base/web/` 한 덩어리로 base resources 밖에 섰다(CDN 배치는 안 더한다). 오버레이는 JSON6902 로 호스트 문자열만 바꾸고, ConfigMap 의 주소 둘이 그 호스트와 갈린다. 전환 절차·되돌리는 길은 [4.2](codebase.md) §6.3a. **게이트가 렌더 결과를 센다**(REQ-CB-046) — 그 전에는 문법만 봤고 두 오버레이의 Ingress 경로가 0개였다 | **4단계의 플러그인 절반**([4.1](scope.md) §2.3) — `plugin/hooks/hooks.http.json` 6곳·`managed-settings.example.json` 6곳·`codex/config.toml` 1곳·`marketplace.json` 의 주소를 `api.` 로 옮기고 **`plugin.json` 버전과 함께** 배달하는 일(REQ-PLG-017 · 매뉴얼 ko·en 10곳도 같이). 실제 DNS·인증서 전환은 운영 주체의 몫이고 절차는 [4.2](codebase.md) §6.3a 가 적는다. 여기에 **앞문 nginx 의 한계**가 딸려 있다: `map` 은 오리진 **하나**(`NERV_WEB_URL`)만 받는다 — envsubst 가 변수를 한 줄로만 치환하고 같은 값이 두 줄이면 nginx 가 기동을 거부한다. 최종 강제는 앱 가드라 실해는 없고(REQ-CB-013), 두 오리진을 앞문도 보게 하려면 `map` 대신 다른 판정이 필요하다 |
