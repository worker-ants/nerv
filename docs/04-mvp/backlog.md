...
>
> v1.21 변경(2026-09-24 — 스토리 없이 들어온 수정 하나, **사람 결정**): §1.4 셋째 표에 **멤버 표가 사람을 흩었다** 한 줄([4.5](screens.md) REQ-WEB-194). 스토리 수·`done` 수는 그대로다. 같은 줄에 **조직 수준 조작도 조직 admin 만**([4.4](api.md) REQ-API-171)을 더했다.
>
...
| 멤버 표가 사람을 흩었다 | `apps/web/src/routes/settings/members.tsx`(`groupByPerson` · 상속 칩) · `lib/document-title.ts`(신설)·`components/app-shell.tsx`(탭 제목) · L1 `settings-scope.spec.tsx` 2건(수정 전 2건 실패) · `document-title.spec.ts` 3건 · [3.6](../03-proposal/ui-wireframes.md) §2.8 주석 정정 · `apps/api/src/modules/auth/auth.service.ts`(조직 수준 조작 — `assertOrgAdmin`·`createProject` 가 `assertCanManageScope`) · `settings/workspace.tsx`(조직 수준은 조직 admin, 프로젝트 줄은 그 프로젝트의 admin) · L2 `scope-admin.spec.ts` 3건 · `workspace.spec.tsx` 2건 | 범위마다 한 줄이라 같은 사람이 표 여기저기 흩어졌고, 프로젝트 줄은 조직 전체 역할을 숨겼다. 사람마다 한 묶음 + 상속 표식으로 모았다. 탭 제목이 늘 "NERV" 라 탭을 여럿 열면 가를 길이 없었다([4.5](screens.md) REQ-WEB-194). 조직·프로젝트 경계 점검의 PR 5(결정 2) — 이것으로 다섯이 끝났다. 곁들여 조직 이름·삭제·새 프로젝트도 조직 admin 만으로 좁혔다 — 한 프로젝트의 admin 이 조직 이름을 바꾸고 프로젝트를 늘릴 수 있었다([4.4](api.md) REQ-API-171 · 수정 전 코드에서 L2 3건 · L1 2건 실패) |
