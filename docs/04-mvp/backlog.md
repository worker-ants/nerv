...
| 작업이 다음 상태로 가는 문이 없었다 | `apps/web/src/features/task-board/next-actions.ts`(신설 — 상태 → 다음 행동 표) · `routes/p.$proj/tasks.$task.tsx`(머리의 단추 · [클레임]은 준비됨·리스 지난 것만 · [막힘 풀기] · 완료 폼은 진행 중이거나 [완료…] · 스펙 영향 고르지 않음에서 시작 · 증적 없음 미리 말함 · 위임 명세 [고치기]·❌ · 막힘 표시 제 카드) · `tasks.index.tsx`(4요소가 찬 backlog 카드의 [준비됨으로 올리기] · 늘 잠긴 "ready 전이" 걷음 · [+ 새 작업]·[채우기] 역할 잠금) · `features/task-board/delegation-form.tsx`(열리면 스크롤·포커스 · 안내가 생성 경로와 맞음 · 상세 무효화) · `lib/format.ts`(작업·클레임 상태 · 리뷰 종류·상태 · 증적 종류 라벨)·`sessions.$session.tsx`·`requirement-panel.tsx` · `packages/schema/src/scopes.ts`(`TASK_CREATE_ROLES`·`TASK_EDIT_ROLES` — 컨트롤러 가드와 화면이 같이 씀)·`constants.ts`(`DELEGATION_PLACEHOLDER_TEXTS`) · `apps/api/src/modules/task/task.service.ts`(목록 `delegation_complete` 를 `filledSql` 로) · L2 `rest-surface.spec.ts` 자리표시자 1건(옛 SQL 에서 실패) · L1 `next-actions.spec.ts` 12건 · `task-next-door.spec.tsx` 12건(수정 전 코드에서 모두 실패) · 매뉴얼 ko·en `tasks.md`("다음 단계는 작업 상세의 머리에 있습니다" · 없는 [진행 중] 단추를 실제 이름으로 · 상태 표에 화면 이름) | 웹에서 4요소를 다 채워 만든 작업이 backlog 를 나갈 문이 없었고, 작업 상세의 전이 단추가 상태를 보지 않아 막힘을 풀 단추도 클레임한 작업을 진행·검토로 옮길 단추도 없었으며, 완료 게이트의 "스펙 영향 없음" 이 미리 체크돼 있었다. 위임 명세는 읽기 전용이었고, 단추는 역할을 보지 않아 폼을 다 채운 뒤에야 403 을 알았으며, 같은 상태가 화면마다 코드값과 라벨로 갈렸다. 보드 목록은 임포트 자리표시자를 찬 것으로 셌다([4.5](screens.md) REQ-WEB-202·203 · [4.4](api.md) REQ-API-175). UI/UX 검토의 넷째 묶음(P04 — WORK-01·02·09·14·X2·X3). |
