...
>
> v0.48 변경(2026-09-07 — 여섯째 스프린트 ①): §1.4 셋째 표에 **임포터의 abort 게이트 · 규칙 전표 대조** 한 줄([4.7](importer.md) REQ-IMP-023~026).
>
...
| 임포터의 abort 게이트 · 규칙 전표 대조 | `cli/src/run.ts`(`halted`) · `report/index.ts`(`RULES`·`hintFor`·`withHints`) · `index.ts`(`failureReport`) · `parse/git.ts`(`headOf`) | 중단이라 적어 놓고 plan·review 는 그대로 전송했다 · 전표와 코드의 등급이 세 자리 갈렸다 · `hint` 는 채우는 코드가 0곳이었다 · `root_commit` 은 `null` 고정이었다([4.7](importer.md) REQ-IMP-023~026) |
