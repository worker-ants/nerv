...
| MinIO 이미지를 당길 수 없게 됐다 | `deploy/compose/docker-compose.yml`·`docker-compose.e2e.yml`(서버를 `pgsty/minio:RELEASE.2026-08-04T00-00-00Z` 로 — 커뮤니티 포크, 개발·e2e 전용) · `deploy/k8s/base/backup/cronjob.yaml`(initContainer 를 `rclone/rclone:1.75.1` 로 · `MC_CONFIG_DIR` 제거) · `deploy/scripts/nerv-backup.sh`·사본(`mc mirror` → `rclone sync` · env 로만 정의한 원격 · 앱과 같은 주소 모양·리전) · `deploy/scripts/nerv-restore.sh`(첨부 개수를 rclone 으로 센다) · 실측: 옛 이미지가 쓴 볼륨을 pgsty 가 읽음 · CronJob 과 같은 조건(`postgres:18-alpine` · 읽기 전용 루트)에서 동기화·삭제 반영·rclone 없음 exit 2·틀린 자격증명 exit 1(사본 보존) · L3 통과 | MinIO 가 공개 이미지를 거둬(도커허브 2026-09-11 · quay.io 2026-09-24) e2e 가 스택을 띄우지 못했고, 운영 백업 CronJob 의 initContainer 도 당길 이미지를 잃었다. 공식 후속 `aistor/minio` 는 라이선스 없이 S3 를 거절한다(사람 결정 · [4.2](codebase.md) v1.69). 남은 것: 첨부 동기화는 여전히 CI 에서 돌지 않는다(`restore-roundtrip.spec.ts` 는 `NERV_BACKUP_SKIP_BLOBS=1` — 러너에 S3 와 rclone 이 없다). |
