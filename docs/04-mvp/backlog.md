...
>
> v1.21 변경(2026-09-24 — 스토리 없이 들어온 수정 하나, **사람 결정**): §1.4 셋째 표에 **멤버 표가 사람을 흩었다** 한 줄([4.5](screens.md) REQ-WEB-194). 스토리 수·`done` 수는 그대로다. 같은 줄에 **조직 수준 조작도 조직 admin 만**([4.4](api.md) REQ-API-171)을 더했다. 조직 전체 토큰 표도 조직 admin 만([4.4](api.md) REQ-API-172).
>
