...
>
> 문서 버전 v0.21 · 2026-09-06 · HTML 파생본: [scope.html](../html/scope.html)
>
...
| 플러그인 스킬 | 6종(MVP 약속 5종 + Phase 2 `/nerv:review` 가 2026-08-23 앞당겨 들어왔다) | §4.3 |
...
### 4.3 플러그인 스킬 범위 — MVP 약속 5종 · 패키지 6종
...
SKILL.md **6종**의 파일 전문·hooks.json·온보딩 절차는 [4.6 플러그인과 온보딩](plugin.md)이 소유한다(`.mcp.json` 은 패키지에 없다 — 2026-09-04 개정).

> **스킬 수와 도구 수가 다른 이유.** 스킬은 도구와 1:1이 아니다 — `/nerv:import`는 MCP 도구가 아니라 로컬 CLI를 실행하는 절차 스킬이다(§4.2). 플러그인이 배포하는 것은 "무엇을 어떤 순서로 하느냐"이고, 서버 권한은 여전히 PAT 권한이 정한다.
