...
>
> 문서 버전 v0.24 · 2026-09-13 · HTML 파생본: [scope.html](../html/scope.html)
>
...
| 확정 | 무엇 | 결정일 | 딸려 오는 것 |
| --- | --- | --- | --- |
| 호스트 이름 | `app.nerv.example.com`(웹) · `api.nerv.example.com`(API) | 2026-09-13 | **같은 등록 도메인 아래여야 한다.** 쿠키를 `Domain=.nerv.example.com` 으로 상위에 두면 두 호스트가 같은 사이트라 `SameSite=Lax` 가 그대로 선다. 완전히 다른 등록 도메인은 `SameSite=None; Secure` 를 강제하고, 그 순간 CSRF 방어가 브라우저의 sameSite 에서 **우리 허용목록 하나**로 옮겨온다. **대가**: 그 도메인 아래의 모든 서브도메인이 세션 쿠키를 읽으므로, 신뢰하지 않는 호스트를 그 아래 두지 않는다는 것이 이 결정에 딸려 오는 운영 약속이다 |
...
| 2 | CORS 와 쿠키 도메인을 세운다 — 같은 오리진에서는 프리플라이트가 일어나지 않으므로 현재 동작은 그대로다. 허용 오리진은 코드가 추측하지 않고 `NERV_WEB_URL` + `NERV_TRUSTED_ORIGINS` 의 합집합이며, better-auth 의 `trustedOrigins` 와 **같은 목록**이어야 한다(두 곳이 갈리면 로그인은 되는데 API 가 막힌다). 와일드카드는 쓰지 않는다 — `credentials: true` 와 `*` 는 함께 서지 않는다 | 남음 |
