...
| FR-15 | 에이전트 연동 | ◐ | ◐ | ◐ | MCP tools P0 8종 + P1 10종 = **18종**(§4.2) + PAT, Claude Code 플러그인 v1(스킬 5종 + hooks + `.mcp.json`), 훅 수집기 | Codex 완전 지원·`AGENTS.md` 배포·OAuth 2.1은 Phase 2(§5) |
...
### 4.2 MCP 도구 범위 — 18종 (P0 8종 + P1 10종)

도구 정의(입력·출력·권한·멱등성)는 [에이전트 연동 설계](../03-proposal/agent-integration.md) §2.3 카탈로그가 정본이며, 여기서는 이름·티어·Phase 배정만 인용한다. **MVP = 18종, 그 위에 Phase 2 리뷰 2종이 얹혀 카탈로그는 20종이다.**

**16종 → 18종 (2026-08-30 확정 — 사람 결정)**: `nerv_task_get`·`nerv_task_create` 를 더한다. 근거는 **표면이 비대칭이었다**는 사실이다 — REST 에는 Task 의 생성·단건 조회·목록이 처음부터 있었는데(`POST /tasks` · `GET /tasks/{task}` · `GET /tasks`) MCP 에는 `next`·`claim`·`heartbeat`·`release`·`update` 다섯뿐이었다. 그래서 에이전트는 **자기가 클레임할 다음 것만 볼 수 있었고**, 이미 진행 중인 Task 를 펼쳐 읽거나 "이건 이번 작업 밖의 별도 건이다" 를 남길 방법이 없었다 — 그 결과 별도 작업이 질문 카드로 흘러가거나 그냥 잊혔다(사람 보고 2026-08-30). 목록은 새 도구를 만들지 않는다: `nerv_task_next` 가 "지금 집을 수 있는 것"을 주고, 특정 건은 `nerv_task_get` 이 준다.
