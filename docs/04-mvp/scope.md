...
>
> 문서 버전 v0.20 · 2026-09-05 · HTML 파생본: [scope.html](../html/scope.html)
>
...
| FR-15 | 에이전트 연동 | ◐ | ◐ | ◐ | MCP tools P0 8종 + P1 14종 = **22종**(§4.2) + PAT, Claude Code 플러그인 v1(스킬 5종 + hooks + `.mcp.json`), 훅 수집기 | Codex 완전 지원·`AGENTS.md` 배포·OAuth 2.1은 Phase 2(§5) |
...
### 4.2 MCP 도구 범위 — 22종 (P0 8종 + P1 14종)
...
> **스킬이 5종인데 도구가 16종인 이유.** 스킬은 도구와 1:1이 아니다 — `/nerv:import`는 MCP 도구가 아니라 로컬 CLI를 실행하는 절차 스킬이다(§4.2). 플러그인이 배포하는 것은 "무엇을 어떤 순서로 하느냐"이고, 서버 권한은 여전히 PAT 권한이 정한다.
