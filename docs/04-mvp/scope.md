...
>
> 문서 버전 v0.27 · 2026-09-20 · HTML 파생본: [scope.html](../html/scope.html)
>
...
| 4 | 새 호스트 둘을 세우고 전환한다 — DNS·인증서·Ingress·CDN. **옛 호스트는 지우지 않고 남긴다**(되돌리는 길이 그것이다). 플러그인 훅의 하드코딩된 주소는 이 단계에서 `plugin.json` 버전과 **함께** 간다([4.6](plugin.md) · REQ-PLG-017) — 먼저 바꾸면 배달만 되고 서버는 옛 주소다 | 남음 |
