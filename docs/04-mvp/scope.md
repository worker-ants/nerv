...
>
> v0.33 변경(2026-09-24 — 파생본의 FR-12 가 v0.31 을 받지 못했다, 전수 대조): **새 결정 없음 · §3.2 FR-12 행의 파생본.** v0.31 이 md 의 FR-12 를 고치면서(**알림의** 메일 채널만 Phase 2 이고 가입 인증·초대 메일은 2026-09-22 에 들어왔다) **파생본은 "Slack·메일·다이제스트는 Phase 2" 인 채로 남았다** — 바로 다음 v0.32 가 같은 문서의 발신 행을 채우면서도 이 행은 지나쳤다. 둘 다 **고정 ID 가 없는 표 행**이라 규약 1 의 검사가 보지 못하는 자리이고, 그래서 같은 부류가 두 번 연달아 샜다. 검사를 ID 행까지 넓히는 일은 [4.2](codebase.md) v1.60 이 맡았지만(REQ-CB-051) **이 행은 여전히 사람이 지킨다.**
>
> v0.32 변경(2026-09-24 — 발신 스택의 스위치 이름이 바뀐다): **새 결정 없음.** §2.1 스택 표의 발신(메일) 행이 가리키는 설정 이름을 `NERV_SMTP_URL` 에서 `NERV_MAIL_HOST` 로 고쳤다 — 접속 정보가 여섯 키로 나뉘었다([4.2](codebase.md) §5.2 · REQ-CB-050). 스택 자체(nodemailer · SMTP)는 그대로다. 곁들여 **파생본에 이 행이 통째로 없던 것**을 채웠다(v0.31 이 md 만 고쳤다 — 규약 1 의 검사는 버전·절·고정 ID 만 보므로 ID 없는 표 행은 그대로 샌다).
>
...
>
> v0.29 변경(2026-09-20 — 공개 주소 분리 4단계 ②, 사람 지시): **§2.3 단계 표의 4단계 → 완료 · 새 REQ 없음**(행동 요구는 [4.6](plugin.md) REQ-PLG-017 이 갖는다). 플러그인이 들고 있던 하드코딩된 주소 13곳이 `api.` 로 갔고 `plugin.json` 이 0.2.23 이 됐다 — **버전과 함께** 가는 것이 이 표가 적어 둔 순서다. **네 단계가 저장소 쪽에서는 전부 닫혔다.** 남는 것은 운영 주체의 전환(DNS·인증서·CDN)이고, 그 절차와 되돌리는 길은 [4.2](codebase.md) §6.3a 다 — 이 표는 그 절차의 근거로 남는다.
>
> v0.28 변경(2026-09-20 — 공개 주소 분리 4단계 ①, 사람 지시): **§2.3 단계 표의 4단계 → 부분 · 새 REQ 없음**(행동 요구는 [4.2](codebase.md) REQ-CB-045·046). k8s 지형이 갈렸다 — base 는 `api.` 호스트의 표면 여섯만 갖고, 화면의 `/` 규칙과 웹 파드는 `base/web/` 한 덩어리로 base resources 밖에 섰다(이 문서가 확정해 둔 "웹 파드" 행의 실물이다). 전환 절차와 되돌리는 길은 [4.2](codebase.md) §6.3a 가 적는다. **남은 것은 플러그인의 하드코딩된 주소와 `plugin.json` 버전**이고, 그것이 이 표가 "먼저 바꾸면 배달만 되고 서버는 옛 주소다" 라고 적어 둔 그 순서다.
>
> v0.27 변경(2026-09-20 — 공개 주소 분리 3단계, 사람 지시): **§2.3 단계 표의 3단계 → 완료 · 새 REQ 없음**(행동 요구는 [4.2](codebase.md) REQ-CB-044). 화면이 API 주소를 런타임에 읽는다 — 앞문이 `/config.json` 을 내어 주고(CDN 배치는 같은 내용의 정적 파일), 부팅은 그 값을 읽은 **뒤에** 그린다. **파일이 없으면 같은 오리진**이므로 개발 루프와 지금의 한 호스트 배치는 한 줄도 바뀌지 않는다. **남은 것은 4단계**(DNS·인증서·Ingress·CDN 전환)이고, 그때 바꾸는 것은 번들이 아니라 그 파일의 값과 호스트다.
>
> v0.26 변경(2026-09-20 — 쿠키로 오는 쓰기를 서버가 대조한다, 사람 확정): **새 REQ 없음 — §2.3 확정 표의 "대가" 칸 한 줄**(행동 요구는 [4.2](codebase.md) REQ-CB-043). 쿠키 도메인을 넓히면 그 아래 모든 호스트가 세션 쿠키를 받고, `SameSite=Lax` 는 **같은 사이트**라 그 호스트를 막지 않는다 — 그 자리의 방어선이 운영 약속 하나뿐이었다. 이제 서버가 쓰기 요청의 `Origin` 을 허용 목록과 대조한다. 확정 셋과 단계 표는 그대로다.
>
> v0.25 변경(2026-09-20 — 공개 주소 분리 2단계, 사람 지시): **§2.3 단계 표의 2단계 → 완료 · 새 REQ 없음**(행동 요구는 [4.2](codebase.md) REQ-CB-041·042 가 갖는다). 허용 오리진을 `NERV_WEB_URL` + `NERV_TRUSTED_ORIGINS` 의 **한 목록**으로 세우고 CORS 와 better-auth 가 그것을 함께 본다. 쿠키에서 드러난 사실 하나를 이 표에 반영한다 — **서브도메인으로 가른다고 `Domain` 을 상위로 올려야 하는 것은 아니다**: `SameSite=Lax` 가 보는 것은 오리진이 아니라 **사이트**라, 같은 등록 도메인 아래면 호스트 전용 쿠키 그대로 선다. 그래서 위 확정 표의 "같은 등록 도메인 아래여야 한다" 는 그대로 살아 있고(그것이 `Lax` 를 세우는 조건이다), 상위 도메인 쿠키는 **손잡이**(`NERV_COOKIE_DOMAIN`)로 남는다 — 쓰면 딸려 오는 대가도 그대로다. 검증은 2단계에 적어 둔 대로 L2 가 브라우저 없이 프리플라이트 응답과 `Set-Cookie` 의 `Domain` 을 직접 센다. **3·4단계는 남았다.**
>
> v0.24 변경(2026-09-13 — 공개 주소를 둘로 가른다, 사람 확정): **§2.3 신설 · 새 REQ 없음(행동 요구는 [4.2](codebase.md) REQ-CB-036·037 이 갖는다).** 한 호스트가 화면과 API 를 경로로 나눠 서빙하던 것을 `app.` / `api.` **서브도메인 둘**로 가른다 — 웹을 CDN 정적 호스팅으로 내보내고 API 를 외부에 공개하기 위해서다. 확정 셋(호스트 이름 · env 이름 둘 다 신설 · 웹 파드를 오버레이로 내림)과 **각각에 딸려 오는 대가**, 그리고 네 단계의 순서를 적는다. **1단계는 완료다** — 이름만 갈랐고 두 값이 같은 오리진이라 동작은 바뀌지 않았다. 확정 스택(§2.1)은 한 줄도 건드리지 않는다: 이것은 스택 변경이 아니라 **배포 지형** 결정이다.
>
> v0.23 변경(2026-09-07 — 수가 실물과 달랐다, 개선 계획 여덟째 스프린트): **새 요구사항 없음.** 요약의 "MCP 도구는 20종" 을 **22종**(P0 8 + P1 14 · 카탈로그 24 — 리뷰 2종이 Phase 2 에서 얹혔다)으로, §2.1 의 "워크스페이스 4종" 을 **5종**(`plugin` 포함 — 문서 대조 테스트를 `pnpm test` 한 줄에 태우려고 워크스페이스에 둔다)으로 고친다. 같은 문서 §3.2 FR-15 는 이미 22종이라 적고 있었다 — 한 문서 안에서 두 수가 달랐다.
>
...
| 계층 | 확정 | 결정일 | 핵심 규약 |
| --- | --- | --- | --- |
| 언어/모노레포 | TypeScript + pnpm workspace — **구현 코드는 전부 저장소 `codebase/` 하위**. 워크스페이스 **5종**: `apps/web` · `apps/api` · `apps/cli` · `packages/schema` · `plugin`(배포되는 파일 묶음 — 문서 대조 테스트를 `pnpm test` 한 줄에 태우려고 워크스페이스에 둔다) | 2026-08-13 (3부 원안) · 코드 위치 2026-08-21 · `apps/cli` 2026-08-22 | Turborepo는 빌드 시간이 아플 때 도입(트리거만 기록). 모노레포 루트 = `codebase/`([4.2 코드베이스와 배포](codebase.md) §1, REQ-CB-015). `apps/cli`(`@nerv/cli` — 임포터)는 컨테이너가 아니라 **원본 체크아웃이 있는 장비에 설치되는 클라이언트**다([codebase.md](codebase.md) §1.3, REQ-CB-016·017) |
...
| 인증 | **better-auth** | 2026-08-20 (구성 정정 2026-09-24) | **웹 세션만 맡긴다**(이메일+비밀번호·세션 쿠키). 조직·멤버십은 도메인 테이블이 소유하고 **organization 플러그인은 켜지 않는다**(2026-08-22 구현 확정 — 켜면 역할의 정본이 둘이 된다 · [4.3](database.md) §2.16). PAT 는 api-key 플러그인이 아니라 **자체 `api_token` 테이블**이다(해시 저장·프로젝트 소속 · [4.3](database.md) §2.2 — 2026-09-24 사람 결정으로 문서를 실물에 맞춤: apiKey 테이블을 함께 쓰면 토큰이 두 곳에 살게 된다). OAuth 2.1 리소스 서버는 Phase 2 |
...
| TipTap | md 직렬화 왕복 손실 실측 발생 | Milkdown 재검토 — **2026-09-22 종료**(조건 소멸: 웹이 md 를 되돌려 보내지 않는다 · 아래 점화 기록 ②) |
...
>
> **2026-09-22 — 이 트리거는 종료됐다(조건 소멸).** 웹에서 스펙 본문 편집을 걷으면서([4.5](screens.md) REQ-WEB-173) 웹은 md 를 **화면으로 옮기기만** 하고 되돌려 보내지 않는다. 직렬화가 호출되지 않으므로 위 2유형이 **데이터에 닿을 경로 자체가 없어졌고**, 저장 게이트(REQ-WEB-031)도 막을 저장이 없어 함께 폐지됐다. 위의 실측과 최소 재현은 그대로 둔다 — `loss-probe.spec.ts`·`roundtrip-spike.spec.ts` 는 이제 **편집기의 충실도를 기록하는 스파이크**이지 게이트가 아니다. **조건이 사라진 것과 조건이 충족되지 않은 것은 다르다**: 언젠가 웹이 다시 md 를 쓰게 되면 이 줄이 그대로 되살아난다.
>
...
### 2.3 공개 주소 — `app.` / `api.` 서브도메인 분리 (2026-09-13 확정)

**한 호스트를 둘로 가른다.** 지금 `nerv.example.com` 하나가 화면과 API 를 **경로로** 나눠 서빙한다(§2.1 배포 행 · [4.2 코드베이스와 배포](codebase.md) §6.3 Ingress). 목적은 둘이다 — **웹을 CDN 정적 호스팅으로 내보내고, API 를 외부에 공개하는 것.** 아래 셋은 2026-09-13 사람 확정이며 §2.1 의 확정 스택과 같은 무게로 **재논의하지 않는다.**

| 확정 | 무엇 | 결정일 | 딸려 오는 것 |
| --- | --- | --- | --- |
| 호스트 이름 | `app.nerv.example.com`(웹) · `api.nerv.example.com`(API) | 2026-09-13 | **같은 등록 도메인 아래여야 한다.** 두 호스트가 같은 사이트이므로 `SameSite=Lax` 가 그대로 선다 — `SameSite` 가 보는 것은 오리진이 아니라 **사이트**라, 세션 쿠키를 상위 도메인으로 올리지 않아도(호스트 전용이어도) 화면이 API 를 부를 때 쿠키가 실린다(2026-09-20 정정 — 이 칸은 상위 도메인 쿠키를 그 조건인 것처럼 적고 있었다. 상위 도메인은 손잡이다 · [4.2](codebase.md) §5.2c). 완전히 다른 등록 도메인은 `SameSite=None; Secure` 를 강제하고, 그 순간 CSRF 방어가 브라우저의 sameSite 에서 **우리 허용목록 하나**로 옮겨온다. **대가**: 그 도메인 아래의 모든 서브도메인이 세션 쿠키를 읽으므로, 신뢰하지 않는 호스트를 그 아래 두지 않는다는 것이 이 결정에 딸려 오는 운영 약속이다. 2026-09-20 부터 그 약속 **위에** 서버 판정이 하나 더 선다 — 쿠키로 인증하는 쓰기는 `Origin` 을 허용 목록과 대조한다([4.2](codebase.md) §5.2c · REQ-CB-043) |
| env 이름 | `NERV_WEB_URL` · `NERV_API_URL` **둘 다 신설**, `NERV_PUBLIC_URL` 은 **걷는다** | 2026-09-13 | 한쪽이 옛 이름을 물려받으면 "어느 뜻을 물려받았는가" 가 소비자마다 다시 물어야 할 질문이 된다. **대가**: 기존 배치가 전부 깨진다 — 그래서 옛 이름만 설정된 서버는 **기동을 거부한다**([4.2](codebase.md) §5.2 걷힌 이름 · REQ-CB-036·037) |
| 웹 파드 | `base/web/` 를 **base resources 에서 내리고 파일은 남긴다** — 파드가 필요한 오버레이만 그것을 더한다 | 2026-09-13 | `base/embed/` 가 이미 쓰는 패턴이다(파일은 base 에 있고 `resources:` 에는 없다). CDN 배치는 웹 파드 없이 서고, 자가호스팅(NFR-01)은 오버레이가 `web/` 와 `/` → `nerv-web` ingress 규칙을 함께 더해 선다 — **ingress 도 같이 갈린다**: base 는 API 경로만 갖고 `/` 규칙은 자가호스팅 오버레이의 몫이다 |

**순서가 곧 되돌릴 수 있음이다.** 네 단계로 옮기고, 3단계까지는 **옛 주소가 그대로 살아 있다.**

| 단계 | 무엇 | 상태 |
| --- | --- | --- |
| 1 | **이름 둘을 신설하고 옛 이름을 걷는다** — 도메인은 그대로. 여섯 소비자가 어느 쪽을 쓰는지만 정한다. 두 값이 같은 오리진이므로 **동작은 한 줄도 바뀌지 않는다** | **완료**(2026-09-13 · [4.2](codebase.md) REQ-CB-036·037) |
| 2 | CORS 와 쿠키 도메인을 세운다 — 같은 오리진에서는 프리플라이트가 일어나지 않으므로 현재 동작은 그대로다. 허용 오리진은 코드가 추측하지 않고 `NERV_WEB_URL` + `NERV_TRUSTED_ORIGINS` 의 합집합이며, better-auth 의 `trustedOrigins` 와 **같은 목록**이어야 한다(두 곳이 갈리면 로그인은 되는데 API 가 막힌다). 와일드카드는 쓰지 않는다 — `credentials: true` 와 `*` 는 함께 서지 않는다. **쿠키 도메인은 손잡이로 둔다**(`NERV_COOKIE_DOMAIN` · 기본 비움): 같은 등록 도메인 아래면 호스트 전용 쿠키로 `Lax` 가 그대로 서므로 상위 도메인은 넓힐 때만 적고, 두 주소의 공통 상위가 아니면 기동을 거부한다 | **완료**(2026-09-20 · [4.2](codebase.md) §5.2c · REQ-CB-041·042) |
| 3 | 웹을 **런타임 설정**으로 부팅시킨다 — 빌드 타임(`VITE_*`)에 구우면 환경마다 다른 번들이 되어 "같은 산출물을 승격한다" 가 깨진다. SPA 가 자기 오리진에서 설정 하나를 읽고, 배포는 그 파일만 환경에 맞게 놓는다. **없으면 같은 오리진이다** — 파일이 없는 배치(개발 루프)가 정상 경로이므로 폴백이 지금까지의 동작이다 | **완료**(2026-09-20 · [4.2](codebase.md) §5.2d · REQ-CB-044) |
| 4 | 새 호스트 둘을 세우고 전환한다 — DNS·인증서·Ingress·CDN. **옛 호스트는 지우지 않고 남긴다**(되돌리는 길이 그것이다). 플러그인 훅의 하드코딩된 주소는 이 단계에서 `plugin.json` 버전과 **함께** 간다([4.6](plugin.md) · REQ-PLG-017) — 먼저 바꾸면 배달만 되고 서버는 옛 주소다 | **완료**(2026-09-20) — k8s 지형은 base 가 `api.` 호스트만 갖고 화면이 `base/web/` 한 덩어리로 갈라진 모양이며([4.2](codebase.md) §6.3 · REQ-CB-045·046), 플러그인의 배달 주소 13곳이 `api.` 로 가고 `plugin.json` 이 0.2.23 이 됐다([4.6](plugin.md) · REQ-PLG-017). **실제 DNS·인증서 전환은 운영 주체의 몫**이고 그 절차와 되돌리는 길은 [4.2](codebase.md) §6.3a 다 |

> **개발 루프는 가르지 않는다.** compose 는 앞문 하나가 화면과 API 를 함께 서빙하는 한 포트를 유지한다 — 로컬에서까지 호스트를 가르면 사람마다 `/etc/hosts` 를 고쳐야 하고, 그것은 NFR-01("명령 몇 개로 로그인 화면까지")과 정면으로 부딪힌다. **대가를 명시한다**: 개발에서는 같은 오리진이라 CORS 경로가 로컬에서 한 번도 검증되지 않는다(로컬 초록이 CI 초록을 뜻하지 않는다 — [AGENTS.md](../../AGENTS.md) 구현 규약 7 이 겪은 모양이다). 그래서 2단계에서 **L2 가 프리플라이트 응답과 `Set-Cookie` 의 `Domain` 을 직접 센다** — 브라우저 없이 되는 검증이다.
>
> **영향이 없다고 확인한 것**(2026-09-13 실측): 웹 클라이언트에 `EventSource` 사용 **0건** — `/sse` 는 에이전트·CLI 전용이라 브라우저 CORS 대상이 아니다. S3 presigned 는 이미 별도 호스트다([4.2](codebase.md) REQ-CB-034). MCP·훅 인증은 PAT 헤더라 쿠키와 무관하다.
...
- [4.6 플러그인과 온보딩](plugin.md) — §2 스킬 5종 전문과 Codex 경계
