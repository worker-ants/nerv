...
>
> 문서 버전 v0.15 · 2026-09-02 · HTML 판: [scope.html](../html/scope.html)
>
...
| FR-15 | 에이전트 연동 | ◐ | ◐ | ◐ | MCP tools P0 8종 + P1 12종 = **20종**(§4.2) + PAT, Claude Code 플러그인 v1(스킬 5종 + hooks + `.mcp.json`), 훅 수집기 | Codex 완전 지원·`AGENTS.md` 배포·OAuth 2.1은 Phase 2(§5) |
...
### 4.2 MCP 도구 범위 — 20종 (P0 8종 + P1 12종)

도구 정의(입력·출력·권한·멱등성)는 [에이전트 연동 설계](../03-proposal/agent-integration.md) §2.3 카탈로그가 정본이며, 여기서는 이름·티어·Phase 배정만 인용한다. **MVP = 20종, 그 위에 Phase 2 리뷰 2종이 얹혀 카탈로그는 22종이다.**
...
티어 표기는 도구 위험 티어 **A1~A4**다. 스펙 변경 게이트 티어 **T0~T3**([스펙 워크플로우와 거버넌스](../03-proposal/spec-workflow.md) §2.4)와는 **다른 축**이므로 혼용하지 않는다. MCP 도구 22종 ↔ 내부 서비스 ↔ REST 대응 표는 [4.4 API 명세](api.md) §4가 소유한다.
