...
| FR-15 | 에이전트 연동 | ◐ | ◐ | ◐ | MCP tools P0 8종 + P1 11종 = **19종**(§4.2) + PAT, Claude Code 플러그인 v1(스킬 5종 + hooks + `.mcp.json`), 훅 수집기 | Codex 완전 지원·`AGENTS.md` 배포·OAuth 2.1은 Phase 2(§5) |
...
### 4.2 MCP 도구 범위 — 19종 (P0 8종 + P1 11종)

도구 정의(입력·출력·권한·멱등성)는 [에이전트 연동 설계](../03-proposal/agent-integration.md) §2.3 카탈로그가 정본이며, 여기서는 이름·티어·Phase 배정만 인용한다. **MVP = 19종, 그 위에 Phase 2 리뷰 2종이 얹혀 카탈로그는 21종이다.**
